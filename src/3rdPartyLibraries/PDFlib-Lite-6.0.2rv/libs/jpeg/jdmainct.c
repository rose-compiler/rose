 ...
 *
 * This file contains the jmain buffer controller for decompression.
 * The jmain buffer lies between the JPEG decompressor proper and the
 * post-processor; it holds downsampled data in the JPEG colorspace.
 *
 * Note that this code is bypassed in raw-data mode, since the application
 * supplies the equivalent of the jmain buffer in that case.
 */
...
/*
 * In the current system design, the jmain buffer need never be a full-image
 * buffer; any full-height buffers will be found inside the coefficient or
 * postprocessing controllers.  Nonetheless, the jmain controller is not
...
typedef my_dmain_controller * my_main_ptr;
...
jinit_d_main_controller (j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  jmain = (my_main_ptr)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				SIZEOF(my_dmain_controller));
  cinfo->jmain = (struct jpeg_d_main_controller *) jmain;
