...
/* $Id: pc_output.c,v 1.2 2007/01/16 05:14:37 dquinlan Exp $
 *
...
/* PC (9/23/2006): Helper function for MacOSX only, taken from MoreFilesX.C sample code */
#if (defined(MAC) || defined(MACOSX)) && TARGET_API_MAC_CARBON && !defined(__MWERKS__)
static OSStatus
FSPathMakeFSSpec(
  const UInt8 *path,
  FSSpec *spec,
    ...
    size_t (*writeproc)(pdc_output *out, void *data, size_t size))
{
	...
	if (filename && !strcmp(filename, "-")) {
	    ...
	    if (FSPathMakeFSSpec((const UInt8 *) filename, &fSpec, NULL) == noErr) {
