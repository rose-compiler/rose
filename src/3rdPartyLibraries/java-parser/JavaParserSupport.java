...
class JavaParserSupport {
    ...
    class LocalOrAnonymousType {
        ...
        public String qualifiedName() {
            return (package_name.length() > 0 ? (package_name + ".") : package_name) + typename + "." + (isAnonymous ? "" : ("_" + method_name + "_." + simplename));
        }
        
        public LocalOrAnonymousType(String package_name, String typename, String simplename, String method_name, boolean isAnonymous) {
    ...
    //
    public HashMap<String, TypeDeclaration> userTypeTable = new HashMap<String, TypeDeclaration>();
    ...
    public String getMethodKey(MethodBinding method_binding) {
        ...
        String result = getCanonicalName(method_binding.declaringClass) + "_" + getMethodName(method_binding) + "(";
        if (method_binding.parameters != null) {
            for (int i = 0, length = method_binding.parameters.length; i < length; i++) {
                if (i > 0)
                    result += ", ";
                result += method_binding.parameters[i].debugName();
            }
        }
        return result + ")";
    }
    ...
    public int getMethodIndex(MethodBinding method_binding) {
        ...
        return typeDeclarationMethodTable.get(key);
    }
    ...
    void identifyUserDefinedTypes(TypeDeclaration node, UnitInfo unit_info) {
        ...
        String full_name = getCanonicalName(node.binding);
        userTypeTable.put(full_name, node); // keep track of classes mapped to user-defined TypeDeclarations
        ...
        if (unit.types != null) { // There are type declarations!?
            ...
            for (int i = 0, max = unit.types.length; i < max; i++) {
                ...
                if (node.name != TypeConstants.PACKAGE_INFO_NAME) { // package-info are not in the class table
                    ...
                    if (processed_object == null) { // Type not yet processed. Process this java source.
                    ...
                    else if (! (processed_object instanceof TypeDeclaration)) { // this type was already processed by a class or a ReferenceBinding?
                        ...
                        throw new DuplicateTypeException("Invalid attempt to redefine the type " + getCanonicalName(node.binding) + "\n" + 
                                                         "The prior definition is in file " + new String(((TypeDeclaration) processed_object).compilationResult.fileName) + "\n" +
                                                         "This definition is in file " + new String(node.compilationResult.fileName));
                    }
                }
            }
    ...
    public void processConstructorDeclarationHeader(ConstructorDeclaration constructor, JavaToken jToken) {
        ...
        boolean is_public = (constructor.binding != null) && (constructor.binding.isPublic());
        boolean is_protected = (constructor.binding != null) && (constructor.binding.isProtected());
    ...
    public void processMethodDeclarationHeader(MethodDeclaration method, JavaToken jToken) {
        String name = getMethodName(method.binding);
     ...
     * @param jToken
     */
    void processQualifiedNameReference(QualifiedNameReference node, UnitInfo unit_info) {
        ...
        if (node.binding instanceof FieldBinding) {
            type_binding = node.actualReceiverType; // Trying this for now ....
        }
    ...
    public String getPackageName(TypeBinding binding) {
        if (binding instanceof LocalTypeBinding) {
            ...
            return (special_type.isAnonymous() ? special_type.package_name : "");
        }
...
//System.out.println("*** Checking name " + package_name + " -- " + type_name);
...
//if (found != null)
//System.out.println("*** Found type " + qualified_name + " in main package");
...
//System.out.println("*** Type " + qualified_name + " found a perfect match with as a single import");                    
                        return false;
                    }
                    else if (type_name.equals(new String(reference_binding.shortReadableName()))) {
//if (reference_binding.isNestedType())
//System.out.println("*** Inner Type " + qualified_name + " found a perfect match with as a single import; type_name = " + type_name);                    
//else System.out.println("*** Found type " + qualified_name + "\'s simple name matches the name in a single import");
                        return true;
                    }
                }
...
//System.out.println("*** Type " + qualified_name + "was found in an on-demand import");                    
...
//System.out.println("*** No conflict found for type " + qualified_name + " in main package");
        }
    ...
    void addEnumValuesMethodDeclaration(TypeDeclaration node, UnitInfo unit_info, int method_index) {
        JavaToken location = unit_info.getDefaultLocation();
        generateAndPushType(node.binding, unit_info, location, true /* is_formal_parameter_type_mapping */);
    ...
    void addEnumValueOfMethodDeclaration(TypeDeclaration node, UnitInfo unit_info, int method_index) {
        ...
        generateAndPushType(string_binding, unit_info, location, true /* is_formal_parameter_type_mapping */);
    ...
    boolean addEnumValuesMethodDeclaration(ReferenceBinding type_binding, UnitInfo unit_info, int method_index) {
        ...
        // 
        TypeBinding parameter_type_bindings[] = new TypeBinding[0];
        MethodBinding method_binding = type_binding.getExactMethod("values".toCharArray(), parameter_type_bindings, null);
        ...
        //
        JavaToken location = unit_info.getDefaultLocation();
        generateAndPushType(type_binding, unit_info, location, true /* is_formal_parameter_type_mapping */);
        JavaParser.cactionArrayTypeReference(1, location);
        method_index++; // calculate token index for method "values"
    ...
    boolean addEnumValueOfMethodDeclaration(ReferenceBinding type_binding, UnitInfo unit_info, int method_index) {
        ...
        // Check to see if the values() method is contained in type_binding and has been processed already. 
        // 
        TypeBinding string_binding = type_binding.getExactMethod("toString".toCharArray(), new TypeBinding[0], null).returnType;
    ...
    private void buildMethod(UnitInfo unit_info, AbstractMethodDeclaration method, int method_index) {
        ...
        if (method.typeParameters() != null) {
            ...
            for (int i = 0; i < type_parameters.length; i++) {
                ...
                JavaParser.cactionBuildTypeParameterSupport(type_parameter.binding.debugName(), (unit_info == null ? default_location : unit_info.createJavaToken(type_parameter)));
            }
        }
        ...
        if (method.isConstructor()) {
            ...
            generateAndPushType(method.binding.returnType, unit_info, method_location, true /* is_formal_parameter_type_mapping */);
        }
...
if (method_name.equals("makeYoungest")) {
System.out.println("Argument " + j + " is of type " + arg.type.resolvedType.debugName() + " (" + arg.type.resolvedType.getClass().getCanonicalName() + ")");
}

                generateAndPushType(arg.type.resolvedType, unit_info, arg_location, true /* is_formal_parameter_type_mapping */);
...
//System.out.println("Traverse Reference Binding of " + qualified_name);
        ...
        //
        //
        if (binding.debugName().equals("java.lang.Object")) { // If we are processing Object, signal this to the translator.
        ...
        if (type_variables != null) {
            for (int i = 0; i < type_variables.length; i++) {
                ...
                JavaParser.cactionBuildTypeParameterSupport(type_parameter_name, location);
            }
        }
        ...
        if ((! binding.isInterface()) && super_class != null) {
            ...
            //
            generateAndPushType(super_class, unit_info, location, false /* is_formal_parameter_type_mapping */);
        }
        ...
        if (interfaces != null) {
            for (int i = 0; i < interfaces.length; i++) {
                ...
                // When we have access to a user-specified source, we use it in case this type is a parameterized type.
                //
                generateAndPushType(interfaces[i], unit_info, location, false /* is_formal_parameter_type_mapping */);
            }
        }
        ...
        if (field_bindings != null) {
            for (int i = 0; i < field_bindings.length; i++) {
                ...
                if (field_binding.type != null) { // May be null if enum field
                    ...
                    generateAndPushType(field_binding.type, unit_info, location, false /* is_formal_parameter_type_mapping */);
        ...
        try {
            method_bindings = binding.methods();
        }
        catch(Throwable e) {
            ; // do nothing
        }
            ...
            for (int i = 0; i < method_bindings.length; i++) {
                ...
                if (method_type_variables != null) {
                    for (int k = 0; k < method_type_variables.length; k++) {
                        ...
                        JavaParser.cactionBuildTypeParameterSupport(new String(method_type_variable.sourceName), location);
                    }
                }
                ...
                if (method_binding.isConstructor()) {
                    ...
                    generateAndPushType(method_binding.returnType, unit_info, location, true /* is_formal_parameter_type_mapping */);
                }
                ...
                if (parameters != null) {
                    for (int j = 0; j < parameters.length; j++) {
                        ...
                        generateAndPushType(parameter, unit_info, location, true /* is_formal_parameter_type_mapping */);
...
//System.out.println("Traverse Type Declaration of " + qualified_name);
        ...
        if (node.binding.debugName().equals("java.lang.Object")) { // If we are processing Object, signal this to the translator.
            JavaParser.cactionSetupObject();
        }
        ...
        if (verboseLevel > 2)
         ...
         if (parameters != null) {
             ...
             for (int i = 0; i < parameters.length; i++) {
                 ...
                 JavaParser.cactionBuildTypeParameterSupport(new String(parameter.name), location);
             }
         }
        ...
        if ((! node.binding.isInterface()) && node.binding.superclass != null) {
            ...
            // case of an Anonymous type, node.superclass is null!
            //
            generateAndPushType(node.binding.superclass, unit_info, (node.superclass == null ? unit_info.getDefaultLocation() : unit_info.createJavaToken(node.superclass)), false /* is_formal_parameter_type_mapping */);
        }
        ...
        for (int k = 0; k < node_list.length; k++) {
            ...
            if (class_member instanceof TypeDeclaration) {
                ...
                if (field instanceof Initializer) {
                    ...
                    generateAndPushType(field.binding.type, unit_info, field_location, false /* is_formal_parameter_type_mapping */);
...
//System.out.println("Updating ReferenceBinding for " + qualified_name);
        ...
        if (node.typeParameters != null) {
            for (int i = 0; i < node.typeParameters.length; i++) {
                ...
                JavaParser.cactionUpdateTypeParameterSupport(new String(parameter.name), -1 /* method_index*/, unit_info.createJavaToken(parameter));
            }
        }
        ...
        for (int k = 0; k < node_list.length; k++) {
            ...
            if (class_member instanceof TypeDeclaration) {
                ...
                if (! typeDeclarationMethodTable.containsKey(getMethodKey(binary_binding))) { // Certain incompletely processed methods are ignored in BinaryTypeBinding in the preprocessing phase.
                    ...
                    if (args != null) {
                        for (int j = 0; j < args.length; j++) {
                            JavaToken arg_location = unit_info.createJavaToken(args[j]);
                            generateAndPushType(args[j].type.resolvedType, unit_info, arg_location, true /* is_formal_parameter_type_mapping */);
                        }
                    }
                    ...
                    if (method.typeParameters() != null) {
                        ...
                        for (int i = 0; i < num_type_parameters; i++) {
                            TypeParameter type_parameter = type_parameters[i];
                            JavaParser.cactionUpdateTypeParameterSupport(type_parameter.binding.debugName(), assigned_index, unit_info.createJavaToken(type_parameter));
                        }
                    }
        ...
        if (type_binding instanceof ParameterizedTypeBinding) {
            ...
            if (type_binding instanceof CaptureBinding) {
                ...
                if (capture_binding.wildcard != null) {
                    generateAndPushType(capture_binding.wildcard, unit_info, location, is_formal_parameter_type_mapping);
                }
                else {
                ...
                if (scope_binding instanceof TypeBinding) {
                    ...
                    if (is_formal_parameter_type_mapping) {
                        JavaParser.cactionArgumentTypeParameterReference(package_name, type_name, (int) -1 /* no method index */, type_parameter_name, location);
                    }
                    else {
            ...
            if (! wildcard_binding.isUnboundWildcard()) { // there is a bound!
                ...
                generateAndPushType(wildcard_binding.bound, unit_info, location, is_formal_parameter_type_mapping);
            }
...
// -------------------------------------------------------------------------------------------
    ...
    public void translate(ArrayList<CompilationUnitDeclaration> units, String language_level) {
        ...
        if (this.objectBinding == null) {
            ...
            this.objectBinding = (ReferenceBinding) TypeBinding.wellKnownType(unit_info.unit.scope, TypeIds.T_JavaLangObject);
            preprocessClass(this.objectBinding, unit_info);
...
System.out.println("Starting translation of " + unit_info.fileName);
