	...
	connection( false )
{
   ...
   // DQ & NT (10/23/2003): Added support for configuration of MySQL
	setDatabaseParameters("" , "" , "", "",&myPort,&myCompress,&myTimeout,&myTe,mySock,&myFlag);
}
...
void GlobalDatabaseConnection::DEBUG_dump( void ) {
	...
	if(connected) {
		...
		Result *tables = select("SHOW TABLES;");
		for(Result::iterator i= tables->begin(); i!=tables->end(); i++ ) {
			Row row = *i;
...
int GlobalDatabaseConnection::initialize( int drop ) {
	...
	if(! connection.real_connect( "", dbHost.c_str(), dbUsername.c_str(), dbPwd.c_str(), dbPort, dbCompress, dbTimeout, dbSock.c_str(), dbFlag ) ) {
		cout << " con fail " << endl;
		return 1;
	}
	...
	if(! connection.select_db( dbName ) ) {
		...
		cout << " creating db " << endl; // debug
		connection.create_db( dbName );
		if(! connection.select_db( dbName ) ) {
			cout << " create fail " << endl; // debug
			return 2;
		}
	}
