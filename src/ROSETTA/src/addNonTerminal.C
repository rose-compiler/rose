...
// ******************************************************
// This file is a copy of the addTerminal.C file (we will
// make modifications as required to handle NonTerminals)
// ******************************************************
...
#include <sage3.h>
#define variant_type_enum Cxx_GrammarVariants
#define LAST_TAG Cxx_Grammar_LAST_TAG
...
void
Grammar::addNewNonTerminal ( SgProject & project, NonTerminal & X )
   {
  ...
  //   -->  (realArray:doubleArray,floatArray; intArray, Indexing: Index,Range).
     ...
     printf ("Inside of Grammar::addNewNonTerminal (NonTerminal) X(%s:%s) \n",X.lexeme,X.name);
     ...
     if (X.isType() == TRUE)
	  ...
	  BUILD_CHILD_TERMINAL_MACRO ( X_ReferenceType       , "ReferenceType",
       ...
       //                             "X_ModifierType"       , "X_T_MODIFIER" )
          ...
          BUILD_CHILD_TERMINAL_MACRO ( X_TemplateInstantiationType, "TemplateInstantiationType",
                                      "X_TemplateInstantiationType","X_T_TEMPLATE_INSTANTIATION")
#if 0
       ...
       //                             "X_ClassType"          , "X_T_CLASS" )
          BUILD_CHILD_NONTERMINAL_MACRO ( X_ClassType, X | X_TemplateInstantiationType,
                                         "ClassType", "X_ClassType", "X_T_CLASS" )
#else
       ...
       // BUILD_CHILD_NONTERMINAL_MACRO (X_ClassType, X, "ClassType","X_ClassType","X_T_CLASS");
          NonTerminal    &   ClassType    = getNonTerminal("ClassType");

       // Since the ClassType is already a nonterminal we don't have to convert it to a nonterminal!
	  ...
	  ROSE_ASSERT(X_ClassTypePtr != NULL);
       ...
       // But not for nonTerminals
          X_ClassType.copyTerminalData (&ClassType);

          printf ("Need to copy the child terminals and nonterminals to handle addition of nonterminal! \n");
       ...
       // because it is a child terminal and for the X_ClassType since no SgX_ClassType 
...
#if 0
       // Actually, we don't want to debug it now!
       ...
       // printf ("X_FunctionType ---> ");       X_FunctionType.show(); printf ("\n");
          ...
          printf ("X_ClassType ---> ");          X_ClassType.show(); printf ("\n");
     ...
     if (X.isStatement() || X.isExpression() || X.isType())
       ...
       // new expressions (and types if Y is a type).
          if (X.isExpression() || X.isType())
            ...
            // DQ (4/11/2001) These are no longer terminals (added a template child terminal)
            ...
            //                             "X_ClassDefinition",          "X_CLASS_DEFN_STMT" );
               BUILD_CHILD_TERMINAL_MACRO ( X_TemplateInstantiationDecl, "TemplateInstantiationDecl",
                                           "X_TemplateInstantiationDecl","X_TEMPLATE_INST_DECL_STMT" );
               BUILD_CHILD_TERMINAL_MACRO ( X_TemplateInstantiationDefn, "TemplateInstantiationDefn",
                                           "X_TemplateInstantiationDefn","X_TEMPLATE_INST_DEFN_STMT" );

            // DQ (4/11/2001) These are no longer terminals (added a template child terminal)
               BUILD_CHILD_NONTERMINAL_MACRO (X_ClassDeclaration,X_TemplateInstantiationDecl,
                                              "ClassDeclaration","X_ClassDeclaration","X_CLASS_DECL_STMT");
               BUILD_CHILD_NONTERMINAL_MACRO (X_ClassDefinition,X_TemplateInstantiationDefn,
                                              "ClassDefinition","X_ClassDefinition","X_CLASS_DEFN_STMT");
  ...
  // Now look at the AST to find the class associated with the input Terminal or nonterminal
  // and add it's member functions to the grammar.  This is one of the more interesting ways
  // in which ROSETTA permits the construction of highly specialized grammars (automatically).

  // Traverse the files a look for the declaration asociated with the input class declaration terminal
     int i = 0;
     for (i=0; i < project.numberOfFiles(); i++)
       ...
       // get the global scope
          SgGlobal* globalScopeStatement = file.get_root();
          ROSE_ASSERT (globalScopeStatement != NULL);

          SgClassDeclaration* classDeclarationPtr = NULL;

          printf ("Now look for all declarations in the global scope! (looking for %s) \n",X.getName());

	  SgDeclarationStatementPtrList::iterator i = globalScopeStatement->get_declarations().begin();
	  while ( i != globalScopeStatement->get_declarations().end() )
             {
            // if ( ( (*i)->variant() == CLASS_DECLARATION) && ( ) )
               if ( (classDeclarationPtr = isSgClassDeclaration(*i)) )
                  {
                    ROSE_ASSERT (classDeclarationPtr != NULL);
                    printf ("classDeclarationPtr->get_name() = %s \n",classDeclarationPtr->get_name().str());
                  }
#if 1
               if ( ( classDeclarationPtr = isSgClassDeclaration(*i) ) && (classDeclarationPtr->get_name() == X.getName() ) )
                    break;
#endif
               i++;
             }
          ...
          ROSE_ASSERT (classDeclarationPtr != NULL);
          SgClassDefinition* classDefinitionPtr = classDeclarationPtr->get_definition();
          ROSE_ASSERT (classDefinitionPtr != NULL);
          SgDeclarationStatementPtrList & memberList = classDefinitionPtr->get_members();
	  SgDeclarationStatementPtrList::iterator j = memberList.begin();

       // loop through the list of member declarations
          for (j = memberList.begin(); j != memberList.end(); j++)
             {
            // We will add terminals to represent only the public members
               if ( (*j)->get_declarationModifier().get_accessModifier().isPublic() )
                  {
                    SgFunctionDeclaration*       functionDeclaration       = isSgFunctionDeclaration(*j);
                    SgMemberFunctionDeclaration* memberFunctionDeclaration = isSgMemberFunctionDeclaration(*j);
                    if ( (functionDeclaration != NULL) || (memberFunctionDeclaration != NULL) )
                       {
                      // get the name, copy a function terminal, and add it to the current location
                         if ( (functionDeclaration != NULL) && (memberFunctionDeclaration == NULL) )
                            {
                              printf ("Found a non-member function (name = %s) \n",functionDeclaration->get_name().str());
                            }
                         if (memberFunctionDeclaration != NULL)
                            {
                              printf ("Found a member function (name = %s) \n",memberFunctionDeclaration->get_name().str());
                            }

                         if (functionDeclaration != NULL)
                            {
                              printf ("Building a new terminal (name = %s) \n",functionDeclaration->get_name().str());

#if 0       // BP: 09172001 a bug in the code I presume .....
                           // Generate strings for representation of new terminal (based on function name)
                              BUILD_CHILD_TERMINAL_MACRO ( X_PragmaStatement,           "PragmaStatement",
                                                          "X_PragmaStatement",          "X_PRAGMA_STMT" );
#else
			      // for now do nothing
...
void
Grammar::removeNewNonTerminal (NonTerminal & X )
   {
