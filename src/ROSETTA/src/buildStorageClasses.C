...
Grammar::buildStringForStorageClassSource(AstNodeClass& node) {
    ...
    if (node.hasMembersThatAreStoredInEasyStorageClass()) {
        ...
        writeEasyStorageData += "    (void) out; // avoid compiler warning\n";
        writeEasyStorageData += node.buildStorageClassWriteStaticDataToFileSource();
        writeEasyStorageData += "}\n\n";

        // building the source code of the static member function readEasyStorageDataToFile
        readEasyStorageData  = "\nvoid\n";
        readEasyStorageData += "$CLASSNAMEStorageClass::readEasyStorageDataFromFile(std::istream& in) {\n";
        readEasyStorageData += "    (void) in; // avoid compiler warning\n";
        readEasyStorageData += node.buildStorageClassReadStaticDataFromFileSource();
        readEasyStorageData += "}\n\n";
    }
    // computing the length of the resulting string
    return pickOutIRNodeData + arrangeStaticDataInOneBlock + deleteStaticData + writeEasyStorageData + readEasyStorageData;
}
...
Grammar::buildAccessFunctionSources(AstNodeClass& node) {
    ...
    std::string temp{functionSource};
...
Grammar::generateStorageClassesFiles()
  ...
  // Building the file StorageClasses.h
     ...
     ASSERT_require(!this->astVariantToNodeMap.empty());
...
AstNodeClass::evaluateType(std::string& varTypeString) {
    ...
    // This might change, as soon as the enum types have a common name style ...
                 ...
                 (varTypeString == "SgLabelStatement::label_type_enum") ||
              ...
              // Note that these enum names do not conform to the naming scheme used in ROSE.
                 ...
                 (varTypeString == "SgAsmJvmConstantPoolEntry::Kind") ||
...
std::string AstNodeClass::buildStorageClassHeader() {
    ...
    std::vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};

    for (auto stringListIterator = copyList.begin(); stringListIterator != copyList.end(); stringListIterator++) {
        GrammarString* data{*stringListIterator};
        std::string varNameString{string(data->getVariableNameString())};
        std::string varTypeString{string(data->getTypeNameString())};
        std::string varStorageNameString{"storageOf_" + varNameString};
        if (varNameString != "freepointer" && varTypeString.substr(0,7) != "static ") {
            switch (evaluateType(varTypeString)) {
              ...
              default:
                 ...
                 */
                std::cout << " There is a class not handled in buildStorageClasses.C, Line " << __LINE__ << std::endl;
                std::cout << "In class " + classNameString + " caused by variable " << varTypeString + " p_" + varNameString << std::endl;
                ASSERT_require2(false, "Stop immediately, since variable to build is not found ...\n");
                break;
            }
        }
    }
 ...
 */
std::string AstNodeClass::buildStorageClassPickOutIRNodeDataSource()
   {
     std::vector<GrammarString*> copyList;
     std::string classNameString{this->name};
     std::string s;
     s += "    " + classNameString + "* source = (" + classNameString +"*) (pointer);\n";
     s += "#if FILE_IO_EXTRA_CHECK \n";
     s += "    ASSERT_not_null(source);\n";
     s += "    ASSERT_not_null(source->p_freepointer);\n";
     s += "#endif \n";
                    ...
                    switch (evaluateType(varTypeString))
                      ...
                      // DQ (5/1/2009): Added support for rose_graph_integerpair_edge_hash_multimap
                         ...
                         case SGCLASS_POINTER_LIST_POINTER:
                           ...
                           s += "    if (source->p_" + varNameString + " != nullptr) {\n";
...
string AstNodeClass::buildSourceForIRNodeStorageClassConstructor ()
   {
     ...
     s += "    ASSERT_require(p_freepointer == AST_FileIO::IS_VALID_POINTER());\n";
               ...
               switch (evaluateType(varTypeString))
                  {
                    ...
                    case ROSE_GRAPH_INTEGER_PAIR_EDGE_HASH_MULTIMAP:
                      ...
                      s += "         integerpair_edge_it->second = nullptr; printf (\"Error: not implemented support for rose_graph_integerpair_edge_hash_multimap \\n\"); ASSERT_require(false);\n";
                      s += "     }\n";
                      break;
...
*/
std::string AstNodeClass::buildStorageClassReadStaticDataFromFileSource() {
    std::vector<GrammarString*> copyList;
    std::string classNameString{this->name};
    std::string s;
    std::string addString;
...
bool AstNodeClass::hasStaticMembers()
   {
     ...
     vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};
 ...
 */
std::string
AstNodeClass::buildStaticDataMemberListConstructor() {
    std::string s;
    std::string classNameString{this->name};
    std::vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};
    ...
    for (auto stringListIterator = copyList.begin(); stringListIterator != copyList.end(); stringListIterator++) {
        GrammarString* data{*stringListIterator};
        std::string varNameString{std::string(data->getVariableNameString())};
        std::string varTypeString{std::string(data->getTypeNameString())};
        if (varTypeString.substr(0,7) == "static ") {
            varTypeString = varTypeString.substr(7, varTypeString.size()-1);
          ...
          if (varTypeString.substr(0,7) == "static ")
             {
               ...
               if (evaluateType(varTypeString) == SGCLASS_POINTER )
                  {
                    s += "    if (" + classNameString + "_" + varNameString + " != nullptr) {\n";
                    s += "        if (" + classNameString + "::p_" + varNameString + " == nullptr) {\n";
 ...
 */
std::string
AstNodeClass::buildAccessFunctionsForStaticDataMember() {
    std::string s;
    std::string classNameString{this->name};
    std::vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};
...
//#########################################################################################################
/* JH (10/28/2005) : The pickOutData generation of the AstSpecificDataManagingClassStorageClass
 */
 ...
 */
std::string
AstNodeClass::buildStaticDataConstructorSource() {
    std::string s;
    std::string classNameString{this->name};
    std::vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};
    ...
    for (auto stringListIterator = copyList.begin(); stringListIterator != copyList.end(); stringListIterator++) {
        ...
        std::string varNameString{std::string(data->getVariableNameString())};
        std::string varTypeString{std::string(data->getTypeNameString())};
        std::string varStorageNameString{"source.storageOf_" + classNameString + "_" + varNameString};
        if (varTypeString.substr(0,7) == "static ") {
            varTypeString = varTypeString.substr(7,varTypeString.size()-1);
            std::string sg_string = varTypeString;
            switch (evaluateType(varTypeString)) {
     ...
     for (auto stringListIterator = copyList.begin(); stringListIterator != copyList.end(); stringListIterator++)
        {
          ...
          if (varTypeString.substr(0,7) == "static ")
             {
               std::string varStorageNameString{"storageOf_" + classNameString + "_" + varNameString};
               varTypeString = varTypeString.substr(7,varTypeString.size()-1);
               if (varNameString != "freepointer")
                  {
                    switch (evaluateType(varTypeString))
...
std::string
AstNodeClass::buildStaticDataReadEasyStorageDataFromFileSource()
   {
     std::string s;
     std::string classNameString{this->name};
     std::vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};
...
*/
std::string
AstNodeClass::buildStaticDataArrangeEasyStorageInOnePoolSource()
{
     std::string s;
     std::string classNameString{this->name};
     std::vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};
...
*/
std::string
AstNodeClass::buildStaticDataDeleteEasyStorageMemoryPoolSource() {
     std::string s;
     std::string classNameString{this->name};
     std::vector<GrammarString*> copyList{this->getMemberDataPrototypeList(AstNodeClass::LOCAL_LIST,AstNodeClass::INCLUDE_LIST)};
