...
Sawyer::CommandLine::SwitchGroup
EngineBinary::disassemblerSwitches() {
...
Sawyer::CommandLine::SwitchGroup
EngineBinary::partitionerSwitches() {
...
Sawyer::CommandLine::SwitchGroup
EngineBinary::astConstructionSwitches() {
...
EngineBinary::commandLineParser(const std::string &purpose, const std::string &description) {
    using namespace Sawyer::CommandLine;
    Parser parser =
...
EngineBinary::parseCommandLine(int argc, char *argv[], const std::string &purpose, const std::string &description) {
    try {
...
Sawyer::CommandLine::ParserResult
EngineBinary::parseCommandLine(const std::vector<std::string> &args, const std::string &purpose, const std::string &description) {
    return commandLineParser(purpose, description).parse(args).apply();
}

void
EngineBinary::checkSettings() {
    if (!disassembler() && !settings().disassembler.isaName.empty())
        disassembler(Disassembler::lookup(settings().disassembler.isaName))
}
...
EngineBinary::obtainDisassembler() {
    return obtainDisassembler(Disassembler::Base::Ptr());
}

Disassembler::Base::Ptr
EngineBinary::obtainDisassembler(const Disassembler::Base::Ptr &hint) {
    if (!disassembler() && !settings().disassembler.isaName.empty())
        disassembler(Disassembler::lookup(settings().disassembler.isaName));

    if (!disassembler() && interpretation())
        disassembler(Disassembler::lookup(interpretation()));

    if (!disassembler() && hint)
        disassembler(hint);

    if (!disassembler() && settings().disassembler.doDisassemble)
...
#ifdef NOT_MOVED_TO_ENGINE
void
EngineBinary::checkCreatePartitionerPrerequisites() const {
    if (nullptr == disassembler() && settings().disassembler.doDisassemble)
        throw std::runtime_error("EngineBinary::createBarePartitioner needs a prior disassembler");
    if (!memoryMap() || memoryMap()->isEmpty())
        mlog[WARN] <<"EngineBinary::createBarePartitioner: using an empty memory map\n";
}
#endif

#ifdef NOT_MOVED_TO_ENGINE
Partitioner::Ptr
EngineBinary::createBarePartitioner() {
...
#ifdef NOT_MOVED_TO_ENGINE
void
EngineBinary::runPartitioner(const Partitioner::Ptr &partitioner) {
    ASSERT_not_null(partitioner);
...
#ifdef NOT_MOVED_TO_ENGINE
Partitioner::Ptr
EngineBinary::partition(const std::string &fileName) {
    return partition(std::vector<std::string>(1, fileName));
}

void
EngineBinary::savePartitioner(const Partitioner::ConstPtr &partitioner, const boost::filesystem::path &name,
                        SerialIo::Format fmt) {
    Sawyer::Message::Stream info(mlog[INFO]);
...
Partitioner::Ptr
EngineBinary::loadPartitioner(const boost::filesystem::path &name, SerialIo::Format fmt) {
    Sawyer::Message::Stream info(mlog[INFO]);
...
#ifdef NOT_MOVED_TO_ENGINE
void
EngineBinary::labelAddresses(const Partitioner::Ptr &partitioner, const Configuration &configuration) {
    ASSERT_not_null(partitioner);
...
EngineBinary::makeConfiguredDataBlocks(const Partitioner::Ptr &partitioner, const Configuration &configuration) {
...
EngineBinary::makeConfiguredFunctions(const Partitioner::Ptr &partitioner, const Configuration &configuration) {
    ASSERT_not_null(partitioner);
...
#ifdef NOT_MOVED_TO_ENGINE
void
EngineBinary::updateAnalysisResults(const Partitioner::Ptr &partitioner) {
    ASSERT_not_null(partitioner);
    Sawyer::Message::Stream info(mlog[INFO]);
    Sawyer::Stopwatch timer;
...
#ifdef NOT_MOVED_TO_ENGINE
Disassembler::Base::Ptr
EngineBinary::disassembler() const {
    return disassembler_;
}

void
EngineBinary::disassembler(const Disassembler::Base::Ptr &d) {
    disassembler_ = d;
}

Progress::Ptr
EngineBinary::progress() const {
    return progress_;
}

void
EngineBinary::progress(const Progress::Ptr &progress) {
    progress_ = progress;
}
#endif
