...
ExecutionEvent::instanceMapMemory(const TestCase::Ptr &tc, const Location &location, rose_addr_t ip,
    ...
    Ptr retval = instance(tc, location, ip);
...
ExecutionEvent::Ptr
ExecutionEvent::instanceWriteRegister(const TestCase::Ptr &tc, const Location &location, rose_addr_t ip,
...
ExecutionEvent::Ptr
ExecutionEvent::instanceSyscall(const TestCase::Ptr &tc, const Location &location, rose_addr_t ip,
...
const std::string&
ExecutionEvent::comment() const {
    return comment_;
}

void
ExecutionEvent::comment(const std::string &s) {
    comment_ = s;
}
...
ExecutionEvent::toYaml(std::ostream &out, const Database::Ptr &db, std::string prefix) {
    ...
    out <<prefix <<"comment:    " <<StringUtility::yamlEscape(comment()) <<"\n";
