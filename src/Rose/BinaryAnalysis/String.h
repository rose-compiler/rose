...
/** Support for finding strings in memory.
 *
 *  This namespace provides support for various kinds of strings in specimen memory, including an @ref StringFinder
 * "analysis" that searches for strings in specimen memory.  A string is a sequence of characters encoded in one of a
 * variety of ways in memory.  For instance, NUL-terminated ASCII is a common encoding from C compilers.  The characters
 * within the string must all satisfy some valid-character predicate.  The terms used in this analysis are based on the
 * Unicode standard, and are defined here in terms of string encoding (translation of a string as printed to a sequence
 * of octets). Although this analysis can encode strings, its main purpose is decoding strings from an octet stream into
 * a sequence of code points.
 *
 *  Unicode and its parallel standard, the ISO/IEC 10646 Universal Character Set, together constitute a modern, unified
 *  character encoding. Rather than mapping characters directly to octets (bytes), they separately define what
 * characters are available, their numbering, how those numbers are encoded as a series of "code units" (limited-size
 * numbers), and finally how those units are encoded as a stream of octets. The idea behind this decomposition is to
 * establish a universal set of characters that can be encoded in a variety of ways. To describe this model correctly
 * one needs more precise terms than "character set" and "character encoding." The terms used in the modern model
 * follow:
 *
 *  A character repertoire is the full set of abstract characters that a system supports. The repertoire may be closed,
 * i.e. no additions are allowed without creating a new standard (as is the case with ASCII and most of the ISO-8859
 * series), or it may be open, allowing additions (as is the case with Unicode and to a limited extent the Windows code
 * pages). The characters in a given repertoire reflect decisions that have been made about how to divide writing
 * systems into basic information units. The basic variants of the Latin, Greek and Cyrillic alphabets can be broken
 * down into letters, digits, punctuation, and a few special characters such as the space, which can all be arranged in
 * simple linear sequences that are displayed in the same order they are read. Even with these alphabets, however,
 * diacritics pose a complication: they can be regarded either as part of a single character containing a letter and
 * diacritic (known as a precomposed character), or as separate characters. The former allows a far simpler text
 * handling system but the latter allows any letter/diacritic combination to be used in text. Ligatures pose similar
 * problems. Other writing systems, such as Arabic and Hebrew, are represented with more complex character repertoires
 * due to the need to accommodate things like bidirectional text and glyphs that are joined together in different ways
 * for different situations.
 *
 *  A coded character set (CCS) specifies how to represent a repertoire of characters using a number of (typically
 *  non-negative) integer values called code points. For example, in a given repertoire, a character representing the
 * capital letter "A" in the Latin alphabet might be assigned to the integer 65, the character for "B" to 66, and so on.
 * A complete set of characters and corresponding integers is a coded character set. Multiple coded character sets may
 * share the same repertoire; for example ISO/IEC 8859-1 and IBM code pages 037 and 500 all cover the same repertoire
 * but map them to different codes. In a coded character set, each code point only represents one character, i.e., a
 * coded character set is a function.
 *
 *  A character encoding form (CEF) specifies the conversion of a coded character set's integer codes into a set of
 *  limited-size integer code values that facilitate storage in a system that represents numbers in binary form using a
 * fixed number of bits (i.e. practically any computer system). For example, a system that stores numeric information in
 * 16-bit units would only be able to directly represent integers from 0 to 65,535 in each unit, but larger integers
 * could be represented if more than one 16-bit unit could be used. This is what a CEF accommodates: it defines a way of
 * mapping a single code point from a range of, say, 0 to 1.4 million, to a series of one or more code values from a
 * range of, say, 0 to 65,535.
 *
 *  The simplest CEF system is simply to choose large enough units that the values from the coded character set can be
 * encoded directly (one code point to one code value). This works well for coded character sets that fit in 8 bits (as
 * most legacy non-CJK encodings do) and reasonably well for coded character sets that fit in 16 bits (such as early
 * versions of Unicode). However, as the size of the coded character set increases (e.g. modern Unicode requires at
 * least 21 bits/character), this becomes less and less efficient, and it is difficult to adapt existing systems to use
 * larger code values. Therefore, most systems working with later versions of Unicode use either UTF-8, which maps
 * Unicode code points to variable-length sequences of octets, or UTF-16, which maps Unicode code points to
 * variable-length sequences of 16-bit words.
 *
 *  Next, a character encoding scheme (CES) specifies how the fixed-size integer code values should be mapped into an
 * octet sequence suitable for saving on an octet-based file system or transmitting over an octet-based network. With
 * Unicode, a simple character encoding scheme is used in most cases, simply specifying whether the bytes for each
 * integer should be in big-endian or little-endian order (even this isn't needed with UTF-8). However, there are also
 * compound character encoding schemes, which use escape sequences to switch between several simple schemes (such as
 * ISO/IEC 2022), and compressing schemes, which try to minimise the number of bytes used per code unit (such as SCSU,
 * BOCU, and Punycode).
 *
 *  Once the code points of a string are encoded as octets, the string as a whole needs some description to demarcate it
 * from surrounding data. ROSE currently supports two styles of demarcation: length-encoded strings and terminated
 * strings.  A length-encoded string's code point octets are preceded by octets that encode the string length, usually
 * in terms of the number of code points. Decoding such a string consists of decoding the length and then decoding code
 * points until the required number of code points have been obtained.  On the other hand, terminated strings are
 * demarcated from surrounding data by a special code point such as the NUL character for ASCII strings. Decoding a
 * terminated string consists of decoding code points until a terminator is found, then discarding the terminator.
 *
 * @section ex1 Example 1
 *
 *  This example shows how to find all strings in memory that is readable but not writable using a list of common
 * encodings such as C-style NUL-terminated printable ASCII, zero terminated UTF-16 little-endian, 2-byte little-endian
 * length encoded ASCII, etc.
 *
 * @code
 ...
 *
 *  The @ref StringFinder analysis is tuned for searching for strings at unknown locations while trying to decode
 * multiple encodings simultaneously. If all you want to do is read a single string from a known location having a known
 * encoding then you're probabily better off reading it directly from the @ref MemoryMap. The @ref StringFinder analysis
 * can be used for that, but it's probably overkill. In any case, here's the overkill version to find a 2-byte little
 * endian length-encoded UTF-8 string:
 *
 * @code
 ...
 *
 *  The encoders can also be used to decode directly from a stream of octets. For instance, lets say you have a vector
 * of octets that map 1:1 to code values, and then you want to decode the code values as a UTF-8 stream to get some code
 *  points. All decoders are implemented as state machines to make it efficient to send the same octets to many decoders
 *  without having to rescan/reread from a memory map.  The UTF-8 decoder decodes one octet at a time and when it enters
 * the FINAL_STATE or COMPLETED_STATE then a decoded code value can be consumed.
 *
 * @code
 *  #include <Rose/BinaryAnalysis/String.h>
 *  using namespace Rose::BinaryAnalysis::Strings;
...
class ROSE_DLL_API StringFilter {
  ...
  public:
     ...
     * @return true if string matches any filter, false otherwise */
 ...
 *  distinct Unicode characters.  The CharacterEncodingForm (CEF) is responsible for converting a code point to a
 * sequence of one or more code values, or vice versa.  Each code value, which may be multiple bytes, is eventually
 * encoded into a sequence of octets by the @ref CharacterEncodingScheme (CES). */
class ROSE_DLL_API CharacterEncodingForm: public Sawyer::SharedObject {
  ...
  public:
     ...
     *
     *  For instance, an ecoder for UTF-16 will encode a code point into one or more values in the range 0 through
     * (2^16)-1. */
     ...
     *
     *  Processes a single code value and updates the decoder state machine. Returns the decoder's new state. See
     * documentation for @ref Strings::State for restrictions on state transitions. */
 ...
 *
 *  A code value (one or more of which compose a code point, or a single character in a coded character set), is encoded
 * as one or more octets.  For instance, a UTF-16 code value will be converted to two octets in big or little endian
 * order depending on the character encoding scheme. */
class ROSE_DLL_API CharacterEncodingScheme: public Sawyer::SharedObject {
  ...
  public:
    ...
    /** Decode one octet.
     *
     *  Processes a single octet and updates the decoder state machine. Returns the decoder's new state. See
     * documentation for
 ...
 *  versa. It needs to know the number of octets per code value, and the byte order of the octets per code value is
 * larger than one. */
class ROSE_DLL_API BasicCharacterEncodingScheme: public CharacterEncodingScheme {
  ...
  public:
    ...
    virtual Ptr         clone() const override { return Ptr(new BasicCharacterEncodingScheme(*this)); }
...
basicCharacterEncodingScheme(size_t octetsPerValue, ByteOrder::Endianness sex = ByteOrder::ORDER_UNSPECIFIED);

/** Encoding for the length of a string.
 *
 *  Strings that are length-encoded must specify a length encoding scheme that gives the length of the string measured
 * in code points. */
...
class ROSE_DLL_API BasicLengthEncodingScheme: public LengthEncodingScheme {
  ...
  public:
    ...
    virtual Ptr         clone() const override { return Ptr(new BasicLengthEncodingScheme(*this)); }
    virtual std::string name() const override;
...
basicLengthEncodingScheme(size_t octetsPerValue, ByteOrder::Endianness sex = ByteOrder::ORDER_UNSPECIFIED);
 ...
 *
 *  Returns true if the code point is a printable US-ASCII character.  Printable characters are seven-bit code points
 * for which C's @c isprint predicate returns true (anything but control characters). */
class ROSE_DLL_API PrintableAscii: public CodePointPredicate {
  protected:
    PrintableAscii() {}

  public:
    static Ptr          instance() { return Ptr(new PrintableAscii); }
 ...
 *
 *  A string encoding scheme indicates how a string (sequence of code points) is encoded as a sequence of octets and
 * vice versa. */
class ROSE_DLL_API StringEncodingScheme: public Sawyer::SharedObject {
  ...
  protected:
    ...
    StringEncodingScheme(
      const CharacterEncodingForm::Ptr& cef, const CharacterEncodingScheme::Ptr& ces, const CodePointPredicate::Ptr& cpp
  ...
  public:
    ...
    /** Decode one octet.
     *
     *  Processes a single octet and updates the decoder state machine.  Returns the new state. See documentation for
     * @ref Strings::State for restrictions on state transitions. */
    virtual State decode(Octet) = 0;

    /** Consume pending decoded code points.
     *
     *  Returns code points that haven't been consume yet, and then removes them from the decoder.  This can be called
     * from any state because we want the caller to be able to consume code points as they're decoded, which is a little
     * bit different than how @c consume methods operate in the decoders that return scalar values. A @ref reset will
     * discard pending code points. */
    CodePoints consume();
     ...
     *
     *  The character encoding format is responsible for converting each code point to a sequence of code values. For
     * instance, a UTF-16 encoding will convert each code point (a number between zero and about 1.2 million) into a
     * sequence of 16-bit code values.  Each code value will eventually be converted to a pair of octets by the
     ...
     *
     *  The character encoding scheme is responsible for converting each code value to a sequence of one or more octets.
     * The code value is part of a sequence of code values generated by the character encoding format for a single code
     * point. For instance, a character encoding scheme for UTF-16 will need to know whether the octets are stored in
     * bit- or little-endian order.
     *
     * @{ */
     ...
     *  decoding NUL-terminated ASCII strings one might want to consider only those strings that contain printable
     * characters and white space in order to limit the number of false positives when searching for strings in memory.
     *
     * @{ */
...
class ROSE_DLL_API LengthEncodedString: public StringEncodingScheme {
  ...
  protected:
    LengthEncodedString(
      const LengthEncodingScheme::Ptr& les, const CharacterEncodingForm::Ptr& cef,
  ...
  public:
     ...
     *
     *  The declared length is the value of the decoded length prefix, not necessarily the number of code points that
     * have been decoded.  This can be called from any state except it will always return nothing in the INITIAL_STATE.
     * Therefore, this method should be called prior to the @ref consume call. */
...
LengthEncodedString::Ptr lengthEncodedString(
  const LengthEncodingScheme::Ptr& les, const CharacterEncodingForm::Ptr& cef, const CharacterEncodingScheme::Ptr& ces,
  const CodePointPredicate::Ptr& cpp
);

/** Returns a new encoder for length-encoded printable ASCII strings. A byte order must be specified for length
 * encodings larger than a single byte. */
LengthEncodedString::Ptr
lengthEncodedPrintableAscii(size_t lengthSize, ByteOrder::Endianness order = ByteOrder::ORDER_UNSPECIFIED);
 ...
 *
 *  A string whose character octets are followed by octets for a special code point that marks the end of the string but
 * is not included as part of the string's characters.  An example is C-style NUL-terminated ASCII. */
class ROSE_DLL_API TerminatedString: public StringEncodingScheme {
  ...
  protected:
    TerminatedString(
      const CharacterEncodingForm::Ptr& cef, const CharacterEncodingScheme::Ptr& ces,
  ...
  public:
     ...
     *
     *  This can be called from any state except it will always return nothing in the INITIAL_STATE. Therefore, this
     ...
     *
     *  A list of code points (characters) that cause a string to be terminated.  When decoding a string, if a
     * terminating code point is encountered then the string ends at the previous code point even if the terminating
     * code point also satisfies the code point predicate.
     *
     * @{ */
...
class ROSE_DLL_API EncodedString {
  ...
  public:
     ...
     *
     *  A string need not store its code points, in which case this method can decode them from memory.  The memory
     * should be the same as when the string was originally found, otherwise an std::runtime_error might be thrown. */
    void decode(const MemoryMap&);
};

/** %Analysis to find encoded strings.
 *
 *  This analysis searches user-specified parts of a binary specimen's memory space to find strings encoded in various
 * formats specfieid by the user.
 *
...
class ROSE_DLL_API StringFinder {
  public:
    ...
    struct Settings {
         ...
         *
         *  Strings having fewer than this many code points are discarded. If @ref minLength is larger than @ref
         * maxLength then no strings will be matched. */
        size_t minLength;

        /** Maximum length of matched strings.
         *
         *  Strings having more than this many code points are discarded. If @ref maxLength is smaller than @ref
         * minLength then no strings will be matched. */
        size_t maxLength;

        /** Whether to allow overlapping strings.
         *
         *  The number of strings that can overlap at a single address per encoder.  For instance, for C-style
         * NUL-terminated ASCII strings encoded as bytes, if memory contains the consecutive values 'a', 'n', 'i', 'm',
         * 'a', 'l', '\0' then up to seven strings are possible: "animal", "nimal", "imal", "mal", "al", "l", and "". If
         * the maximum overlap is set to three then only "animal", "nimal", and "imal" are found. Setting the maximum
         * overlap to zero has the same effect as setting it to one: no overlapping is allowed.  The overlap limits are
         * applied before results are pruned based on length, so if the minimum legnth is four, the "imal" and shorter
         * strings won't be found even though they are decoded under the covers.
         *
         *  A maximum overlap of at least two is recommended if two-byte-per-character encoding is used when detecting
         *  NUL-terminated ASCII strings. The reason is that one decoder will be active at one address while another
         * decoder is desired for the next address; then if the first address proves to not be part of a string, the
         * second address can still be detected as a string.  Similarly, a maximum overlap of at least four is
         * recommended for four-byte-per-character encodings.  Length-encoded strings will have similar issues. */
        size_t maxOverlap;

        /** Whether to keep only longest non-overlapping strings.
         *
         *  If set, then only the longest detected strings are kept.  The algorithm sorts all detected strings by
         * decreasing length, then removes any string whose memory addresses overlap with any prior string in the list.
         */
  ...
  public:
    /** Constructor.
     *
     *  Initializes the analysis with default settings but no encoders. Encoders will need to be added before this
     * analysis can be used to find any strings. */
     ...
     *
     *  If this property is set, then the process of decoding strings does not actually store the code points
     * (characters) of the string.  This is useful when searching for lots of strings to reduce the amount of memory
     * required. A string can be decoded again later if the code points are needed.
     *
     * @{ */
    bool          discardingCodePoints() const { return discardingCodePoints_; }
    StringFinder& discardingCodePoints(bool b) {
        discardingCodePoints_ = b;
        return *this;
    }
    /** @} */
     ...
     *
     *  When searching for strings, this analysis must know what kinds of strings to look for, and does that with a
     * vector of pointers to encoders.  The default is an empty vector, in which no strings will be found.
     *
     * @{ */
     ...
     *
     *  Returns the switch group that describes the command-line switches for this analysis. The caller can provide a
     * @ref Settings object that will be adjusted when the command-line is parsed and applied; if no argument is
     * supplied then the settings of this analysis are affected. In either case, the settings or analysis object must
     * still be allocated when the command-line is parsed.
     *
     * @{ */
     ...
     *
     *  Clears previous analysis results (e.g., @ref reset) and then searches for new strings.  The resulting strings
     * can be obtained from the @ref strings method.
     *
     *  The memory constraints indicate where to search for strings, and the properties of this StringFinder class
     * determine how to find strings. Specifically, this class must have at least one encoding registered in order to
     * find anything (see
     *  @ref encoders).
     *
     *  The search progresses by looking at each possible starting address using each registered encoding. The algorithm
     * reads each byte from memory only one time, simultaneously attempting all encoders.  If the MemoryMap constraint
     * contains an anchor point (e.g., @ref MemoryMap::at) then only strings starting at the specified address are
     * returned.
     *
     *  Example 1: Find all C-style, NUL-terminated, ASCII strings contaiing only printable characters (no control
     * characters) and containing at least five characters but not more than 31 (not counting the NUL terminator).  Make
     * sure that the string is in memory that is readable but not writable, and don't allow strings to overlap one
     * another (i.e., "foobar" and "bar" cannot share their last for bytes):
     *
     * @code
     ...
     * sf.find(map.require(MemoryMap::READABLE).prohibit(MemoryMap::WRITABLE)).strings();
     * @endcode */
