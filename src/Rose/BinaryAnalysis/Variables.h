 ...
 *
 *  @li The compiler can elide a source-level variable if it determines that it's not necessary in the binary. I.e., the compiler
 ...
 *
 *  @li The compiler can give the same address to two or more source-level variables if one or more of them is read without being
 ...
 *
 *  @li The compiler might not allocate stack storage for a local variable. E.g., if enough general purpose registers are
 *  available the compiler might use a register to store the local variable.
 *
 *  @li The scope of the local variable in the source language does not generally match the lifetime of the storage location
 ...
 *
 *  @li Variables that the compiler might initially store in registers (such as function arguments) are often spilled to the
 *  stack in order to make those registers available for other purposes within the function.
 *
 *  @li A source-level variable might exist at more than one location in the binary. A common situation is when the compiler
 ...
 *  to the stack.
 *
 *  @li Writes to a stack location that are not followed by reads can be elided, in which case the analysis would indicate that
 *  there was no subsequent write.
 *
 *  @li Many other caveats. */
namespace Variables {}
