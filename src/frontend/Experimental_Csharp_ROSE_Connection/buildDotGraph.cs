...
public class BuildDotGraph : CSharpSyntaxWalker
  ...
  // List of nodes for which we have output nodes in the dot graph. 
  // Used to identify children in the AST graph.
  ...
  // ~BuildDotGraph() => Console.WriteLine($"calling destructor");
     ...
     public override void Visit(SyntaxNode node)
       ...
       // Parent = node;
          ...
          labelMap.TryGetValue(node, out label);
          if (Parent != null)
             {
               labelMap.TryGetValue(Parent, out parentLabel);
             }
       ...
       // parent node and the current node (child node).
          ...
          if (Parent != null)
            ...
            // Console.WriteLine("default edge label = " + edgeLabelName);
               ...
               edgeLabelName = getEdgeLabels(Parent,node);
               nodeLabelName = getNodeLabels(Parent);
       ...
       // Need to output the child nodes since the mechanism above only addresses parents.
          using (file = new System.IO.StreamWriter("csharpAST.dot",true) )
             {
            // using file scope.
               if (processedNodes.Contains(node) == false)
                  {
                    string nodeLabelName = getNodeLabels(node);

                 // Console.WriteLine(indents + "Output child node: node = " + node.Kind() + " label = " + label);
  ...
  // public override void getNodeLabels(SyntaxToken node, string nodeLabel)
  // public string getNodeLabels(SyntaxToken node)
     public string getNodeLabels(SyntaxNode node)
        {
          ...
          switch (node.Kind())
            ...
            // names & type-names
               case SyntaxKind.IdentifierName: // = 8616,
                    ...
                    IdentifierNameSyntax identifierNameNode = node as IdentifierNameSyntax;

                 // nodeLabelName += "\n name = " + node.Kind();
                    nodeLabelName += "\n name = " + identifierNameNode.Identifier;
                    break;
            ...
            // primary expression
               ...
               case SyntaxKind.StringLiteralExpression: // = 8750,
                 // StringLiteralExpressionSyntax stringLiteralExpressionNameNode = node as LiteralExpressionSyntax;
                    LiteralExpressionSyntax literalExpressionNameNode = node as LiteralExpressionSyntax;

                 // Console.WriteLine("   --- literalExpressionNameNode.Token = " + literalExpressionNameNode.Token);

                 // Note that we need to remove the quotes from the token.
                    string temp = "" + literalExpressionNameNode.Token;
            ...
            // type declarations
               case SyntaxKind.ClassDeclaration: // = 8855,
                 // Console.WriteLine("case SyntaxKind.ClassDeclaration: node.Kind = " + node.Kind());

                    ClassDeclarationSyntax classDeclarationNode = node as ClassDeclarationSyntax;

                    nodeLabelName += "\n class = " + classDeclarationNode.Identifier;
