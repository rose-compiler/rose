...
template <class BASIC_TYPE >
class EasyStorage <std::unordered_set<BASIC_TYPE> >
   : public StorageClassMemoryManagement<BASIC_TYPE>
   {
     typedef StorageClassMemoryManagement<BASIC_TYPE> Base;
    public:
     EasyStorage() {}
     void storeDataInEasyStorageClass(const std::unordered_set<BASIC_TYPE>& data_);
     std::unordered_set<BASIC_TYPE> rebuildDataStoredInEasyStorageClass() const;
   };
...
// PP (7/18/2025): Added to support mapping of SgNode* to strings for name mangling support
template <>
class EasyStorage < std::unordered_map<SgNode*, std::string> >
   : public StorageClassMemoryManagement< EasyStorageMapEntry<SgNode*, std::string> >
   {
     typedef StorageClassMemoryManagement< EasyStorageMapEntry<SgNode*, std::string> > Base;
    public:
     void storeDataInEasyStorageClass(const std::unordered_map<SgNode*,std::string>& data_);
     std::unordered_map<SgNode*,std::string> rebuildDataStoredInEasyStorageClass() const;
     static void arrangeMemoryPoolInOneBlock();
     static void deleteMemoryPool();
