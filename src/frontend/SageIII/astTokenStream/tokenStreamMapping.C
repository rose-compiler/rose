  ...
  // subsequences to IR nodes in the AST.
     ...
     public:
       ...
       // static map<size_t,TokenStreamSequenceToNodeMapping*,TokenStreamSequenceToNodeMapping_key> tokenSequencePool;
          static map<TokenStreamSequenceToNodeMapping_key,TokenStreamSequenceToNodeMapping*> tokenSequencePool;
                      ...
                      // Handle the right edge of the AST subtree: modify the edged (trailing whitespace).
                         if (i == (tokenToNodeVector.size()-1))
                           ...
                           // printf ("In evaluateSynthesizedAttribute(): tokenToNodeVector.size() = %zu tokenStream.size() = %zu \n",tokenToNodeVector.size(),tokenStream.size());
                              ROSE_ASSERT(mappingInfo->trailing_whitespace_end == -1 || mappingInfo->trailing_whitespace_end < tokenStream.size());
                            }
                           else
                            {
                              if (mappingInfo->shared == false)
                                ...
                                // printf ("In evaluateSynthesizedAttribute(): tokenToNodeVector.size() = %zu tokenStream.size() = %zu \n",tokenToNodeVector.size(),tokenStream.size());
                                   ROSE_ASSERT(next_mappingInfo->token_subsequence_start == -1 || next_mappingInfo->token_subsequence_start < tokenStream.size());
...
#if 0
                    printf ("starting_line = %d ending_line = %d process_node = %s \n",starting_line,ending_line,process_node ? "true" : "false");
#endif
                    if (process_node == true)
                       {
#if 0
...
#if 1
     printf ("In buildTokenStreamMapping(): filename = %s \n",filename.c_str());
#endif
...
main ( int argc, char* argv[] )
  ...
  // AST consistency tests (optional for users, but this enforces more of our tests)
     ...
     SgSourceFile* sourceFile = isSgSourceFile(&localFile);
