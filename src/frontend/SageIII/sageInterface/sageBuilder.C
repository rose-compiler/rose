...
SageBuilder::buildDefaultConstructor (SgClassType* classType)
  ...
  // Constructors are specified with type void internally, though the type name is not output.
     ...
     SgExprListExp* decoratorList            = nullptr;
          ...
          switch (templateArgument->get_argumentType())
             {
               ...
               case SgTemplateArgument::nontype_argument:
                 ...
                 // DQ (8/12/2013): This can be either an SgExpression or SgInitializedName.
                    ...
                    ROSE_ASSERT (templateArgument->get_expression() == NULL || templateArgument->get_initializedName() == NULL);
                    if (templateArgument->get_expression() != nullptr)
                       {
...
SageBuilder::getTemplateArgumentList( SgDeclarationStatement* decl )
  ...
  // In a template instantiation it is the templateArguments field and from template declarations it is the templateSpecializationArguments field.
     ...
     SgTemplateArgumentPtrList* templateArgumentsList = nullptr;
     ...
     switch(decl->variantT())
        {
          ...
          case V_SgTemplateVariableInstantiation:
             {
               templateArgumentsList = &(isSgTemplateVariableInstantiation(decl)->get_templateArguments());
               break;
             }
...
SageBuilder::getTemplateParameterList( SgDeclarationStatement* decl )
  ...
  // For all other cases it returns NULL (or is an error).
     ...
     SgTemplateParameterPtrList* templateParameterList = nullptr;
...
SageBuilder::setTemplateArgumentsInDeclaration( SgDeclarationStatement* decl, SgTemplateArgumentPtrList* templateArgumentsList_input )
  ...
  // DQ (9/16/2012): Setup the template arguments for any type of template instantiation.
     ...
     if (templateArgumentsList_from_declaration != nullptr)
        {
...
SageBuilder::setTemplateSpecializationArgumentsInDeclaration( SgDeclarationStatement* decl, SgTemplateArgumentPtrList* templateSpecializationArgumentsList_input )
  ...
  // DQ (9/16/2012): Setup the template arguments for any type of template declaration
     ...
     if (templateSpecializationArgumentsList_from_declaration != nullptr)
        {
...
#define DEBUG__buildInitializedName 0
...
SgInitializedName* SageBuilder::buildInitializedName(const SgName& name, SgType* type, SgInitializer* init/*= NULL*/) {
  ...
  std::cout << "SageBuilder::buildInitializedName" << std::endl;
  ...
  if (scope != nullptr)
  {
    ...
    if (SageInterface::is_Fortran_language())
    {
     ...
     // DQ (5/21/2013): Removed direct reference to symbol table (namespace handling is only supported at the SgScopeStatement level).
        ...
        if (v_symbol != nullptr) // find a function parameter with the same name
        {
...
#define DEBUG_BUILD_VARIABLE_DECLARATION 0
  ...
  // If there was a previous use of the variable, then there will be an existing symbol with it's declaration pointing to the SgInitializedName object.
     ...
     if (variableSymbol == nullptr)
       ...
       // then it should not be attached to the class definition as a variable declaration yet, and we should reuse it.
          if (associatedVariableDeclaration != nullptr && associatedVariableDeclaration->get_parent() != nullptr)
             {
          ...
          if (associatedVariableDeclaration != nullptr)
             {
       ...
       // DQ (6/25/2019): Trigger the reuse of the available variable declaration.
          ...
          if (reuseTheAssociatedVariableDeclaration == true)
...
#if DEBUG_BUILD_VARIABLE_DECLARATION
               if (variable->get_initptr() != nullptr)
                  {
            ...
            // Note: we need another one because either one can have an initializer that cannot be shared in the AST.
               SgInitializedName* additional_variable = nullptr;
               ...
               if (scopeStatement != nullptr)
                  {
                    ...
                    if (additional_variable->get_initptr() != nullptr)
                       {
#if DEBUG_BUILD_VARIABLE_DECLARATION
  ...
  // DQ (7/9/2012): Added test (parent should not be set yet; set in parse_statement).
     ...
     if (name != "")
       ...
       // Yes, let's set it to the current top of the scope stack.  This might be a problem if the scope stack is not being used...
          ...
          ASSERT_not_null(varDecl->get_parent());
        }

     SgInitializedName *initName = varDecl->get_decl_item (name);
     ASSERT_not_null(initName);
     ASSERT_not_null(initName->get_declptr());

     if (initName->get_scope())
        {
  ...
  // have to set SgVariableDefintion explicitly
     ...
     ASSERT_not_null(variableDefinition_original);
...
// refactored from ROSETTA/Grammar/Statement.code SgVariableDeclaration::append_variable ()
  ...
  SgVariableDefinition* defn_stmt = nullptr;
  if (!isSgFunctionType(init_name->get_type()))
  {
    Sg_File_Info* copyOfFileInfo = nullptr;
    if (decl->get_file_info() != nullptr)
    {
      ...
      if (decl->get_endOfConstruct() != nullptr)
      {
...
SageBuilder::buildTemplateVariableInstantiation (
  ...
  SgTemplateVariableInstantiation* res = buildTemplateVariableInstantiation_nfi(name, type, varInit, scope, tpl_decl, tpl_args);
  setOneSourcePositionForTransformation(res);
  return res;
}

#define DEBUG__SageBuilder__buildTemplateVariableInstantiation_nfi 0

SgTemplateVariableInstantiation*
SageBuilder::buildTemplateVariableInstantiation_nfi(
  const SgName& name, SgType* type, SgInitializer* varInit, SgScopeStatement* scope,
  SgTemplateVariableDeclaration* /*tpl_decl*/, SgTemplateArgumentPtrList& tpl_args) {
#if DEBUG__SageBuilder__buildTemplateVariableInstantiation_nfi
     std::cout << "SageBuilder::buildTemplateVariableInstantiation_nfi" << std::endl;
     std::cout << "  name = " << name.getString() << std::endl;
     std::cout << "  type = " << std::hex << type << " : " << ( type ? type->class_name() : "" ) << std::endl;
     std::cout << "  scope = " << std::hex << scope << " : " << ( scope ? scope->class_name() : "" ) << std::endl;
#endif
     ASSERT_not_null(type);

     SgName nameWithoutTemplateArguments = name;
     SgName nameWithTemplateArguments = appendTemplateArgumentsToName(name,tpl_args);
#if DEBUG__SageBuilder__buildTemplateVariableInstantiation_nfi
     ...
     SgTemplateVariableInstantiation * varDecl = new SgTemplateVariableInstantiation(name, type, varInit);
     ROSE_ASSERT(varDecl);
#if DEBUG__SageBuilder__buildTemplateVariableInstantiation_nfi
     std::cout << "  varDecl = " << std::hex << varDecl << " : " << ( varDecl ? varDecl->class_name() : "" ) << std::endl;
#endif

     varDecl->set_firstNondefiningDeclaration(varDecl);
     varDecl->get_templateArguments() = tpl_args;
     ...
     ASSERT_not_null((initName->get_declptr()));
#if DEBUG__SageBuilder__buildTemplateVariableInstantiation_nfi
     std::cout << "  initName = " << std::hex << initName << " : " << ( initName ? initName->class_name() : "" ) << std::endl;
#endif
...
SageBuilder::buildTemplateInstantiationTypedefDeclaration_nfi(
  ...
  // parent rather then the scope. But as I recall there was a special corner of C++ that
  // required this sort of support.
     SgSymbol* parent_scope = nullptr;
     if (scope != nullptr)
        {
          ASSERT_not_null(scope->get_parent());
          SgDeclarationStatement* declaration = isSgDeclarationStatement(scope->get_parent());
          if (declaration != nullptr)
             {
     ...
     if (prexisting_template_typedef_symbol != nullptr)
        {
     ...
     ASSERT_require(scope->lookup_template_typedef_symbol(nameWithTemplateArguments) == nullptr);

     SgTypedefType* typedefType = nullptr;
     ...
     ASSERT_require(type_decl->get_base_type() == base_type);
  ...
  // DQ (2/27/2018): This is a change in the constructor semantics, we now have to build the type explicitly.
     ASSERT_require(type_decl->get_type() == nullptr);
  ...
  // the name generated by type_decl->resetTemplateName()), make sure that we have a consistant naming.
     ASSERT_require(type_decl->get_name() == nameWithTemplateArguments);

     if (scope->lookup_template_typedef_symbol(nameWithTemplateArguments) != nullptr)
        {
          printf ("Error: it appears that there is already a symbol in scope = %p = %s for name = %s \n",scope,scope->class_name().c_str(),name.str());
        }
     ASSERT_require(scope->lookup_template_typedef_symbol(nameWithTemplateArguments) == nullptr);
     ...
     ASSERT_not_null(new_typedefType);

     type_decl->set_type(new_typedefType);
     ASSERT_not_null(type_decl->get_type());
     ASSERT_not_null(type_decl->get_scope());

     type_decl->set_firstNondefiningDeclaration(type_decl);
     type_decl->set_definingDeclaration(nullptr);

     setOneSourcePositionNull(type_decl);
     setTemplateArgumentsInDeclaration(type_decl,&templateArgumentsList);

     ASSERT_not_null(type_decl->get_firstNondefiningDeclaration());
     ASSERT_require(type_decl->get_definingDeclaration() == nullptr);
     ASSERT_require(type_decl->get_firstNondefiningDeclaration() == type_decl);
     ...
     if (scope != nullptr)
        {
          ...
          SgTemplateTypedefSymbol* typedef_symbol = new SgTemplateTypedefSymbol(type_decl);
          ASSERT_not_null(typedef_symbol);

          scope->insert_symbol(nameWithTemplateArguments,typedef_symbol);
          type_decl->set_scope(scope);
          type_decl->set_parent(scope);
          ASSERT_not_null(scope->lookup_template_typedef_symbol(nameWithTemplateArguments));
        }
  ...
  // Maintain the global type table
     ...
     if (typeInTable == nullptr)
       ...
       // Delete the one generated so that we could form the mangled name.
          ASSERT_require(typeInTable != funcType);
          delete funcType;
...
SageBuilder::buildMemberFunctionType(SgType* return_type, SgFunctionParameterTypeList* typeList, SgScopeStatement * struct_name, unsigned int mfunc_specifier)
  ...
  // specific issues with the build interface and it's use with the newer EDG 4.3 connection to ROSE.
     ...
     return buildMemberFunctionType(return_type,typeList,classType,mfunc_specifier);
   }
...
SageBuilder::buildNondefiningFunctionDeclaration_T (
  ...
  // DQ (11/23/2011): This change allows this to compile for where SgTemplateFunctionDeclarations are used.
     ...
     for (SgInitializedName* i_name : paralist->get_args())
        {
  ...
  // Liao 11/21/2012: we should assert no transformation only when the current model is NOT transformation
     ...
     ASSERT_not_null(scope);
     ASSERT_not_null(func->get_scope());
     ASSERT_require(func->get_scope() == scope);

     if (SageBuilder::topScopeStack() != nullptr) // This comparison only makes sense when topScopeStack() returns non-NULL value
        {
     ...
     func->set_parent(scope);
     ASSERT_not_null(func->get_firstNondefiningDeclaration());
...
SageBuilder::buildDefiningFunctionDeclaration_T(const SgName & XXX_name, SgType* return_type, SgFunctionParameterList* paralist, bool isMemberFunction, SgScopeStatement* scope, SgExprListExp* decoratorList, unsigned int functionConstVolatileFlags, actualFunction* first_nondefining_declaration, SgTemplateArgumentPtrList* templateArgumentsList)
  ...
  // named differently (from this one) and depricate this function...which I like much better.
     ...
     ASSERT_not_null(first_nondefining_declaration);
     ASSERT_require(first_nondefining_declaration->get_firstNondefiningDeclaration() == first_nondefining_declaration);
     ...
     ASSERT_require(XXX_name.is_null() == false);
     ASSERT_not_null(scope);
     ASSERT_not_null(return_type);
  ...
  // DQ (8/11/2013): Check that the template argument lists are consistant.  The templateArgumentsList can then be considered redundant if this works.
     if (buildTemplateInstantiation == true)
        {
          ...
          ASSERT_require(templateArgumentListsAreEquivalent == true);
        }
     ...
     if (buildTemplateDeclaration == true)
       ...
       // DQ (8/11/2013): Since this is not passed in so we can access it but not assert its equivalence with a redundant input parameter.
          ...
          ASSERT_require(templateArgumentsList == nullptr);
     ...
     ASSERT_require(nameWithoutTemplateArguments.is_null() == false);
     ASSERT_require(nameWithTemplateArguments.is_null() == false);
     ASSERT_not_null(paralist);

     if (SageInterface::language_may_contain_nondeclarations_in_scope() == false)
        {
           ASSERT_require(scope->containsOnlyDeclarations());
        }

  // build function type, manage function type symbol internally
     actualFunction* defining_func = nullptr;
     SgFunctionType* func_type = nullptr;
  ...
  // Make sure these are the same (this will fail until we generate the func_type directly from first_nondefining_declaration).
     ASSERT_require(func_type == first_nondefining_declaration->get_type());

     SgDeclarationStatement* firstNondefiningFunctionDeclaration = nullptr;
  ...
  // DQ (1/26/2013): This fails for ROSE compiling ROSE.
       ...
       // Cong (10/25/2010): Make sure in this situation there is no defining declaration for this symbol.
          ...
          if (temp_function_sym != nullptr)
            ...
            // There is no type for a template function declaration.
               ASSERT_not_null(temp_template_sym);
     ...
     defining_func = new actualFunction(nameWithTemplateArguments,func_type,nullptr);
     ASSERT_not_null(defining_func);
  ...
  // Build either a definition for a template or non-template function definition.
     if (templateFunctionDeclaration == nullptr)
        {
  ...
  // fixup the scope and symbol of arguments,
     for (SgInitializedName* i_name : paralist->get_args())
        {
          i_name->set_scope(func_def);

          SgVariableSymbol* variableSymbol = new SgVariableSymbol(i_name);
          ASSERT_not_null(variableSymbol);
          func_def->insert_symbol(i_name->get_name(), variableSymbol);

       // For variable length array types in the function parameter list
          SgArrayType* arrayType = isSgArrayType(i_name->get_type());
                 ...
                 // Get the list of SgVarRef IR nodes and process each one as above.
                    ...
                    for (size_t i = 0; i < varRefList.size(); i++)
                      ...
                      // Process each index subtree's SgVarRefExp.
                         ...
                         ASSERT_not_null(dimension_variableSymbol);
                         ASSERT_require(dimension_variableSymbol != variableSymbol);
     ...
     ASSERT_not_null(defining_func->get_scope());
  ...
  // Enforce that the return type matches the specification to build a member function.
     if (isMemberFunction == true)
        {
  ...
  // DQ (8/13/2013): Added code to set the template parameters in the defining declaration (if it is a template declaration).
     if (buildTemplateDeclaration == true)
       ...
       // DQ (8/13/2013): Adding test of template parameter lists.
          SgTemplateFunctionDeclaration* templateFunctionDeclaration = isSgTemplateFunctionDeclaration(defining_func);
          ROSE_ASSERT(templateFunctionDeclaration == nullptr || (templateParameterList != nullptr && templateParameterList->size() == templateFunctionDeclaration->get_templateParameters().size()));
          ...
          ROSE_ASSERT(templateMemberFunctionDeclaration == nullptr || (templateParameterList != nullptr && templateParameterList->size() == templateMemberFunctionDeclaration->get_templateParameters().size()));
        }
...
// Build a nondefining SgProcedureHeaderStatement, handle function type, symbol etc transparently [Rasmussen 9/24/2020]
...
buildNondefiningProcedureHeaderStatement(const SgName & name, SgType* return_type, SgFunctionParameterList* param_list,
  ...
  // should always contain the _first_ nondefining declaration (even though this may not be the first one).
    ...
    ASSERT_not_null(isSgProcedureHeaderStatement(nondef_decl));
    ASSERT_not_null(nondef_decl->get_firstNondefiningDeclaration());
...
buildProcedureHeaderStatement(const SgName& name, SgType* return_type, SgFunctionParameterList* parameter_list,
   ...
   {
     ASSERT_not_null(return_type);
     ASSERT_not_null(parameter_list);

     SgFunctionDeclaration* nondef_decl = nullptr;
     ...
     if (func_symbol == nullptr)
        {
          nondef_decl = buildNondefiningFunctionDeclaration_T <SgProcedureHeaderStatement>
                           ( name, return_type, parameter_list, /*isMemberFunction*/false, scope,
                            /*decoratorList*/nullptr, /*functionConstVolatileFlags*/0, nullptr, nullptr, SgStorageModifier::e_default);
        }
       else
        {
     ...
     ASSERT_require(nondef_decl->get_firstNondefiningDeclaration() == nondef_decl);
...
SgAssignInitializer * SageBuilder::buildAssignInitializer_nfi(SgExpression * operand_i /*= nullptr*/, SgType * expression_type /*=nullptr*/)
   {
     SgAssignInitializer* result = new SgAssignInitializer(operand_i, expression_type);
     ASSERT_not_null(result);
     if (operand_i)
...
{
  ASSERT_not_null(funcType); // function type cannot be NULL
    ...
    // in rare cases when function calls are inserted before any prototypes exist
  {
    ...
    ASSERT_require(isMemberFunc == false);  // Liao, 11/21/2012. We assume only regular functions can go into this if-body so we can insert them into global scope by default
 ...
 // TODO: consider C++ template functions
    ...
    if (SageInterface::is_Fortran_language() || SageInterface::is_Jovial_language())
       {
         funcDecl = buildNondefiningFunctionDeclaration_T
                    <SgProcedureHeaderStatement>(name,return_type,parList,false,globalscope,nullptr,false,nullptr,nullptr,SgStorageModifier::e_default);
       }
      else
       {
          funcDecl = buildNondefiningFunctionDeclaration_T
                     <SgFunctionDeclaration>(name,return_type,parList,false,globalscope,nullptr,false,nullptr,nullptr,SgStorageModifier::e_default);
       }
...
SgCudaKernelCallExp * SageBuilder::buildCudaKernelCallExp_nfi(SgExpression * kernel, SgExprListExp* parameters, SgCudaKernelExecConfig * config)
  ...
  // DQ (1/19/2016): Adding template function ref support.
     else if ( (func_ref_exp        != NULL && func_ref_exp->get_symbol_i()->get_declaration()->get_functionModifier().isCudaKernel() == false) &&
...
SageBuilder::buildDoWhileStatement_nfi(SgDoWhileStmt* result, SgStatement * body, SgStatement * condition)
  ...
  // handled (end-of-construct entries).
     ...
     ASSERT_require(result->get_body() == nullptr);
     ASSERT_require(result->get_condition() == nullptr);
     ...
     ASSERT_not_null(result->get_body());
     ASSERT_not_null(result->get_condition());

     ASSERT_require(result->get_body()->get_parent()      == result);
     ASSERT_require(result->get_condition()->get_parent() == result);
   }
...
// Builder functions for type size (kind) expressions for Fortran and Jovial
...
SageBuilder::buildNondefiningEnumDeclaration_nfi(const SgName& name, SgScopeStatement* scope)
   {
  // The support for SgEnumDeclaration is identical to that for SgClassDeclaration (except for the type handling, why is that?).
     ASSERT_not_null(scope);

  // DQ (7/27/2012): Note that the input name should not have template argument syntax.
  // I think this could still fail for a function with a name such as "X<Y>"  strange converstion operators.
     ASSERT_require(SageInterface::hasTemplateSyntax(name) == false);

     SgEnumType* enumType = nullptr;
     SgEnumDeclaration* first_nondefdecl = nullptr;
  ...
  // Set the defining and first non-defining declarations.
     if (first_nondefdecl)
        {
        ...
        {
          nondefdecl->set_firstNondefiningDeclaration(nondefdecl);
          nondefdecl->set_definingDeclaration(nullptr);
        }
  ...
  // Any non-defining declaration is not always a forward declaration.
     ...
     if (scope)
        {
       // Check for an existing symbol (reuse it if it is found).
          SgEnumSymbol* mysymbol = nullptr;
          SgEnumSymbol* existing_symbol = scope->lookup_enum_symbol(name);
     ...
     ASSERT_not_null(nondefdecl->get_type());
     ASSERT_not_null(scope->lookup_enum_symbol(name));
...
SgStatement* SageBuilder::buildStatementFromString(const std::string& s, SgScopeStatement * scope)
{
  ...
  ROSE_ASSERT(AstFromString::c_char== s.c_str());
...
SgExpression * instantiateNonrealRefExps(
  SgExpression * expr,
  std::vector<std::vector<SgTemplateParameter *>> & tpl_params,
  std::vector<std::vector<SgTemplateArgument *>> & tpl_args
) {
  ...
  if (!expr) {
    ...
    unsigned depth = 0;
    unsigned pos = 0;
    for (auto tpl_params_: tpl_params) {
      pos = 0;
      for (auto tpl_param: tpl_params_) {
    ...
    if (depth < tpl_args.size() && pos < tpl_args[depth].size()) {
      SgExpression * res = tpl_args[depth][pos]->get_expression();
      ROSE_ASSERT(res);
      return res;
    } else if (depth < tpl_params.size() && pos < tpl_params[depth].size()) {
      SgExpression * dft = tpl_params[depth][pos]->get_defaultExpressionParameter();
      ROSE_ASSERT(dft);
      dft = instantiateNonrealRefExps(dft, tpl_params, tpl_args);
      ROSE_ASSERT(dft);
      return dft;
    }
#if DEBUG_Rose_Builder_Templates_instantiateNonrealRefExps
    std::cerr << "Error: In instantiateNonrealRefExps: this should not be reached!!!" << std::endl;
#endif
    return vref;
...
#endif
    SgExpression * eres = instantiateNonrealRefExps(szo->get_operand_expr(), tpl_params, tpl_args);
    ROSE_ASSERT(szo->get_operand_expr() == nullptr || eres != nullptr);
    szo->set_operand_expr(eres);

    SgType * tres = instantiateNonrealTypes(szo->get_operand_type(), tpl_params, tpl_args);
    ROSE_ASSERT(szo->get_operand_type() == nullptr || tres != nullptr);
    szo->set_operand_type(tres);
...
SgExpression * instantiateNonrealRefExps(
  ...
  std::vector<std::vector<SgTemplateParameter *>> tpl_params_{tpl_params};
  std::vector<std::vector<SgTemplateArgument *>> tpl_args_{tpl_args};
  return instantiateNonrealRefExps(expr, tpl_params_, tpl_args_);
}
...
SgType * instantiateNonrealTypes(
  ...
  if (!type) {
    ...
    std::cout << "    ->get_qualified_name() = " << nrdecl->get_qualified_name().getString() << std::endl;
//  std::cout << "    ->unparseToString()    = " << nrdecl->unparseToString() << std::endl;
    ...
    if (tpldecl != nullptr) {
      ...
      if (xtpldecl != nullptr) {
        ...
        for (auto inst_tpl_arg: inst_tpl_args) {
          inst_tpl_arg->set_parent(inst_decl);
        }
      ...
      auto depth = nrdecl->get_template_parameter_depth();
      auto position = nrdecl->get_template_parameter_position();
#if DEBUG_Rose_Builder_Templates_instantiateNonrealTypes
      std::cout << "    ->get_template_parameter_position() = " << std::dec << position << std::endl;
      std::cout << "    ->get_template_parameter_depth() = " << std::dec << depth << std::endl;
#endif
      ROSE_ASSERT(depth > 0);
    ...
    ROSE_ASSERT(btype != nullptr);
    SgModifierType * rtype = SageBuilder::buildModifierType(btype);
    ROSE_ASSERT(rtype != nullptr);
    rtype->get_typeModifier() = mtype->get_typeModifier();
    return rtype;
  } else {
    std::cerr << "!!! type = " << std::hex << type << " : " << ( type ? type->class_name() : "" ) << std::endl;
    ROSE_ABORT(); // TODO maybe pointer/reference
  }
  return nullptr;
}
...
SgType * instantiateNonrealTypes(
  ...
  std::vector<std::vector<SgTemplateParameter *>> tpl_params_{tpl_params};
  std::vector<std::vector<SgTemplateArgument *>> tpl_args_{tpl_args};
  return instantiateNonrealTypes(type, tpl_params_, tpl_args_);
}
