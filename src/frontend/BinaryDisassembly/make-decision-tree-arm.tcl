...
proc doLeaf {insn} {
  ...
  if {[info exists astNode($insn)]} {
    return [format "{instruction = new SgAsmArm$astNode($insn)(address,  mnemonic, \"\"); goto done_arm;}"]
  } else {
    return [format "{instruction = new SgAsmInstruction(address,  mnemonic, \"\"); goto done_arm;}"]
  }
}
...
proc makeSwitch {len prefix indent} {
  ...
  if {[array size firstChar] == 0} {
    ...
    puts "${indent}} else {"
    puts "${indent}  instruction = new SgAsmInstruction(address,  mnemonic, \"\"); goto done_arm;"
    puts "${indent}}"
  } else {
    ...
    puts "${indent}  default: instruction = new SgAsmInstruction(address,  mnemonic, \"\"); goto done_arm;"
    puts "${indent}}"
  }
}
...
puts "  default: instruction = new SgAsmInstruction(address,  mnemonic, \"\");"
puts "}"
puts "done_arm: ;"
