...
// ********************************************************************
// ********************************************************************
//     General support functions for c_actioFn implementation
// ********************************************************************
// ********************************************************************
...
build_implicit_program_statement_if_required()
  ...
  // we need to generate a program-stmt as a compiler generated construct.
     ...
     if (currentScope->variantT() == V_SgGlobal)
       ...
       // references.
          // fixup_forward_type_declarations();  // DXN (01/25/2011): forward type reference is fixed up in c_action_derived_type_stmt and c_action_derived_type_spec
...
#if DXN_DEBUG
     outputState("At BOTTOM of R304 c_action_name()");
#endif
   }
...
void c_action_char_length(ofp_bool hasTypeParamValue)
  ...
  // if (astTypeStack.empty() == true)
     if (astBaseTypeStack.empty() == true)
       ...
       // DXN (12/16/2010): Need to consider the case char_length : T_LPAREN type_param_value T_RPAREN
          SgExpression* stringLength;
          if (hasTypeParamValue)
             { // need to use astTypeParameterStack since in c_action_type_param_value
        	   // the top of astExpressionStack was popped and pushed onto astTypeParameterStack
           	   ROSE_ASSERT(!astTypeParameterStack.empty());
       ...
       // DXN (12/17/2010):
          if (astTypeStack.empty())
             ...
             {
           	   SgArrayType* currentType = isSgArrayType(astTypeStack.front());
           	   if(currentType != NULL)
           	      {
           		    currentType->set_base_type(stringType);
           	      }
           	   else
           	      {
           		   astTypeStack.push_front(stringType);
...
// Build a SgClassDeclaration to hold the Fortran Type (maybe it should be a SgFortranType derived from a SgClassDeclaration?)
  ...
  // printf ("id->text = %s \n",id->text);
     ...
     // DXN (01/12/2011):
     // need to check for the case where the derived type has been previously defined as a forward declaration.
     // If it has been defined as a forward declaration, then complete its definition;
     // otherwise build a new derived type statement.
     // NOTE: type re-definition may occur.
     SgDerivedTypeStatement* derivedTypeStatement = NULL;
     SgClassSymbol* derivedTypeSymbol = trace_back_through_parent_scopes_lookup_derived_type_symbol(id->text, currentScope );
     if (derivedTypeSymbol == NULL)
     	 {
           derivedTypeStatement = buildDerivedTypeStatementAndDefinition(id->text,currentScope);
     	 }
     else
     	 {
    	   SgClassDeclaration* nonDefiningClassDecl = derivedTypeSymbol->get_declaration();
    	   if (nonDefiningClassDecl->get_definingDeclaration() == NULL)
    	 	   {  // forward reference case: fix up
    		     ...
    		     derivedTypeStatement = new SgDerivedTypeStatement(id->text,SgClassDeclaration::e_struct,NULL,classDefinition);
    		     classDefinition->set_declaration(derivedTypeStatement);
    		     derivedTypeStatement->set_definingDeclaration(derivedTypeStatement);
    		     derivedTypeStatement->set_firstNondefiningDeclaration(nonDefiningClassDecl);
    		     derivedTypeStatement->set_scope(currentScope);
    		     derivedTypeStatement->set_parent(currentScope);
    		     derivedTypeStatement->set_type(nonDefiningClassDecl->get_type());
    		     nonDefiningClassDecl->set_definingDeclaration(derivedTypeStatement);
    		     nonDefiningClassDecl->get_scope()->remove_symbol(derivedTypeSymbol);
    		     nonDefiningClassDecl->set_scope(currentScope);
    		     nonDefiningClassDecl->set_parent(currentScope);
    		     currentScope->insert_symbol(id->text, derivedTypeSymbol);

    		     while (!astAttributeSpecStack.empty())
   		       	     {
   		    	       setDeclarationAttributeSpec(derivedTypeStatement,astAttributeSpecStack.front());
    	   ...
    	   else
    	 	   {
                 derivedTypeStatement = buildDerivedTypeStatementAndDefinition(id->text,currentScope);
                 // DXN (01/24/2011):
                 // if this derived type is defined inside a function
                 //      replace the function return type with this derived type if their names are the same
                 SgFunctionDefinition* pFuncDef = isSgFunctionDefinition(currentScope->get_scope());
                 if (pFuncDef != NULL)
                    {
                	  SgFunctionType* pFuncType = pFuncDef->get_declaration()->get_type();
                	  SgClassType* pReturnType = isSgClassType(pFuncType->get_return_type());
                	  if (pReturnType && (derivedTypeStatement->get_name() == pReturnType->get_name()))
                	     {
                		   pFuncType->set_return_type(derivedTypeStatement->get_type());
                	     }
                    }
    	 	   }
     	 }
...
void c_action_end_type_stmt(Token_t *label, Token_t *endKeyword, Token_t *typeKeyword, Token_t *id, Token_t *eos)
  ...
  // handled separately (and are a bit more complex).
     //fixup_possible_incomplete_function_return_type();  // DXN (01/21/2011): TO CHECK
#endif
#endif

#if DXN_DEBUG
  // Output debugging information about saved state (stack) information.
     outputState("At BOTTOM of R433 c_action_end_type_stmt()");
#endif
   }
...
// DXN (01/25/2011): process component_decl using most of the code from entity_decl
     if ( SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL )
        {
          string current_filename = getCurrentFilename();
#if ROSE_OFP_MINOR_VERSION_NUMBER >= 8 & ROSE_OFP_PATCH_VERSION_NUMBER >= 0
          cout << "component name = " << id->text << " file name = " << current_filename << "hasComponentArraySpec = " << hasComponentArraySpec
        		  << " hasCoArraySpec = " << hasCoArraySpec << " hasCharLength = " << hasCharLength << " hasComponentInitialization = " << hasComponentInitialization << endl;
#else
          printf ("In R442 c_action_component_decl(): save variableName = %s file = %s \n",id->text,current_filename.c_str());
#endif
        }

#if DXN_DEBUG
  // Output debugging information about saved state (stack) information.
     outputState("At TOP of R442 c_action_component_decl()");
#endif
          ...
          if (classType != NULL || arrayType != NULL)
             {
               ...
               if (arrayType != NULL)
                  ...
                  {
                    ROSE_ASSERT(astExpressionStack.size() == 1);
                    initializer = isSgConstructorInitializer(astExpressionStack.front());
                    if (initializer != NULL)
                       {
                         // printf ("This is a SgConstructorInitializer, we need an example of code that generates this case. \n");
                         // DXN (01/26/2011): for example, type (T) :: x = T()
                      ...
                      else
                       {
                          ...
                          // Leave it on the stack and process it later;
                          // DXN (01/26/2011): for example, type(T), bind (C, name = "abc") :: x
  ...
  // DQ (5/15/2008): Added support to make sure that this is not a previously declared name (see test2008_29.f)
     // DXN: for example, type declaration of a subroutine/function parameter,
     // the parameter has already appeared in the formal parameter list.
     ...
     ROSE_ASSERT(initializedName != NULL);
        {
#if DXN_DEBUG
     outputState("Before cleaning up the stacks in R442 c_action_component_decl()");
#endif
  ...
  // FMZ (2/19/2007): Added for CoArray
     ...
     if ( SgProject::get_verbose() > DEBUG_COMMENT_LEVEL )
          printf ("In R442 c_action_component_decl(): Initializer for variable initializedName = %p = %s is %p = %p \n",initializedName,name.str(),initializer,initializedName->get_initializer());
...
#if DXN_DEBUG
     outputState("At BOTTOM of R442 c_action_component_decl()");
#endif
...
void c_action_derived_type_spec(Token_t * typeName, ofp_bool hasTypeParamSpecList)
   {
    ...
    if (derivedTypeSymbol == NULL)
       {
    	// DXN (01/27/2011):the type specified is a forward reference;
    	// Get the appropriate scope to build a forward referenced derived type:
    	// if the parent of the current scope is a SgDerivedTypeStatement then get the outer scope, otherwise stay with the current scope:
    	SgDerivedTypeStatement* parent = isSgDerivedTypeStatement(getTopOfScopeStack()->get_parent());
    	SgScopeStatement* scope = (parent != NULL)?	getTopOfScopeStack()->get_scope(): getTopOfScopeStack();

         // build a non-defining class declaration in this scope to represent this forward reference type.
         SgDerivedTypeStatement* nondefiningClassDeclaration = new SgDerivedTypeStatement(typeName->text,SgClassDeclaration::e_struct,NULL,NULL);

         // set the usual fields for this  non-defining class declaration:
         ...
         nondefiningClassDeclaration->set_definingDeclaration(NULL);  // to indicate there is no defining class declaration yet.

         // create a class symbol the non-defining class declaration and add it to the local symbol table.
         SgClassSymbol* classSymbol = new SgClassSymbol(nondefiningClassDeclaration);
         scope->insert_symbol(typeName->text,classSymbol);
         derivedType = nondefiningClassDeclaration->get_type();
       }
    else
       {
       // The derived type symbol may only be a forward reference, but it should not matter.
...
#endif
// DXN (01/24/2011) : why change the function type?
 ...
 * data_implied_do
 *
 * @param id T_IDENT token: the control variable
 * @param hasThirdExpr Flag to specify if optional third expression was 
 * given.  true means there is a stride.
 */
void c_action_data_implied_do(Token_t *id, ofp_bool hasThirdExpr)
  ...
  // Output debugging information about saved state (stack) information.
     ...
     SgName do_variable_name(id->text);

     // get the lower bound, upper bound and stride:
     ...
     // build the variable initialization do_variable_name = lowerBound:
     // TODO: Build appropriate scope for implied do loop control variable; its type must be INTEGER.
     SgVariableSymbol* variableSymbol = lookup_variable_symbol_build_if_needed(do_variable_name, astScopeStack.front());
     if (variableSymbol == NULL)
        {
    	  printf("Implicit None = %s \n", isImplicitNoneScope()? "true": "false");
    	  printf("No surrounding declaration found for the implied do loop control variable %s \n", do_variable_name.str());
    	  ROSE_ASSERT(false);
        }
     SgVarRefExp* doLoopVar = SageBuilder::buildVarRefExp(variableSymbol);
     ROSE_ASSERT(doLoopVar != NULL);
     SgExpression* doVarInitialization = SageBuilder::buildAssignOp(doLoopVar,lowerBound);
     ROSE_ASSERT(doVarInitialization != NULL);

     // get the data-implied-do-object-list and build the data-implied-do object:
     ROSE_ASSERT(!astExpressionStack.empty());
     SgExprListExp* objectList = isSgExprListExp(astExpressionStack.front());
     ROSE_ASSERT(objectList != NULL);
     setSourcePosition(objectList);
     astExpressionStack.pop_front();
     SgScopeStatement* implied_do_scope = NULL; // TODO: need to build the correct scope to satisfy implied do semantic.
     SgImpliedDo* impliedDo = new SgImpliedDo(doVarInitialization,upperBound, stride, objectList,implied_do_scope);
     ROSE_ASSERT(impliedDo != NULL);
     setSourcePosition(impliedDo);
     objectList->set_parent(impliedDo);
     stride->set_parent(impliedDo);
     upperBound->set_parent(impliedDo);
     doVarInitialization->set_parent(impliedDo);

     // push the implied do object on the astExpressionStack:
     astExpressionStack.push_front(impliedDo);
       ...
       // We have a "DO I=1,100" style loop
          ...
          if (astExpressionStack.size() == 3)
            ...
            // There is a ending index, starting, index, and the index variable on the stack
               ...
               astNameStack.pop_front();
               SgVariableSymbol* variableSymbol = lookup_variable_symbol_build_if_needed(variableName, astScopeStack.front()); // DXN (12/21/2010)
               ROSE_ASSERT(variableSymbol != NULL);
...
#if 1
  // Output debugging information about saved state (stack) information.
     outputState("At TOP of R831 c_action_do_variable()");
#endif
...
void c_action_io_implied_do()
  ...
  // Output debugging information about saved state (stack) information.
     ...
     // DXN: The implied_do_control object is at the top of the stack.
     // See c_action_io_implied_do_control.
     // get the do loop var initialization, the upper bound and the increment:
     SgExprListExp* implied_do_control = isSgExprListExp(astExpressionStack.front());
     ...
     ROSE_ASSERT(upperBound != NULL);
     SgExpression* increment = implied_do_control->get_expressions()[2];
...
#if 1
  // Output debugging information about saved state (stack) information.
     outputState("IN R917 c_action_io_implied_do(): before checking for SgExprListExp* objectList = isSgExprListExp(astExpressionStack.front()");
#endif

     // get the io-implied-do-object-list and build the io-implied-do object:
     ROSE_ASSERT(!astExpressionStack.empty());
     SgExprListExp* objectList = isSgExprListExp(astExpressionStack.front());
     // objectList should be a list of expressions, per c_action_io_implied_do_object:
     ROSE_ASSERT(objectList != NULL);
     setSourcePosition(objectList);
     astExpressionStack.pop_front();
     SgScopeStatement* implied_do_scope = NULL;  // TODO: need to build the correct scope to satisfy implied do semantic.
     ...
     // push the io-implied-do object on the astExpressionStack:
     astExpressionStack.push_front(impliedDo);
...
void c_action_io_implied_do_control()
  ...
  // Output debugging information about saved state (stack) information.
...
#endif
     // DXN (01/04/2011): build the whole implied_do_control object with stride;
     // the stride may be an SgNullExpression.
     ...
     // build the variable initialization do_variable_name = lowerBound:
     // DXN TODO: Build appropriate scope for implied do loop control variable; its type must be INTEGER.
     SgVariableSymbol* variableSymbol = lookup_variable_symbol_build_if_needed(do_variable_name, astScopeStack.front());
     if (variableSymbol == NULL)
        {
    	  printf("Implicit None = %s \n", isImplicitNoneScope()? "true": "false");
     ...
     // build the implied-do-control object and push it on the astExpressionStack:
...
void c_action_actual_arg_spec(Token_t * keyword)
   {
    ...
    // DXN (01/06/2011): keyword may be NULL and
	// when it not NULL, must create SgActualArgumentExpression here and not later.
#if DXN_DEBUG
     outputState("At TOP of R1220 c_action_actual_arg_spec()");
#endif
	...
	if (label != NULL)
	   {
		 ...
		 /**
		  * TODO: replace SgVarRefExp with SgLabelRefExp;
		 **/
		 astExpressionStack.push_front(p_actualArg);
...
#if 1
  // Output debugging information about saved state (stack) information.
     outputState("At TOP of R1236 c_action_return_stmt");
#endif

     // DXN (01/05/2011): get the return expression from the astExpresssionStack:
     if (hasScalarIntExpr)
        {
          ROSE_ASSERT(!astExpressionStack.empty());
...
void c_action_end_of_stmt(Token_t * eos)
   {
	...
	if ( SgProject::get_verbose() > 0)  // DXN
