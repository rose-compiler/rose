...
// DXN (12/20/2010):
// lookup the variable in parent scopes;
// if found
//    return the corresponding variable symbol,
// else if the current scope is not implicit none
//    build the corresponding implicit variable declaration
//    look up again and return the lookup result.
SgVariableSymbol* lookup_variable_symbol_build_if_needed (const SgName & variableName, SgScopeStatement* currentScope )
   {
     SgVariableSymbol* variableSymbol = NULL;
     SgFunctionSymbol* functionSymbol = NULL;
...
buildDerivedTypeStatementAndDefinition (string name, SgScopeStatement* scope)
  ...
  // Set the non defining declaration in the defining declaration (both are required)
     ...
     assert(nondefiningClassDeclaration != NULL);
...
SgType* generateImplicitType( string name )   {
...
/*
// DQ (3/3/2008): Note that array declarations that appear in types are are processed
// using different rules and different enum values for similar concepts.
...
#if 0
// DQ (10/25/2010): I just noticed that this function is no longer called.
void
...
processBindingAttribute( SgDeclarationStatement* declaration)
  ...
  // printf ("EXITING at base of processBindingAttribute() \n");
  // ROSE_ASSERT(false);
   }
...
#endif
       // Capture the procedure parameters and take their names off the token stack (astNameStack).
          int asteriskCount = 0;
          while (astNameStack.empty() == false)
             {
            ...
            // Now build associated SgVariableSymbol and put it into the current scope (function definition scope)
              ...
              procedureDefinition->insert_symbol(argName,variableSymbol);
...
#if 0
                           // DQ (4/26/2008): This is required for type initialization (which appear 
                           // to be handled a bit like functions and arrays and are equally ambigious 
                           // at early stages of translation).
                              ROSE_ASSERT(astInitializerStack.empty() == false);
                              subscript = astInitializerStack.front();
                              astInitializerStack.pop_front();

                              SgExprListExp* expressionList = isSgExprListExp(subscript);
                              if (expressionList != NULL)
                                 {
                                   for (unsigned long i = 0; i < expressionList->get_expressions().size(); i++)
                                      {
                                        printf ("expressionList->get_expressions()[i] = %s \n",expressionList->get_expressions()[i]->class_name().c_str());
                                      }
                                 }
#endif
...
use_statement_fixup()
   {
     ...
     // fixup_possible_incomplete_function_return_type(); // DXN (01/25/2011): forward type reference is fixed up in c_action_derived_type_stmt and c_action_derived_type_spec

     // DXN (01/28/2011): if the use statement is inside a function definition
     // fix up the return type of the function if the function return type has the same name as one
     // of the derived types defined in the used modules.
