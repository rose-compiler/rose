...
void build_implicit_program_statement_if_required()
{
    ...
    if (currentScope->variantT() == V_SgGlobal)
    {
        if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
            printf(
                    "Building an implicit program-stmt: currentScope = %p = %s \n",
...
void c_action_specification_part(int numUseStmts, int numImportStmts,
...
#endif
{
...
#if ROSE_OFP_MINOR_VERSION_NUMBER >= 8 & ROSE_OFP_PATCH_VERSION_NUMBER >= 0
        printf(
                "In c_action_specification_part: numUseStmts = %d numImportStmts = %d numImplStmts = %d numDeclConstructs = %d \n",
    ...
    if (SgProject::get_verbose() > 1)
    {
        ...
        printf(
                "################################################################################# \n");
        printf(
                "#############   END OF DECLARATIONS START OF ACTION STATEMENTS   ################ \n");
        printf(
                "################################################################################# \n");
        printf("\n\n");
    }
...
void c_action_declaration_construct()
{
    ...
    if (astLabelSymbolStack.empty() == false)
    {
        if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
            printf(
                    "WARNING: clearing the astLabelSymbolStack (depth = %zu) \n",
...
void c_action_executable_construct()
{
    ...
    if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
        printf(
                "In c_action_executable_construct(): astExpressionStack = %zu astNodeStack = %zu \n",
    ...
    {
        printf(
                "Error: There appear to be unused expressions on the expression stack \n");
    }
...
void c_action_action_stmt()
{
    ...
    if (astNodeStack.empty() == false)
    {
        if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
            printf(
                    "Clearing the astNodeStack of IR nodes that were pushed so that a possible where statement could use them \n");
 ...
 */
void c_action_defined_operator(Token_t * definedOp,
        ofp_bool isExtendedIntrinsicOp)
{
    if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
        printf(
                "In c_action_defined_operator(): definedOp = %p = %s isExtendedIntrinsicOp = %s \n",
                definedOp, definedOp != NULL ? definedOp->text : "NULL",
                isExtendedIntrinsicOp ? "true" : "false");
    ...
    {
        printf(
                "astNameStack is non-empty (skip pushing another copy on the stack: astNameStack.front() = %s \n",
...
void c_action_data_ref(int numPartRef)
{
                    ...
                    else
                    {
                        ...
                        if (functionArguments != NULL)
                        {
                            SgFunctionCallExp* functionCallExp =
                                    new SgFunctionCallExp(functionRefExp,
    ...
    for (size_t i = 0; i < variableSymbolList.size(); i++)
    {
        ...
        bool hasSelectionSubscriptList =
                qualifiedNameList[lastElement - i].hasSelectionSubscriptList;
        bool hasImageSelector =
                qualifiedNameList[lastElement - i].hasImageSelector;
        ...
        if (variableType->variantT() != V_SgFunctionType)
        {
            ...
            if (hasCo_deref)
            {
                ...
                SgVarRefExp* temp_var = new SgVarRefExp(temp_variableSymbol); // DXN: TODO -need to consider case with aliasing
                ...
                variable = new SgVarRefExp(temp_variableSymbol); // DXN (08/19. 2011): TODO should use the top of astNameStack because of aliasing?
            ...
            if (hasSelectionSubscriptList == true)
            {
                ...
                if (astExpressionStack.empty() == false)
                {
                    ...
                    SgArrayType* arrayType = isSgArrayType(
                            indexExpression->get_type());
                    if (arrayType == NULL && stackHoldsAnIndexExpression
                            == true)
                    {
                        astExpressionStack.pop_front();
                        variable = new SgPntrArrRefExp(arrayVariable,
        ...
        case V_SgFunctionType:
        {
            ...
            else
            {
                ...
                if (stackHoldsAnIndexExpression == true)
                {
                    ...
                    functionSymbol = globalScope->lookup_function_symbol(
                            programName);
                    ROSE_ASSERT(functionSymbol != NULL);
...
void c_action_result_name()
{
    ...
    SgInitializedName* initializedName = new SgInitializedName(arg_name,
            generateImplicitType(arg_name.str()));
        ...
        Token_t * keyword2, Token_t * name, Token_t * eos)
{
    ...
    if (func_decl != NULL && func_decl->isFunction() == true)
    {
        ...
        if (isSgTypeVoid(func_return_type) != NULL)
        {
            ...
            {
                SgFunctionType* implicit_type = generateImplicitFunctionType(
                        func_name);
