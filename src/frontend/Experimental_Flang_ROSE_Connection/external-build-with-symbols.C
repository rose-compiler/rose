...
void SymbolBuildVisitor::PrintSymbols(
...
void SymbolBuildVisitor::Indent(std::ostream &out, int indent) const {
  for (int i{0}; i < indent; ++i) {
    out << ' ';
  }
}

void SymbolBuildVisitor::Post(const parser::Name &name) {
...
void BuildWithSymbols(std::ostream &out, const parser::Program &program,
    parser::Encoding encoding) {
  SymbolBuildVisitor visitor;
  parser::Walk(program, visitor);
  parser::preStatementType preStatement{
      [&](const parser::CharBlock &location, std::ostream &out, int indent) {
        visitor.PrintSymbols(location, out, indent);
      }};
  builder::Build(out, program, encoding, false, true, &preStatement);
}
}
