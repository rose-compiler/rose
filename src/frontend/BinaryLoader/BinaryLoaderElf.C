...
#if 0
void BinaryLoaderElf::handleSectionMapped(SgAsmGenericSection* section)
{
 ...
 */
void getDynamicVars(SgAsmElfFileHeader* elfHeader, 
		    string& rpath,
		    string& runpath)
{
...
Rose_STL_Container<string> getLdPreload(SgAsmElfFileHeader* /*header*/)
{
  Rose_STL_Container<string> ret;
...
Rose_STL_Container<string> BinaryLoaderElf::getDLLs(SgAsmGenericHeader* header,
						    const Rose_STL_Container<string> &dllFilesAlreadyLoaded)
{
  ...
  Rose_STL_Container<string> ldLibs=getLdPreload(elfHeader);

  std::string rpath,runpath;
  getDynamicVars(elfHeader,rpath,runpath);
  
  Rose_STL_Container<string> libPaths;
  ...
  Rose_STL_Container<string> ldLibPaths = getLdLibraryPaths(elfHeader);
  ...
  if(!runpath.empty()){
    ...
    Rose_STL_Container<string> runPaths = parseLdPath(runpath);
    libPaths.insert(libPaths.end(),runPaths.begin(),runPaths.end());
  }
  
  if((header->get_isa() & SgAsmGenericHeader::ISA_FAMILY_MASK) == 
     SgAsmGenericHeader::ISA_X8664_Family){
...
void BinaryLoaderElf::addSectionsForLayout(SgAsmGenericHeader* header, 
...
//typedef std::map<string, SgAsmElfSymbol*> SymbolMap;
...
public:
  ...
  string dump_versioned_name() const
  {
...
public:
  ...
  public:
	...
	return get_base_version();
      ...
      string neededVersion=version.get_version_need()->get_name()->c_str();
  ...
  /* We store a list of Entries.  The first entry is the 'base' version
     ...
     will be used
   */
  typedef std::map<string, Entry> BaseMap;

  const Entry* find(string name) const
  {
  ...
  const Entry* find(string name, string version) const
  {
    if(version.empty())
      return find(name);
    else
		      ...
		      const size_t addrSize)
{
  ...
  string symbolName=relocVSymbol.get_name();
...
bool relocateInterpLibraries(SgAsmInterpretation* interp)
{
  ...
  // build map for each symbol table map<string,SgElfSymbol>
  ...
  for(size_t h=0; h < headers.size(); ++h){
	...
	if(verbose) printf("\n");
	string symName=symver.get_name();
  ...
  for( size_t i=0; i < symbolMaps.size(); ++i){
    ...
    for(; newSymbolIter != symbolMap->end(); ++newSymbolIter){
      ...
      const string symName = newSymbolIter->first;
...
/**
 */
bool BinaryLoaderElf::relocateAllLibraries(SgBinaryComposite* binaryFile)
{
