...
/** Motorola S-Record.
 *
 *  Motorola S-record is a file format, created by Motorola, that conveys binary information in ASCII hex text form. This file
 *  format may also be known as SRECORD, SREC, S19, S28, S37. It is commonly used for programming microcontrollers, EPROMs, and
 *  other types of programmable logic devices. In a typical application, a compiler or assembler converts a program's source
 *  code (such as C or assembly language) to machine code and outputs it into a HEX file. The HEX file is then imported by a
 *  programmer to "burn" the machine code into a ROM, or is transferred to the target system for loading and execution. */
class SRecord {
public:
    /** Type of S-Record. */
    enum Type {
...
public:
     ...
     *
     *  An S-Record checksum is the 1's complement of the sum of data bytes modulo 256. */
