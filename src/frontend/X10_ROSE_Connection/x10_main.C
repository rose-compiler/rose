...
// separate the work on Java from the rest of ROSE and support the ROSE
// configuration language only options.
list<SgScopeStatement *> astX10ScopeStack;

bool emptyX10StateStack() {
    // Use the scope stack to indicate if we have a value scope available as part of Java lanaguage processing.
    return astX10ScopeStack.empty();
}


SgScopeStatement *getTopOfX10ScopeStack() {
    ROSE_ASSERT(astX10ScopeStack.empty() == false);
    SgScopeStatement* topOfStack = astX10ScopeStack.front();
