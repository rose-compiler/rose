...
UnparserX86::emitTypeName(std::ostream &out, SgAsmType *type, State &state) const {
    ...
    if (SgAsmIntegerType *it = isSgAsmIntegerType(type)) {
        ...
        out <<"v" <<vt->get_nElmts();
        emitTypeName(out, vt->get_elmtType(), state);
    } else {
...
UnparserX86::emitOperandBody(std::ostream &out, SgAsmExpression *expr, State &state) const {
    ...
    if (SgAsmBinaryExpression *bin = isSgAsmBinaryExpression(expr)) {
        emitOperandBody(out, bin->get_lhs(), state);
        ...
        emitOperandBody(out, bin->get_rhs(), state);
        ...
        if (!isLea) {
            emitTypeName(out, mr->get_type(), state);
            out <<" ";
            if (mr->get_segment()) {
                emitOperandBody(out, mr->get_segment(), state);
                out <<":";
            }
            out <<"[";
            emitOperandBody(out, mr->get_address(), state);
            out <<"]";
        }

    } else if (SgAsmDirectRegisterExpression *rr = isSgAsmDirectRegisterExpression(expr)) {
        emitRegister(out, rr->get_descriptor(), state);

    } else if (SgAsmIndirectRegisterExpression *rr = isSgAsmIndirectRegisterExpression(expr)) {
        std::ostringstream ss;
        emitRegister(ss, rr->get_descriptor(), state);
        std::string s = ss.str();
