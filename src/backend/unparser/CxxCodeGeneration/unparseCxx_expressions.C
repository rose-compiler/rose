...
//#else
  ...
  // curprint ( "\n /* Inside of unparseMFuncRef func_name = " + func_name + " */ \n");
  ...
  // Return false for any parameter of an expression list (which will be comma separated).
     ...
     switch (expr->variant())
       ...
       // case ASSIGN_INIT:
          ...
          default:
             {
               int parentVariant = GetOperatorVariant(parentExpr); // TODO operator variants
               ...
               //int exprVariant = GetOperatorVariant(expr); TODO operator variants
...
Unparse_ExprStmt::getPrecedence(SgExpression* exp) {
  // DQ (11/24/2007): This is a redundant mechanism for computing the precidence of expressions
     ...
     int variant = GetOperatorVariant(exp);
     switch (variant)
        {
