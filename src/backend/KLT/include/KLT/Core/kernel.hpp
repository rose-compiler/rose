...
class Kernel {
  ...
  protected:
    ...
    ///
    bool p_content_done;
    /// the list of perfectly nested loops, it will become the outter most iteration domain of the kernel (fully parallel)
  ...
  public:
    ...
    /// \return List of the coefficients in the order they need to be provided to the kernel
    ...
    /// \return List of perfectly nested loops that will become the outter most iteration domain of the kernel
    const std::list<LoopTrees::loop_t *> & getPerfectlyNestedLoops() const;

    /// \return List of statements composing the body of the perfectly nested loop.
    const std::list<LoopTrees::node_t *> & getBodyBranches() const;
