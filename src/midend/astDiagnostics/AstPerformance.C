...
=======
		  Sleep(1000);
>>>>>>> a6cafed8910248195d25eb34c65c22e1f660a388
#else
          sleep(1);
#endif
          counter++;
...
struct rusage
  {
    ...
    long int ru_nivcsw;
     ...
     if (i != data.end())
        {
          int numberOf_IR_Nodes     = numberOfNodes();
          int numberOfKiloBytesUsed = memoryUsage() / (1 << 10);
	  ...
	  if (currentUsage.informationValid()) {
       ...
       //      (*i)->get_resolution(),numberOf_IR_Nodes,numberOfKiloBytesUsed,memoryComputedFromSystem);
            printf ("Performance Report (timer resolution = %f, number of IR nodes = %d, memory used (calculated from AST memory pool) = %d Kilobytes, memory used (actual) = %8.3f Megabytes ): \n",
                 (*i)->get_resolution(),numberOf_IR_Nodes,numberOfKiloBytesUsed,memoryComputedFromSystem);
	  } else {
