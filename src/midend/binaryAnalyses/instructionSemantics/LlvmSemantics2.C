...
{
    const RegisterDictionary *dictionary = currentState()->get_register_state()->get_register_dictionary();
...
RiscOperators::get_stored_registers()
{
    ...
    RegisterStatePtr regstate = RegisterState::promote(currentState()->get_register_state());
...
{
    RegisterDescriptors retval;
    RegisterStatePtr cur_regstate = RegisterState::promote(currentState()->get_register_state());
...
RiscOperators::get_instruction_pointer()
{
    ...
    return SValue::promote(currentState()->get_register_state()->readRegister(EIP, this));
}
...
{
    prev_regstate = RegisterState::promote(currentState()->get_register_state()->clone());
    mem_writes.clear();
}
    ...
    for (size_t i=0; i<insns.size(); ++i) {
        ...
        try {
            ...
            if (quiet_errors) {
                ...
                operators->currentState()->get_register_state()->writeRegister(IP_REG, fallthrough_va, operators.get());
            } else {
                throw;
            }
        }
