...
RiscOperators::resetState() {
    ...
    get_state()->clear();
    RegisterStatePtr registers = RegisterState::promote(get_state()->get_register_state());
...
RiscOperators::registerVariableName(const RegisterDescriptor &reg) {
    ...
    const RegisterDictionary *registers = get_state()->get_register_state()->get_register_dictionary();
