...
class Constraint {
  bool needsToRun;
  bool getNeedsToRun() const {return needsToRun;}
  void setNeedsToRun() {needsToRun = true;}
  void resetNeedsToRun() {needsToRun = false;}
  void runFull() { // Does changed checking and updating
    if (!getNeedsToRun()) return;
    resetNeedsToRun();
    run();
  }

  protected:
  Constraint(): needsToRun(true) {}
...
inline void Variable::pushChanges() const {
