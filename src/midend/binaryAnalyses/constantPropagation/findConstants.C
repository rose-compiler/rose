...
struct LatticeElement {
  ...
  LatticeElement(): isTop(true), name(0), definingInstruction(NULL), negate(false), offset(0) {}
  LatticeElement(uint64_t name, SgAsmx86Instruction* definingInstruction, bool negate, uint64_t offset): isTop(false), name(name), definingInstruction(definingInstruction), negate(negate), offset(offset & (GenMask<uint64_t, Len>::value)) {}
  LatticeElement(bool isTop, uint64_t name, SgAsmx86Instruction* definingInstruction, bool negate, uint64_t offset): isTop(isTop), name(name), definingInstruction(definingInstruction), negate(negate), offset(offset & (GenMask<uint64_t, Len>::value)) {}
  static LatticeElement constant(uint64_t c, SgAsmx86Instruction* definingInstruction) {return LatticeElement(0, definingInstruction, false, c);}
  static LatticeElement nonconstant(uint64_t name, SgAsmx86Instruction* definingInstruction) {return LatticeElement(name, definingInstruction, false, 0);}
...
ostream& operator<<(ostream& o, const LatticeElement<Len>& e) {
  if (e.isTop) {o << "<top>"; return o;}
  if (e.name != 0) {
    if (e.negate) o << "-";
    o << "v" << e.name;
    if (e.offset != 0) o << "+";
  }
  if (e.offset != 0 || e.name == 0) o << e.offset;
  if (e.definingInstruction != NULL) o << " [from " << unparseInstructionWithAddress(e.definingInstruction) << "]";
  return o;
}
...
struct XVariable: public Variable {
  ...
  XVariable(): value(), myName(++xvarNameCounter), def(currentInstruction) {}
  ...
  LatticeElement<Len> get() const {return value;}
};

template <size_t Len>
struct XVariablePtr {
  XVariable<Len>* var;
  XVariablePtr(): var(NULL) {}
  XVariablePtr(XVariable<Len>* var): var(var) {}
  operator XVariable<Len>*() const {return var;}
  XVariable<Len>* operator->() const {return var;}
};
...
bool mayAlias(const MemoryWrite& a, const MemoryWrite& b) {
  ...
  if (addr1.isTop) return false;
  if (addr2.isTop) return false;
  if (addr1.name != addr2.name) return true;
  if (addr1.name != 0 && addr1.negate != addr2.negate) return true;
  ...
  if (offsetDiff < a.len || offsetDiff > (uint32_t)(-b.len)) return true;
  return false;
}
...
struct MemoryWriteSet {
  ...
  MemoryWriteSet(): isTop(true), writes() {}
  ...
  bool getValueAtAddress(LatticeElement<32> address, LatticeElement<Len>& result, uint32_t resultName, SgAsmx86Instruction* resultDef) const {
    MemoryWrite mw;
    mw.address = address;
    ...
    for (size_t i = 0; i < writes.size(); ++i) {
      if (mustAlias(writes[i], mw)) {
        ...
        result = LatticeElement<Len>(data.isTop, data.name, data.definingInstruction, data.negate, data.offset); return true;}
    }
...
struct MemoryVariable: public Variable {
  ...
  const MemoryWriteSet& get() const {return mws;}
};
...
struct UnaryConstraint: public Constraint {
  ...
  UnaryConstraint(XVariablePtr<OutputLen> result, XVariablePtr<InputLen> var): result(result), var(var) {}
  virtual void run() const {
  ...
  virtual void markDependencies() {addDependency(var);}
...
#define UNARY_COMPUTATION(name, InLen, OutLen, Formula) \
  XVariablePtr<(OutLen)> name(XVariablePtr<(InLen)> a) { \
    ...
    struct IC: public UnaryConstraint<(InLen), (OutLen)> { \
      IC(XVariablePtr<(OutLen)> result, XVariablePtr<(InLen)> var1): UnaryConstraint<(InLen), (OutLen)>(result, var1) {} \
...
#define UNARY_COMPUTATION_SPECIAL(name, InLen1, OutLen, Formula) \
  XVariablePtr<(OutLen)> name(XVariablePtr<(InLen1)> a) { \
    ...
    struct IC: public UnaryConstraint<(InLen1), (OutLen)> { \
      IC(XVariablePtr<(OutLen)> result, XVariablePtr<(InLen1)> var1): UnaryConstraint<(InLen1), (OutLen)>(result, var1) {} \
      virtual void run() const { \
...
struct BinaryConstraint: public Constraint {
  ...
  BinaryConstraint(XVariablePtr<OutputLen> result, XVariablePtr<InputLen1> var1, XVariablePtr<InputLen2> var2): result(result), var1(var1), var2(var2) {}
  virtual void run() const {
    LatticeElement<InputLen1> le1 = var1->get();
    LatticeElement<InputLen2> le2 = var2->get();
    if (le1.isTop || le2.isTop) {result->set(LatticeElement<OutputLen>()); return;}
    if (le1.name != 0 || le2.name != 0) {result->set(LatticeElement<OutputLen>::nonconstant(result->myName, result->def)); return;}
    uint64_t newVal = this->compute(le1.offset, le2.offset);
    result->set(LatticeElement<OutputLen>::constant(newVal, result->def));
  }
  virtual void markDependencies() {addDependency(var1); addDependency(var2);}
...
#define BINARY_COMPUTATION(name, InLen1, InLen2, OutLen, Formula) \
  XVariablePtr<(OutLen)> name(XVariablePtr<(InLen1)> a, XVariablePtr<(InLen2)> b) { \
    ...
    struct IC: public BinaryConstraint<(InLen1), (InLen2), (OutLen)> { \
      IC(XVariablePtr<(OutLen)> result, XVariablePtr<(InLen1)> var1, XVariablePtr<(InLen2)> var2): BinaryConstraint<(InLen1), (InLen2), (OutLen)>(result, var1, var2) {} \
  ...
  XVariablePtr<(OutLen)> name(XVariablePtr<(InLen1)> a, XVariablePtr<(InLen2)> b) { \
    ...
    struct IC: public BinaryConstraint<(InLen1), (InLen2), (OutLen)> { \
      ...
      virtual void run() const { \
        ...
        if (le1.isTop || le2.isTop) {result->set(LatticeElement<(OutLen)>()); return;} \
        Formula \
      } \
      virtual uint64_t compute(uint64_t, uint64_t) const {abort();} \
    }; \
    (new IC(result, a, b))->activate(); \
    return result; \
  }
...
struct TernaryConstraint: public Constraint {
  ...
  TernaryConstraint(XVariablePtr<OutputLen> result, XVariablePtr<InputLen1> var1, XVariablePtr<InputLen2> var2, XVariablePtr<InputLen3> var3): result(result), var1(var1), var2(var2), var3(var3) {}
  virtual void run() const {
    LatticeElement<InputLen1> le1 = var1->get();
    LatticeElement<InputLen2> le2 = var2->get();
    LatticeElement<InputLen3> le3 = var3->get();
    if (le1.isTop || le2.isTop || le3.isTop) {result->set(LatticeElement<OutputLen>()); return;}
    if (le1.name != 0 || le2.name != 0 || le3.name != 0) {result->set(LatticeElement<OutputLen>::nonconstant(result->myName, result->def)); return;}
    uint64_t newVal = this->compute(le1.offset, le2.offset, le3.offset);
    result->set(LatticeElement<OutputLen>::constant(newVal, result->def));
  }
  virtual void markDependencies() {addDependency(var1); addDependency(var2); addDependency(var3);}
...
#define TERNARY_COMPUTATION(name, InLen1, InLen2, InLen3, OutLen, Formula) \
  XVariablePtr<(OutLen)> name(XVariablePtr<(InLen1)> a, XVariablePtr<(InLen2)> b, XVariablePtr<(InLen3)> c) { \
    ...
    struct IC: public TernaryConstraint<(InLen1), (InLen2), (InLen3), (OutLen)> { \
      IC(XVariablePtr<(OutLen)> result, XVariablePtr<(InLen1)> var1, XVariablePtr<(InLen2)> var2, XVariablePtr<(InLen3)> var3): TernaryConstraint<(InLen1), (InLen2), (InLen3), (OutLen)>(result, var1, var2, var3) {} \
  ...
  XVariablePtr<(OutLen)> name(XVariablePtr<(InLen1)> a, XVariablePtr<(InLen2)> b, XVariablePtr<(InLen3)> c) { \
    ...
    struct IC: public TernaryConstraint<(InLen1), (InLen2), (InLen3), (OutLen)> { \
      ...
      virtual void run() const { \
        ...
        if (le1.isTop || le2.isTop || le3.isTop) {result->set(LatticeElement<(OutLen)>()); return;} \
        Formula \
      } \
      virtual uint64_t compute(uint64_t, uint64_t, uint64_t) const {abort();} \
    }; \
    (new IC(result, a, b, c))->activate(); \
    return result; \
  }
...
XVariablePtr<Len> bottom() {
  struct BottomConstraint: public Constraint {
    ...
    BottomConstraint(XVariablePtr<Len> var): var(var) {}
    virtual void run() const {
...
struct MergeConstraint: public Constraint {
  ...
  MergeConstraint(XVariablePtr<Len> result, XVariablePtr<Len> var1): result(result), var1(var1) {}
  virtual void run() const {
    result->set(var1->get());
  }
  virtual void markDependencies() {addDependency(var1);}
};

struct MemoryMergeConstraint: public Constraint {
  MemoryVariable* result;
  MemoryVariable* var1;
  MemoryMergeConstraint(MemoryVariable* result, MemoryVariable* var1): result(result), var1(var1) {}
  virtual void run() const {
    result->set(var1->get());
  }
  virtual void markDependencies() {addDependency(var1);}
...
struct RegisterSet {
  ...
  RegisterSet() {
    for (size_t i = 0; i < 8; ++i) gpr[i] = new XVariable<32>();
    for (size_t i = 0; i < 6; ++i) segreg[i] = new XVariable<16>();
    for (size_t i = 0; i < 16; ++i) flag[i] = new XVariable<1>();
    memoryWrites = new MemoryVariable();
  }

  void setToBottom() {
    for (size_t i = 0; i < 8; ++i) gpr[i] = bottom<32>();
    for (size_t i = 0; i < 6; ++i) segreg[i] = bottom<16>();
    for (size_t i = 0; i < 16; ++i) flag[i] = bottom<1>();
    memoryWrites->set(MemoryWriteSet::bottom());
  }

  void mergeIn(const RegisterSet& rs) {
    for (size_t i = 0; i < 8; ++i) (new MergeConstraint<32>(gpr[i], rs.gpr[i]))->activate();
    for (size_t i = 0; i < 6; ++i) (new MergeConstraint<16>(segreg[i], rs.segreg[i]))->activate();
    for (size_t i = 0; i < 16; ++i) (new MergeConstraint<1>(flag[i], rs.flag[i]))->activate();
    (new MemoryMergeConstraint(memoryWrites, rs.memoryWrites))->activate();
  }
};

ostream& operator<<(ostream& o, const RegisterSet& rs) {
  for (size_t i = 0; i < 8; ++i) o << gprToString((X86GeneralPurposeRegister)i) << " = " << rs.gpr[i] << endl;
  for (size_t i = 0; i < 6; ++i) o << segregToString((X86SegmentRegister)i) << " = " << rs.segreg[i] << endl;
  for (size_t i = 0; i < 16; ++i) o << flagToString((X86Flag)i) << " = " << rs.flag[i] << endl;
  o << "memory = ";
  if (rs.memoryWrites->get().isTop) {
    ...
    for (size_t i = 0; i < rs.memoryWrites->get().writes.size(); ++i) {
      o << "  " << rs.memoryWrites->get().writes[i].address << ":" << rs.memoryWrites->get().writes[i].len << " -> " << rs.memoryWrites->get().writes[i].data << "\n";
    }
    o << "}\n";
  }
  return o;
...
template <size_t Len, size_t Len2> // Only safe when MSBs don't matter (i.e., you can't extract bits from something and then use this to put in zeros -- the original bits will probably appear again)
UNARY_COMPUTATION_SPECIAL(extendByMSB, Len, Len2, {result->set(LatticeElement<Len2>(le1.name, le1.definingInstruction, le1.negate, le1.offset));})

template <size_t From, size_t To, size_t Len>
UNARY_COMPUTATION_SPECIAL(extract, Len, To - From,
                          {if (From == 0) {result->set(LatticeElement<To - From>(le1.name, le1.def, le1.negate, le1.offset)); return;}
                           if (le1.name != 0) {result->set(LatticeElement<To - From>::nonconstant(result->myName, result->def)); return;}
                           result->set(LatticeElement<To - From>::constant((le1.offset >> From) & (SHL1<uint64_t, To - From>::value - 1), result->def));})
...
struct FindConstantsPolicy {
  ...
  XVariablePtr<Len> number(uint64_t n) {
    struct NumberConstraint: public NullaryConstraint<Len> {
      uint64_t val;
      NumberConstraint(XVariablePtr<Len> var, uint64_t val): NullaryConstraint<Len>(var), val(val) {}
  ...
  BINARY_COMPUTATION(concat, Len1, Len2, Len1 + Len2, {return a | (b << Len1);})

  template <size_t From, size_t To, size_t Len>
  UNARY_COMPUTATION_SPECIAL(extract, Len, To - From,
                            {if (From == 0) {result->set(LatticeElement<To - From>(le1.name, le1.definingInstruction, le1.negate, le1.offset)); return;}
                             if (le1.name != 0) {result->set(LatticeElement<To - From>::nonconstant(result->myName, result->def)); return;}
                             result->set(LatticeElement<To - From>::constant((le1.offset >> From) & (SHL1<uint64_t, To - From>::value - 1), result->def));})

  XVariablePtr<1> false_() {return number<1>(0);}
  XVariablePtr<1> true_() {return number<1>(1);}
  XVariablePtr<1> undefined_() {return new XVariable<1>();}

  template <size_t Len>
  UNARY_COMPUTATION_SPECIAL(invert, Len, Len, {if (le1.name == 0) result->set(LatticeElement<Len>::constant(~le1.offset, result->def)); else result->set(LatticeElement<Len>(le1.name, le1.definingInstruction, !le1.negate, ~le1.offset));})

  template <size_t Len>
  UNARY_COMPUTATION_SPECIAL(negate, Len, Len, {if (le1.name == 0) result->set(LatticeElement<Len>::constant(-le1.offset, result->def)); else result->set(LatticeElement<Len>(le1.name, le1.definingInstruction, !le1.negate, -le1.offset));})
  ...
  BINARY_COMPUTATION_SPECIAL(xor_, Len, Len, Len, \
    {if (le1 == le2) {result->set(LatticeElement<Len>::constant(0, result->def)); return;}
     if (le1.name == 0 && le2.name == 0) {result->set(LatticeElement<Len>::constant(le1.offset ^ le2.offset, result->def)); return;}
  ...
  XVariablePtr<Len> ite(XVariablePtr<1> sel, XVariablePtr<Len> ifTrue, XVariablePtr<Len> ifFalse) {
    ...
    struct IteConstraint: public Constraint {
      ...
      XVariablePtr<1> sel;
      IteConstraint(XVariablePtr<Len> result, XVariablePtr<1> sel, XVariablePtr<Len> ifTrue, XVariablePtr<Len> ifFalse): result(result), ifTrue(ifTrue), ifFalse(ifFalse), sel(sel) {}
      virtual void run() const {
      ...
      virtual void markDependencies() {addDependency(sel); addDependency(ifTrue); addDependency(ifFalse);}
    };
  ...
  BINARY_COMPUTATION_SPECIAL(add, Len, Len, Len,
                             {if (le1.name == 0 || le2.name == 0) {result->set(LatticeElement<Len>(le1.name + le2.name, result->def, le1.negate || le2.negate, le1.offset + le2.offset)); return;}
                              if (le1.name == le2.name && le1.negate == !le2.negate) {result->set(LatticeElement<Len>::constant(le1.offset + le2.offset, result->def)); return;}
                              result->set(LatticeElement<Len>::nonconstant(result->myName, result->def));})

  template <size_t Len>
  TERNARY_COMPUTATION_SPECIAL(add3, Len, Len, 1, Len,
                             {if ((le1.name == 0) + (le2.name == 0) + (le3.name == 0)) {result->set(LatticeElement<Len>(le1.name + le2.name + le3.name, result->def, le1.negate || le2.negate || le3.negate, le1.offset + le2.offset + le3.offset)); return;}
                              if (le1.name == le2.name && le3.name == 0 && le1.negate == !le2.negate) {result->set(LatticeElement<Len>::constant(le1.offset + le2.offset + le3.offset, result->def)); return;}
                              result->set(LatticeElement<Len>::nonconstant(result->myName, result->def));})
  ...
  template <size_t Len>
  XVariablePtr<Len> addWithCarries(XVariablePtr<Len> a, XVariablePtr<Len> b, XVariablePtr<1> carryIn, XVariablePtr<Len>& carries) { // Full case
  ...
  BINARY_COMPUTATION(rotateLeft, Len, SALen, Len, {return IntegerOps::rotateLeft<Len>(a, b);})

  template <size_t Len, size_t SALen>
  BINARY_COMPUTATION(rotateRight, Len, SALen, Len, {return IntegerOps::rotateRight<Len>(a, b);})

  template <size_t Len, size_t SALen>
  BINARY_COMPUTATION(shiftLeft, Len, SALen, Len, {return IntegerOps::shiftLeft<Len>(a, b);})

  template <size_t Len, size_t SALen>
  BINARY_COMPUTATION(shiftRight, Len, SALen, Len, {return IntegerOps::shiftRightLogical<Len>(a, b);})

  template <size_t Len, size_t SALen>
  BINARY_COMPUTATION(shiftRightArithmetic, Len, SALen, Len, {return IntegerOps::shiftRightArithmetic<Len>(a, b);})

  template <size_t Len1, size_t Len2>
  BINARY_COMPUTATION(signedMultiply, Len1, Len2, Len1 + Len2, {return (IntegerOps::signExtend<Len1, 64>(a) * IntegerOps::signExtend<Len2, 64>(b));})

  template <size_t Len1, size_t Len2>
  BINARY_COMPUTATION(unsignedMultiply, Len1, Len2, Len1 + Len2, {return (a * b);})

  template <size_t Len1, size_t Len2>
  BINARY_COMPUTATION(signedDivide, Len1, Len2, Len1, {return (IntegerOps::signExtend<Len1, 64>(a) / IntegerOps::signExtend<Len2, 64>(b));})

  template <size_t Len1, size_t Len2>
  BINARY_COMPUTATION(signedModulo, Len1, Len2, Len2, {return (IntegerOps::signExtend<Len1, 64>(a) % IntegerOps::signExtend<Len2, 64>(b));})

  template <size_t Len1, size_t Len2>
  BINARY_COMPUTATION(unsignedDivide, Len1, Len2, Len1, {return (a / b);})

  template <size_t Len1, size_t Len2>
  BINARY_COMPUTATION(unsignedModulo, Len1, Len2, Len2, {return (a % b);})

  template <size_t Len>
  UNARY_COMPUTATION(leastSignificantSetBit, Len, Len, {for (int i = 0; i < (int)Len; ++i) {if (a & shl1<uint64_t>(i)) return i;} return 0;})

  template <size_t Len>
  UNARY_COMPUTATION(mostSignificantSetBit, Len, Len, {for (int i = (int)Len - 1; i >= 0; --i) {if (a & shl1<uint64_t>(i)) return i;} return 0;})

  XVariablePtr<32> filterIndirectJumpTarget(XVariablePtr<32> x) {return x;}
  XVariablePtr<32> filterCallTarget(XVariablePtr<32> x) {return x;}
  XVariablePtr<32> filterReturnTarget(XVariablePtr<32> x) {return x;}
  ...
  XVariablePtr<Len> readMemory(X86SegmentRegister segreg, XVariablePtr<32> addr, XVariablePtr<1> cond) {
    struct ReadMemoryConstraint: public Constraint {
      ...
      virtual void markDependencies() {addDependency(memory); addDependency(addr);}
    };
  ...
  XVariablePtr<64> rdtsc() {return number<64>(0);} // FIXME
...
int main(int argc, char** argv) {
  ...
  while (n && !isSgAsmInterpretation(n)) n = isSgAsmNode(n->get_parent());
  ROSE_ASSERT (n);
  ...
  vector<linux_syscall> syscalls = getSyscalls("/usr/src/kernels/2.6.9-78.0.1.EL-x86_64/include/linux/syscalls.h", "syscall_list");
  for (map<uint64_t, RegisterSet>::const_iterator i = policy.rsets.begin(); i != policy.rsets.end(); ++i) {
    ...
    cout << "Address 0x" << hex << i->first << dec << ": " << (policy.isInstructionExternallyVisible(insn) ? "EXTERN " : "") << unparseInstruction(insn) << endl;
    // cout << i->second << endl;
    if (insn->get_kind() == x86_int) {
      ...
      if (eax.isTop || eax.name != 0) { // Not a constant
        ...
        for (size_t param = 0; param < call.arguments.size(); ++param) {
          ...
          X86GeneralPurposeRegister paramRegs[] = {x86_gpr_bx, x86_gpr_cx, x86_gpr_dx, x86_gpr_si, x86_gpr_di, x86_gpr_bp};
          assert (param < 6);
