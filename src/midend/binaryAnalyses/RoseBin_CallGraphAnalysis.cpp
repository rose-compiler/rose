...
RoseBin_CallGraphAnalysis::findClusterOfNode(SgGraphNode* next_n,
  ...
  SgAsmFunction* func = isSgAsmFunction(next_n->get_SgNode());
  ROSE_ASSERT(func);
  std::map<SgAsmFunction*,int>::const_iterator clust = visited.find(func);
  if (clust!=visited.end()) {
    ...
    cerr << "    >>>> RoseBin_CallGraph: NOT YET PROCESSED  : ." <<
      RoseBin_support::HexToString(isSgAsmFunction(next_n->get_SgNode())->get_address()) << " " << next_n << endl;
  }
  ...
  while (!worklist.empty()) {
    ...
    cerr << "    >>>> RoseBin_CallGraph: successor size of  : ." <<
      RoseBin_support::HexToString(isSgAsmFunction(current->get_SgNode())->get_address()) <<
    ...
    for (;succ!=successors_f.end();++succ) {
      ...
      SgAsmFunction* func_next = isSgAsmFunction(next->get_SgNode());
      ROSE_ASSERT(func_next);
        ...
        if (it==visited_f.end()) {
          ...
          std::map<SgAsmFunction*,int>::const_iterator clust = visited.find(func_next);
          if (clust!=visited.end()) {
            // found it in clusters
            resultsCluster=clust->second;
    if (debug)
  ...
  for (;it!=curr_cluster.end();++it) {
    SgGraphNode* node = *it;
    SgAsmFunction* func_next = isSgAsmFunction(node->get_SgNode());
    ROSE_ASSERT(func_next);
...
// typedef rose_hash::unordered_map <std::string, SgGraphNode*> nodeType;
  ...
  // convert the visited map into the std::map<int,std::set<SgAsmFunction*> >
  std::map<SgAsmFunction*,int>::const_iterator it = visited.begin();
  for (;it!=visited.end();++it) {
    SgAsmFunction* func = it->first;
    int cluster = it->second;
    std::set<SgAsmFunction*> setFunc;
    std::map<int,std::set<SgAsmFunction*> >::const_iterator findClust = ret.find(cluster);
  ...
  // test
  std::map<int,std::set<SgAsmFunction*> >::const_iterator comps = ret.begin();
