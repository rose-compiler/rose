...
//		cout <<"\tuse interesting parent is of class:"<<interestingUseParent->class_name()<<"\n\t\t and is "<<interestingUseParent->unparseToString()<<endl;
...
//    if (DUVariableAnalysisExt::isUse(varRef) || DUVariableAnalysisExt::isIUse(varRef))
    {
        // cout <<"intersetingUseParent for "<< initName->get_name ().getString() << " is "<<interestingUseParent->unparseToString()<<" and is of type "<<interestingUseParent->class_name()<<endl;
      ...
      for (unsigned int j=0;j<defPlaces.size();j++)
      {
        ...
        // cout <<"intersetingDefParent for "<< initName->get_name ().getString() << " is "<<interestingDefParent->unparseToString()<<" and is of type "<<interestingDefParent->class_name()<<endl;
        // cout <<"next inreresting parent :" <<getNextParentInterstingNode(interestingDefParent->get_parent())->unparseToString()<< " and is of type "<<getNextParentInterstingNode(interestingDefParent->get_parent())->class_name()<<endl;
