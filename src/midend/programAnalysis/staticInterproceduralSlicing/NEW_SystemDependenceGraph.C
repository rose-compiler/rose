...
#ifdef VERBOSE_DEBUG
    cout << "now at declaration for: " << fDec->unparseToString() << "\n";
#endif
    if (fDec->get_definition() == NULL)
    {

#ifdef VERBOSE_DEBUG
    cout << "Did not find definition " << (void *)(fDec) << "\n" << endl;
#endif
      ipi=new InterproceduralInfo(fDec);
      
#ifdef VERBOSE_DEBUG
  cout << "create Safe Configurations\n" << endl;
#endif
...
std::vector<InterproceduralInfo*> SystemDependenceGraph::getPossibleFuncs(SgFunctionCallExp * funcCall)
{
  ...
  SgFunctionDeclaration *f_Decl = fsym->get_declaration(); //for testing
  SgFunctionDeclaration *fD = funcCall->getAssociatedFunctionDeclaration();
  SgFunctionDeclaration *fDDef = (SgFunctionDeclaration*)fD->get_definingDeclaration();

#ifdef VERBOSE_DEBUG
  cout << "f_Decl= " << f_Decl << " ,fD= "<<fD<<" ,fDDef="<< fDDef<< endl;
#endif

// uw-mist this will make sure that the edges points to the definition node and not the prototype
  if(fDDef != NULL) {
#ifdef VERBOSE_DEBUG
...
// uw-mist: this is wrong implementation
// pass1 should go over PARAMETER_IN instead of PARAMETER_OUT. 
// pass1 should go over CALL as well. 
// pass2 should go over PARAMETER_OUT instead of PARAMETER_IN. 
// pass2 should not go over CALL.
// 
 ...
 */
  ...
  int edgeTypes1 = CONTROL | DATA | SUMMARY | CALL | PARAMETER_IN;
  int edgeTypes2 = CONTROL | DATA | SUMMARY | PARAMETER_OUT;
  
  set < DependenceNode * >start;
  start.insert(node);
  
  set < DependenceNode * >phase1nodes = _getReachable(start, edgeTypes1);
  writeDot((char*) "sysbk.stage1.slice.dot");
  set < DependenceNode * >phase2nodes = _getReachable(phase1nodes, edgeTypes2);
  writeDot((char*) "sysbk.stage2.slice.dot");
  return phase2nodes;
