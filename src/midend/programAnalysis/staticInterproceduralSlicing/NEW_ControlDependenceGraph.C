...
void ControlDependenceGraph::computeAdditionalFunctioncallDepencencies()
{
  for (Rose_STL_Container< SgNode * >::iterator  i=functionCalls.begin();i!=functionCalls.end();i++)
  {
    ...
    establishEdge(getNode(call),getNode(DependenceNode::ACTUALRETURN,call),DependenceGraph::DATA);
    ...
    {
      // and establish an edge, since the dependencies for those nodes depend to the dependency of the parent node and parameters and subexpressions are not represented here... just belong
      ...
      // however it is necessary to maintain a syntactic correctness for the parameters
      ...
      establishEdge(getNode(call),getNode(DependenceNode::ACTUALIN,*j),DependenceGraph::CONTROL);
