...
// void ProgramDependenceGraph::_addCDG() {
...
// void ProgramDependenceGraph::_mergeDDG () {
...
void ProgramDependenceGraph::_summarize() {
  ...
  for (funcCalls = _interprocedural->callsite_map.begin(); 
       funcCalls != _interprocedural->callsite_map.end();
       funcCalls++) {
    ...
    for (i = callsite.actual_in.begin(); i != callsite.actual_in.end(); i++) {
      ...
      //for (j = callsite.actual_out.begin(); j != callsite.actual_out.end(); j++) {
...
set<DependenceNode *> ProgramDependenceGraph::getSlice(DependenceNode * node) {

  int edgeTypesToFollow = 
    CONTROL |
    DATA |
    SUMMARY |
    CALL |
