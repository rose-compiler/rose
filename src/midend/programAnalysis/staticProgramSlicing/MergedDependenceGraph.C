...
void MergedDependenceGraph::_mergeGraph(DependenceGraph * graph) {
  ...
  for (i = nodes.begin(); i != nodes.end(); i++) {
    ...
    for (j = succs.begin(); j != succs.end(); j++) {
      ...
      //go through the various edge types and "establish" that edge in the current graph
...
set<DependenceNode *> MergedDependenceGraph::_getReachable(set<DependenceNode *> start, int edgeTypesToFollow) {
  ...
  while (remaining.size() != 0) {
	...
	//if any of the edges are in edgeTypesToFollow, then we add this element to the list
	bool addToRemaining = false;
	for (set<EdgeType>::iterator j = eds.begin(); j != eds.end(); j++) {
	  EdgeType edge = *j;
	  if (edge & edgeTypesToFollow) {//it's a bit vector
	    addToRemaining = true;
	  }
	}
	if (addToRemaining) {
