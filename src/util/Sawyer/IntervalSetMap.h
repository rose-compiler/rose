...
public:
    ...
    bool containsAnywhere(const Interval &interval, const typename Set::Value &value) {
        BOOST_FOREACH (const typename Super::Node &node, this->findAll(interval)) {
    ...
    bool containsEverywhere(const Interval &interval, const typename Set::Value &value) {
        if (interval.isEmpty())
            return false;
        BOOST_FOREACH (const typename Super::Node &node, this->findAll(interval)) {
            if (!node.value().exists(value))
