...
\begin{itemize}
   ...
   \item {\bf Java} : \\
          A Java compiler and virtual machine are required to use the Open
          Fortran Parser and other Java-based parts of ROSE.
\end{itemize}
...
\begin{itemize}
   \item {\tt ../ROSE/configure --with-boost=[BOOST\_installTree] --with-java} \\
         ...
         --prefix=`pwd`}).  The Open Fortran Parser will also be enabled,
         allowing ROSE to process Fortran code.  The programs {\tt java},
         {\tt javac}, and {\tt jar} must be either in your PATH or in {\tt
         \$JAVA\_HOME/bin}.
   ...
   \item {\tt ../ROSE/configure --prefix=`pwd` --with-binarysql --with-boost=[BOOST\_installTree]} \\
         ...
         IDA Pro). 
   \item {\tt ../ROSE/configure --prefix=`pwd` --with-javaport=yes SWIG=swig --with-boost=[BOOST\_installTree] --with-java} \\
