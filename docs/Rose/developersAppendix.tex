...
on the compile farm see \url{http://http://nmi.cs.wisc.edu/}.\\
These tests can are run against the HEAD revision of the public svn repository. \\
...
For every test a build and a test run is started.

\subsection{Adding a test}
To add a test which can be run on the Compile Farm you need to add a options
file to the svn repository. This options file sets some bash variables
which control the conditions and name the test is run. The right place to put
those files is <rose\_dir>/scripts/nmi/build\_configs. \\
Overview of the options:
\begin{itemize}
 \item {\tt TITLE} - The title of the test
 \item {\tt DESCRIPTION} - Short text to describe the test
 \item {\tt PLATFORMS} - On which platforms this test will run. An overview of
                         the different platforms can be seen here: 
 ...
 \item {\tt PREREQS} - Define what software this run needs. The prereqs available
                       ...
                       \url{http://nmi-s005.cs.wisc.edu/nmi/index.php?page=pool/platform}
                       and clicking on the platforms you whish to use.
...
PLATFORMS="x86_64_deb_4.0, x86_64_deb_5.0, x86_64_fc_4, x86_64_fc_5, x86_64_fc_8, x86_64_fc_9, x86_64_rhap_5, x86_64_rhap_5.2, x86_64_rhas_3, x86_64_rhas_4, x86_64_sles_8, x86_64_sles_9, x86_cent_4.2, x86_deb_3.1, x86_deb_4.0, x86_deb_5.0, x86_fc_5, x86_rhap_5, x86_rhas_3, x86_rhas_4, x86_sl_4.4, x86_sles_8, x86_sles_9, x86_slf_3, x86_suse_10.0, x86_suse_10.2"
\end{verbatim}

\subsection{Manually submitting tests}
\begin{enumerate}
 \item Log on to nmi-s005.cs.wisc.edu
 \item Get the latest submission scripts:
 \begin{itemize}
  \item {\tt svn checkout https://outreach.scidac.gov/svn/rose/trunk/scripts/nmi rose\_nmi }
  \item OR: {\tt cd rose\_nmi; svn up }
 \end{itemize}
 \item See whats already there: 
 \begin{itemize}
  \item {\tt ls build\_configs}
  \item {\tt \$\{EDITOR\} build\_configs/<options\_file>}
 \end{itemize}
 \item After you have chosen the test you want to run execute:
       {\tt ./submit.sh build\_configs/<options\_file>}
       to submit test.
 \item Point your browser to: 
       \url{http://nmi-web.cs.wisc.edu/nmi/index.php?page=results%2Foverview&opt_project=rose+compiler}
       To open the results overview page. Your submitted test should be shown as "running"
\end{enumerate}

\subsection{Cron automated tests}
Jobs can be added to the cronjobs file in the directory <rose\_dir>/scripts/nmi
These cronjobs will be loaded every night into the crontab file on the submission
host (this is done by the first entry, which executes update.sh). In order to
add your own build tests, simply add a line there (see {\tt man 5 crontab} for
more information). Be sure to test your submission before adding it to the cronjobs.

{\bf NOTE:} If you want your cronjobs to be permanent, ad this to your local svn
...
Example entry:
\begin{verbatim}
# run the minimal_default test every other day at 1 am
* 1 */2 * * cd ${CWD}; ./submit.sh build_configs/minimal_default
\end{verbatim}
