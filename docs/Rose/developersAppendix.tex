...
one at SciDAC Outreach Center. These two repositories share histories so that
you can work closely with us as an external developing using our latest work. 
Some tips for usign them are gatehred in this section.


\subsection{Continous Integration in ROSE}

The ROSE projct uses a workflow that automates the central principles of
continous integration in order to make integrating the work from different
...
%%Insert picture on structure of developers work here

Each developer is encouraged to develop a feature or
fix a bug in isolation and submit the code so that it can be considered for
integration as soon as possible. When a developer submits a code for integation
it will be tested by our test suite and integrated into our master bracnh if
it passes all tests.

The testing of each developers work is done by a continous integration server
called Hudson. We provide developers with a git repository where a repository
where he can create any number of branches. Each branch should relate to a
feature that this developer is working on and be relatively short-lived. The
actual development is done on a develop copy of this repository and a developer
will push to a branch when his work is ready for integration. When a developer push
to the ROSE git repository his work will be automatically added to a queue of
jobs that will be tested by Hudson.

\subsection{The External Repository}

\subsection{The Internal Repository}

The ROSE internal repository keeps track of the latest work of each developer. Each
...
an continous integration workflow using git best practises. The master branch
...
a branch owned by another developer, but a group branche can be created that 
...
considered for integation with master. We currently only do automatic testing on
...
\item List content in the repository:
  \begin{itemize}
          \item list root info: \textit{git clone 
...
\item Using a branch:
   \begin{itemize}
          ...
          \item creating a branch in the central git: \textit{git push origin origin:refs/heads/branch-name} 
          \item start working on a branch: \textit{git checkout branch-name} 
...
The look for the names (e.g. "F77_only") in "sageSupport.C" in ROSE/src/frontend/SageIII
