...
% Some helpful notes from Robb
Other Ideas, Issues, etc.
\begin{enumerate}
   \item Class-based \\
...
\fixme{This might be too low level for the proposed API.}
\begin{enumerate}
...
\begin{enumerate}
   ...
   \begin{enumerate}
      ...
      \item Dwarf Debug Format \\
      This is automatically constructed as part of reading the binary executable if it is available.
...
\begin{enumerate}
   ...
   \begin{enumerate}
	...
	documented.
      ...
      \begin{enumerate}
         \item Ptrace: uses the Unix ptrace() system call to trace over the network a binary executing on a remote linux machine.
         \item Pin: Traces an executable running on the same machine as ROSE.
%        \item Ether: Traces an executable running in Windows XP on a Xen virtual machine on the same hardware as ROSE.
      \end{enumerate}
