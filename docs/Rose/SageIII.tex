...
\section { Copying SAGE Nodes and Subtrees}
...
\subsection{ How Templates effects the IR }
...
% and setup the required template instantation behavior (member data such as doNotInstantiate, etc.).
...
% instatiate all speciaizations into a separate file (a {\em template specialization file}
...
these as well.  The general philosphy is to put as much information into the IR as possible
...
\subsection{Problems with the EDG/ROSE Template Instantiation Approach}
    ...
    First lets memtion that EDG does a great job and that the problems that
we have within template instantiation in ROSE are a result of trying to
convert template instatiations in EDG to be formal template specializations 
in ROSE.  This approach presents a better user interface and is substantially 
different to what EDG provides.

    For member functions EDG adds to the EDG AST the instatiations of the member functions
that are required at the end of the file.  Note that it does not declare the functions
before hand except in the instantiated class (which ROSE supresses the output of so that
...
specialized template of the templated clas (which would also have the prototype
...
the templated class has been transformed and ouly then output the class (which would then
be redundant with the forward declaration of its member function!!!

  If I output all instatiations then I don't need to build forward declarations!
if I surpress class declarations then I need to output forward declarations of the member
functions that are used (so that we can output the intantiated member functions at the
end of the file).

We need at least a couple of template instatiation modes:
\begin{enumerate}
   \item Transformations on templates \\
...
// variable declaration of type X<int> which is an instatiation
X<int> x;
\end{verbatim}
}}
...
\subsection{Summary of Template Instatiation Steps}
...
\end{verbatim}
     ...
     The following code is normalized differently (and somewhat inconsistantly):
{\indent
{\mySmallFontSize
...
\end{verbatim}
     ...
     The reasons this is done is because this is that EDG stores the packing allignment
     values directly with the data structure and does not represent the pragma explictly.
     ...
     {\em \#pragma pack()} is alos modified so that it appears immediately before and
     after the opening and closing parens for the class or structure definition.
...
\commentout{
    ...
    heder files (though I would like to avoid doing so to make our work as platform
    independent as possible).  But copying and modifiying the target compiler's header
...
\section{Comments About Declarations (Defining Declarations vs. Nondefining Declartions) }
...
\commentout{
   IR nodes that contain a {\tt get\_mangled\_qualified\_name()} member fucntion are:
\begin{itemize}
     \item SgClassDeclaration
     \item SgClassDefinition
\end{itemize}
...
a unique string to represent a function he/she should form that from a concatination of
