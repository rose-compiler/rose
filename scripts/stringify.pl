...
This is very, very slow on recent versions of Perl. For instance, perl 5.10.1 takes about 30 seconds while 5.20.2 takes
over two hours on the same hardware.
...
sub output_defn {
  ...
  for my $name (sort {$a cmp $b} keys %enum) {
    my $type = $generic_type || $name;
    my $numeric_suffix = $generic_type ? "l" : "";
    my $script_name = '$ROSE_SOURCE/scripts/stringify.pl';
    print OUTPUT <<"EOF";

// DO NOT EDIT -- This function was automatically generated by $script_name.
...
EOF
    for my $v (sort {$a <=> $b} keys %{$enum{$name}}) {
      print OUTPUT "        case ${v}${numeric_suffix}: retval = \"$enum{$name}{$v}\"; break;\n";
    }
print OUTPUT <<"EOF"
    }
    if (retval.empty()) {
...
# Parse command-line
my($do_decls,$generic_type,$defn_output,$decl_output);
while ($ARGV[0] =~ /^-/) {
  if ($ARGV[0] eq '--') {
    shift;
    last;
...
while (my $filename = $files->next_file) {
  ...
  # Skip files that take too long and don't have any useful enums
  next if $filename =~ /\/frontend\/SageIII\/.*\.C$/;
...
# Emit output
output_decl($generic_type, $decl_output) if $do_decls;
output_defn($generic_type, $defn_output) unless $do_decls && !$defn_output;
