...
git pull
# only bootstrap source tree if needed
test -f configure
if [ $? -ne 0 ]; then
  ./build
fi
...
#rm -rf ROSE-build
test -d ROSE-build || mkdir ROSE-build
cd ROSE-build
...
# it will internally check if an existing binary already exists in the /usr/casc/overture/ROSE/git/ROSE_EDG_Binaries/
# before doing the building
make guarded_binary_edg_tarball -j${PROCESS_NUM}
#make source_with_binary_edg_dist DOT_SVNREV=-12345
# copy it to a centralized file location
test -f roseBinaryEDG-*.tar.gz && \
cp roseBinaryEDG-*.tar.gz /usr/casc/overture/ROSE/git/ROSE_EDG_Binaries/. || \
echo "binary file already exists, skipped the actual building."
