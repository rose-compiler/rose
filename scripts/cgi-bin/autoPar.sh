...
# TODO: move common code into some shared include files later
# set environment variables needed to run the translators
ROSE_INSTALL_PATH=/home/ubuntu/opt/rose_inst/bin

roseTranslator="$ROSE_INSTALL_PATH/autoPar"
...
if [ $? -eq 0 ]
then
  echo "Execution of the tool is successful!"
  echo "<br />"
  ...
  echo "Execution of the tool failed! <br />"
fi 

echo "Execution message (empty if no standard output, warnings or errors): <br />"
...
#echo "<p>"
#echo "Version of the g++ used is:<br />"
#g++ --version &>/tmp/gcc$mypid.version
#display_code "/tmp/gcc$mypid.version"
