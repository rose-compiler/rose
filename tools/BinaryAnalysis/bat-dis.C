...
public:
    ...
    void emitBasicBlockBody(std::ostream &out, const P2::BasicBlockPtr &bb, BinaryAnalysis::Unparser::State &state) const override {
        ASSERT_not_null(bb);
        InstructionSemantics::BaseSemantics::RiscOperatorsPtr ops = state.partitioner().newOperators();
        if (InstructionSemantics::BaseSemantics::DispatcherPtr cpu = state.partitioner().newDispatcher(ops)) {
