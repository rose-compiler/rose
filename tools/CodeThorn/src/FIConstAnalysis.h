...
namespace CodeThorn {
  ...
  public:
    ...
    std::string toString() const {
      ...
      if(isTop())
        return "top";
      return std::string("[")+_min.toString()+","+_max.toString()+"]";
    }
  ...
  typedef std::map<VariableId, std::set<AbstractValue> > VarConstSetMap;
  ...
  public:
    ...
    static AbstractValue isConstInSet(AbstractValue val, std::set<AbstractValue> valSet);
  private:
  ...
  public:
    ...
    std::string toString(VariableIdMapping& varIdMapping) {
      std::string varNameString=varIdMapping.uniqueVariableName(varId);
      std::string varValueString=varValue.toString();
  ...
  private:
    ...
    bool detailedOutput;
    std::set<VariableId> variablesOfInterest;
