...
namespace CodeThorn {
  ...
  void TransitionGraph::reduceEStates(std::set<EStatePtr> toReduce) {
    for(std::set<EStatePtr>::const_iterator i=toReduce.begin();i!=toReduce.end();++i) { 
      reduceEState(*i);
    }
  }

  void TransitionGraph::reduceEStates3(std::function<bool(EStatePtr)> predicate) {
