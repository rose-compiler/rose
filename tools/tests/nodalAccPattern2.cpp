...
// A negative case:  not a raja function call 
namespace fake
{
  typedef int Index_type;
  // new specialized policy
...
void foo_fake(double* x, int jp, int kp, RAJA::Index_type begin, RAJA::Index_type end, double rh1)
{
   double * x1, *x2, *x3, *x4; 
   ...
   x3 = x1 + jp; 
   x4 = x1 + kp; 
// This should not be matched since it does not call RAJA::forall
   fake::forall <class fake::seq_exec> (begin, end, [=](int i)
   {
      x1[i] += rh1; 
      x2[i] -= rh1; 
      x3[i] *= rh1; 
