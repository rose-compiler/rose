...
namespace f2cxx
{
  ...
  struct DotPrinter
  {
    ...
    void write_type(SgExpression& n, attr_set opt = ::f2cxx::types)
    {
      if (!with_opt(opt)) 
        return;
    ...
    void expr_node(SgExpression& n, attr_set opt = ::f2cxx::types)
    {
      node(*os, &n, short_name(n), expr_color);
      edge(*os, n.get_parent(), &n, "", "");
      write_type(n, opt);
      recurse(n);
    }
    ...
    void handle(SgInitializedName& n) 
    {
      ...
      normal_node(n, n.get_name(), decl_color);
      
      if (!with_opt(::f2cxx::types)) 
        return;
    ...
    void handle(SgExprListExp& n)    { expr_node(n, ::f2cxx::moretypes); }    
    void handle(SgNullExpression& n) { expr_node(n, ::f2cxx::moretypes); }    
        
    void handle(SgVarRefExp& n) 
    {
      expr_node(n);
      
      if (!with_opt(::f2cxx::decllinks)) 
        return;
