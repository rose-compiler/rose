...
int main(int argc, char* argv[])
{
  ...
  try{
    ...
    {
      Element cur_elem;
      int cluster     = boost::lexical_cast<int>(datasets.getstring(0));
...
//      cur_elem.offset      = boost::lexical_cast<int> ( datasets.getstring(6) );
  ...
  sort( listOfClonePairs.begin(), listOfClonePairs.end(), sort_list() );

  //Perform clone merging

  std::vector<std::pair <Element, Element> >::iterator iItr = listOfClonePairs.begin();
  assert(iItr != listOfClonePairs.end());
  std::pair< Element, Element > currentMerging = *iItr;
  ++iItr;

  std::vector<std::pair <Element, Element> > returnClusters;
  bool first = true;

  try{
    sqlite3_transaction trans2(con);

    for( ; iItr != listOfClonePairs.end(); ++iItr )
    {
      const std::pair< Element, Element >& currentElem = *iItr;

#if 0
      std::cout << currentElem.first.function_id << ":" << currentElem.first.index_within_function << " " << 
        currentElem.second.function_id<< ":" << currentElem.second.index_within_function << std::endl;
      // std::cout << currentMerging.first.last_index_within_function << " --- " << currentMerging.second.last_index_within_function << '\n';
#endif
      if( overlaps(currentMerging, currentElem, windowSize, stride) == true )
      {
#if 0
        if (currentElem.first.last_index_within_function - currentMerging.first.last_index_within_function !=
            currentElem.second.last_index_within_function - currentMerging.second.last_index_within_function) {
          std::cout << "Different stride\n";
        }
#endif
        currentMerging.first.last_index_within_function = currentElem.first.last_index_within_function;
        currentMerging.second.last_index_within_function = currentElem.second.last_index_within_function;

        // std::cout << "toss \n";
        // delete currentElem;

      }else{
        //returnClusters.push_back(currentMerging);
        addCloneToDatabase(con, currentMerging);
        currentMerging = currentElem;
        // std::cout << "keep \n";
  ...
  std::cout << "After  size: " << returnClusters.size() << std::endl;
  
  gettimeofday(&after, NULL);
  getrusage(RUSAGE_SELF, &ru_after);

  insert_timing(con, "findingLargestClones",before,after, ru_before, ru_after );
