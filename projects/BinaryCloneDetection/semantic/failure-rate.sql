...
create table fr_clone_pairs as
    select func1_id, func2_id
        from semantic_funcsim where ((euclidean_d < 1.4) OR (euclidean_d <= 2.0 AND euclidean_d >= 1.4 AND euclidean_d_ratio < 1.5)  ) AND similarity >= (select similarity_threshold from fr_settings) ;
