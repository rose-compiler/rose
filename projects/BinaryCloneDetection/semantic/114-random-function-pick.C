...
usage(int exit_status)
{
    std::cerr <<"usage: " <<argv0 <<" [SWITCHES] [--] DATABASE SPECIMEN\n"
              <<"  This command takes data from a results database and compute rates (false positives, false negatives) produces graphs and tables. \n"
              <<"\n"
...
void 
create_tables_and_indexes(std::string regex_of_targets, int ninsns, double path_threshold, double cg_threshold, double sem_threshold)
{
 ...
 transaction->execute("create table tmp_random_function_stats(func_id integer, total_matches integer, positives integer, negatives integer);");
}
...
evaluate_random_function(int random_fid)
{
  ...
  SqlDatabase::StatementPtr insert_stmt = transaction->statement("insert into tmp_random_function_stats( func_id, total_matches, positives, negatives ) values(?,?,?,?)");
