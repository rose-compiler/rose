...
ScopExtractor::inspectBottomUpFunctionBody(SgProject* project, SgNode* root)
{
  if (polyoptions.getPoCC_Past2Past())
    {
      std::cout << "[PolyOpt][INFO] Using PoCC PAST SCoP extractor" << std::endl;
      SageToPast conv(polyoptions);
      _pastRoots.push_back (conv.toPast (root));
    }
  else
    ScopExtractor::inspectBottomUpFunctionBody_base(project, root);
}
 ...
 */
void
ScopExtractor::inspectBottomUpFunctionBody_base(SgProject* project, SgNode* root)
{
  ...
  // 1-bis: if there are #pragma scop/endscop in the function, perform scop
  // detection only for these regions.
  std::vector<SgNode*> pragmaNodes =
    NodeQuery::querySubTree(root, V_SgPragmaDeclaration);

  std::vector<SgNode*> root_process = std::vector<SgNode*>();
  for (i = pragmaNodes.begin(); i != pragmaNodes.end(); ++i)
    {
      SgPragmaDeclaration* pragmaStmt = isSgPragmaDeclaration(*i);
      std::string pragmaString(pragmaStmt->get_pragma()->get_pragma());
      if((pragmaString.size() != 4) || (pragmaString.compare(0,4,"scop") != 0)) continue;
      SgStatement* stmt = getNextStatement(pragmaStmt);
      root_process.push_back (stmt);
      std::cout << stmt->unparseToCompleteString() << std::endl;
    }
  if (root_process.size() == 0 && !polyoptions.getScWithPragmaOnly())
    root_process.push_back (root);
  for (int pos = 0; pos < root_process.size();  ++pos)
    {
 ...
 *
 */
std::vector<s_past_node_t*>&
ScopExtractor:: getPastRoots() {
  return _pastRoots;
}
