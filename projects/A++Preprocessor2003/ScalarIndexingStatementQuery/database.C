...
#if 0
OperandDataBaseType::OperandDataBaseType( const string & name, 
...
int
OperandDataBaseType::getDimension() const
   {
     ROSE_ASSERT (dimension > -1);

  // error checking
     for (vector<ArrayOperandDataBase>::iterator i = arrayOperandList.begin(); i != arrayOperandList.end(); i++)
        {
...
void
OperandDataBaseType::setDimension( int dim )
   {
...
int
OperandDataBaseType::size() const
   {
...
void
OperandDataBaseType::
addIndexInformation ( const string & variableName, const string & indexName )
   {
#if 1
...
void
OperandDataBaseType::
addIntegerExpressionIndexInformation ( const string & variableName, const string & indexName )
   {
#if 1
...
ArrayTransformationSupport::IndexingAccessEnumType
OperandDataBaseType::globalIndexingProperties() const
   {
  // Compute the global properties of the indexing in the current statement
  // return ArrayTransformationSupport::UniformSizeUniformStride;

  // return the most general case for initial testing
  // return ArrayTransformationSupport::VariableSizeVariableStride;
  // return ArrayTransformationSupport::interpretTransformationOptions(transformationOptions);
     return transformationOption;
   }

void
OperandDataBaseType::setUserOptimizationAssertions( const list<int> & inputTransformationOptions )
   {
  // Allow user to specify optimization assertions
  // transformationOptions = inputTransformationOptions;
     transformationOption =
          ArrayTransformationSupport::interpretTransformationOptions(inputTransformationOptions);
   }
