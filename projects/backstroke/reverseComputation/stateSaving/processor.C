...
SgExpression* EventReverser::instrumentUnaryOp(SgUnaryOp* unary_op)
{
    ...
    SgExpression* fwd_operand_exp = instrumentExpression(fwd_exp->get_operand());
    fwd_exp->set_operand(fwd_operand_exp);

    return fwd_exp;
}

SgExpression* EventReverser::reverseUnaryOp(SgUnaryOp* unary_op)
{
    ...
    if (SgExpression* model_var = isStateVar(unary_op->get_operand()))
    {
        ...
        if (SgPlusPlusOp* pp_op = isSgPlusPlusOp(unary_op))
        {
            ...
            return buildMinusMinusOp(copyExpression(model_var), pp_mode);
        }
        ...
        if (SgMinusMinusOp* mm_op = isSgMinusMinusOp(unary_op))
        {
            ...
            return buildPlusPlusOp(copyExpression(model_var), mm_mode);
        }
    }

    // By default, return the reverser expression of its operand.
    return reverseExpression(unary_op->get_operand());
}

SgExpression* EventReverser::instrumentBinaryOp(SgBinaryOp* bin_op)
{
    ...
    SgExpression* fwd_lhs_exp = instrumentExpression(bin_op->get_lhs_operand());
    SgExpression* fwd_rhs_exp = instrumentExpression(bin_op->get_rhs_operand());
    ...
    if (SgExpression* model_var = isStateVar(bin_op->get_lhs_operand()))
    {
                ...
                isSgSizeOfOp(rhs_operand))
        {
            if (isSgPlusAssignOp(bin_op) ||
                    ...
                    (isSgMultAssignOp(bin_op) && !isZero(isSgValueExp(rhs_operand))) ||
        ...
        {
            return buildBinaryExpression<SgCommaOpExp>(
