                 ...
                 // All other relevant declarations (defining and other nondefining declarations) should share this type node
                    ...
                    if (cls_type)
                    {
                      ...
                      if (cls_decl->get_firstNondefiningDeclaration()!= NULL)
						...
						printf("    cls_type: %s, SgClassDecl: %s\n", cls_type->get_mangled().getString().c_str(), cls_decl->get_mangled_name().getString().c_str());
                          ROSE_ASSERT (false);
                        }
                    }  
                    ...
                    case V_SgInitializedName:
                       {
                         ...
                         if (initializedName != NULL && variableReferenceExpression != NULL && variableReferenceExpression->get_symbol()->get_declaration() == node)
                           ...
                           // Note that __PRETTY_FUNCTION__ is a compiler generated const char* (C string).
                            ...
                            {
			      printf("initname: %s = %s\n", initializedName->sage_class_name(), initializedName->unparseToString().c_str());
     ...
     if (expression != NULL)
        {
               ...
               switch (expression->variantT())
                 ...
                 // IR nodes that have an l-value
                    ...
                    case V_SgXorAssignOp:
                      ...
                      // This used to be violated by the ROSE/tests/nonsmoke/functional/roseTests/astInliningTests/pass16.C test code!
                      // This is now violated by the ./preTest /home/dquinlan/ROSE/NEW_ROSE/tests/nonsmoke/functional/roseTests/programTransformationTests/pass1.C test code!
                      ...
                      // It now passes the inliner tests, but fails in the tests/nonsmoke/functional/roseTests/programTransformationTests, so I
		 ...
		 // Rajesh 11/4/2011 AddressOfOp check
		    //case V_SgAddressOfOp:
                    case V_SgMinusMinusOp:
                    case V_SgPlusPlusOp:
                       {
			...
			printf("uop: %s = %s\n", unaryOperator->sage_class_name(), unaryOperator->unparseToString().c_str());
			printf("oper: %s = %s\n", operand->sage_class_name(), operand->unparseToString().c_str());
