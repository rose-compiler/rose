 ...
 *	01/10/2007	klausk@br.ibm.com	Minor fix - added a special
 *						case when file gid == gid_other
 *	01/10/2007	klausk@br.ibm.com	Fixed. Added setgroups() to
 *						drop supplementary groups
 *      01/04/2007	klausk@br.ibm.com	Updated nobody's uid/gid to 
 *						match RHEL5 ones (99/99)
 *	08/18/2006	camiloyc@br.ibm.com	Added IPC Message Queue tests
 *	08/16/2006	camiloyc@br.ibm.com	Added IPC Semaphores Set tests
 *	08/14/2006	camiloyc@br.ibm.com	Added IPC Shared Memory tests
 *	08/11/2006	camiloyc@br.ibm.com	Added POSIX Message Queue tests
 *	11/18/2004	krisw@us.ibm.com	If access is expected but denied,
 ...
 *						(A1).
 *	10/10/2004	danjones@us.ibm.com	originated by Dan Jones.
 ******************************************************************************/
#include <sys/types.h>
#include <sys/dir.h>
#include <sys/param.h>
...
void testaccess_ipc(int ipc_id,
		    char opt,
		    int mode,
		    int expected,
		    char *outbuf)
{
  ...
  int myerror = 0;
	...
	case 'q':
		...
		if (mode == O_WRONLY) {
			actual = msgsnd(ipc_id, &s_message,
					strlen(s_message.mtext), 0);
		} else {
...
int testall_ipc(int ipc_id, char opt, uid_t uid, gid_t gid)
{
	...
	setids(0,0);
	printf("\nIPC ID: %d - %s\n", ipc_id,
		((opt=='s')?"Semaphore Sets":(opt=='m')?"Shared Memory Segment":
		"Message Queues"));
	...
	if (test == 1) {
		...
		case 'm':
			if ((rc = shmctl (ipc_id, IPC_SET, &shbuf)) == -1) {
				printf("Error setting stat of Shared Memory: "
					"%d\n", errno);
				return(rc);
			}
			break;
		...
		case 's':
			if ((rc = semctl (ipc_id, 0, IPC_SET, &sembuf)) == -1) {
				printf("Error setting stat of Semaphores: "
					"%d\n", errno);
				return(rc);
			}
			break;
		...
		case 'q':
			if ((rc = msgctl (ipc_id, IPC_SET, &msqbuf)) == -1) {
				printf("Error setting stat of Message Queue: "
					"%d\n", errno);
				return(rc);
			}
			break;
...
void check_access (char *pathname, uid_t uid, gid_t gid)
{
	 ...
	 * checking all files.
	 */
	if ((count = scandir(pathname, &entries, file_select,
			     alphasort)) == -1) {
