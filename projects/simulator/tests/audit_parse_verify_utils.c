 ...
 * Return : Nothing
 */
void initialize_rec_part(rec_part *rec, int type)
{
	rec->next_part = NULL;
 ...
 */
int search_for_record(main_list *actual, time_t start, main_list *expected, time_t end)
{
 ...
 */
main_list* append_list (main_list *head, main_list *new)
{
        main_list *ptr;

        if (head == NULL) {     /* first record in list */
                head = new;
        } else {
 ...
 *
 */
rec_part* append_record (main_list **list, rec_part *new)
{
	rec_part *head=(*list)->myrecord; /* pointer to head of inner list */
        rec_part *ptr;

        if (head == NULL) {     /* first part of record */
		(*list)->myrecord = new;
        } else {
 ...
 */
main_list* search_for_serial(main_list* head, unsigned long audit_serial)
{
        main_list* ptr = head;
        if (head == NULL)
 ...
 */
void cleanup_mem(main_list **head)
{
