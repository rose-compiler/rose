...
int main() {
    ...
    // dot exp check
    struct node dot_head;
    no_init_ptr = fn2((void*)dot_head.next, ptr, (float*)dot_head.prev, (double*)dot_head.array1, (char*)NULL);

    // arrow exp check
    struct node* head = (struct node*)malloc(sizeof(struct node));
    no_init_ptr = fn2((void*)head->next, ptr, (float*)head->prev, (double*)head->array1, (char*)NULL);
    #endif
