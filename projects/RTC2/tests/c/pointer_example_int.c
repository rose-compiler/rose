...
int main() {
    ...
    *ptr2 = 9;
    ...
    for(int* new_ptr = start_ptr; new_ptr < start_ptr + PTR_SIZE; new_ptr++) {
        *new_ptr = 5;
        printf("%u\n", *new_ptr);
    }
    ...
    do {
        int* doptr = start_ptr;
        for(int* forptr2 = (int*)malloc(PTR_SIZE*sizeof(int)), *doptr2 = forptr2; doptr2 < (forptr2 + PTR_SIZE) ; doptr2++) {
        }
