...
static int llc(int argc, char **argv) {
//
// TODO: REMOVE THIS!
/*  
printf("Hello World argc = %d;", argc);
...
//
//
int llc(std::vector<std::string> &llc_options, char *bc_filename) {
    char **args = new char*[2 + llc_options.size()];
    args[0] = "llc";
    for (int i = 0; i < llc_options.size(); i++) {
      args[i + 1] = (char *) llc_options[i].c_str();
    }
    args[llc_options.size() + 1] = bc_filename;

    int return_code = 0;
	   
    try {
       int ret = llc(3, args); // system(("llc -filetype=obj " + bc_name).c_str());
    }
    catch(...) {
    }
