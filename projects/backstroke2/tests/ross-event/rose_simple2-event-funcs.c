       ...
       and send it*/
{
      ...
      (*rts.avpush(&(msg -> npart_send))) = nsend;
      tw_event_send(evt);
    }
/* Update local particle count and number of particle sin transit */
{
// model-code explicitly modifying state
...
{
// MS: using a ROSS feature for storing information to be used in the reverse code (here it is an int).
...
{
    (*rts.avpush(&(s -> npart))) += ( *((unsigned int *)bf));
    (*rts.avpush(&(s -> transit))) -= ( *((unsigned int *)bf));
  }
