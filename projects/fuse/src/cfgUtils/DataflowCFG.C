      ...
      for (size_t i = 0; i < currentPaths.size(); ++i) { // check each of the current paths
        ...
        if (!filter((currentPaths[i].*otherSide)())) {
          ...
          for (unsigned int j = 0; j < currentPaths2.size(); ++j) {
            dbg << "makeClosureDF: i="<<i<<" =&gt; j="<<j<<": "<<CFGEdge2Str(currentPaths2[j])<<endl;
            dbg << "isSgAndOp(currentPaths2[j].source().getNode())="<<isSgAndOp(currentPaths2[j].source().getNode())<<", "<<
            ...
            if(((isSgAndOp(currentPaths2[j].source().getNode()) && isSgAndOp(currentPaths2[j].target().getNode())) ||
                (isSgOrOp(currentPaths2[j].source().getNode())  && isSgOrOp(currentPaths2[j].target().getNode()))) &&
               currentPaths2[j].source().getIndex()==1 && currentPaths2[j].target().getIndex()==2)
            { continue; }
