...
#define DEBUG_DSL_CHILDREN 1
            ...
            // printf ("   --- return_synthesizedAttribute.dslChildren.size() = %zu \n",return_synthesizedAttribute.dslChildren.size());
               ...
               if (stencilAstAttribute != NULL)
                 ...
                 // DQ: This can have either an initializer (one child), or none.
                 // The case of none is a simple variable declaration using the default constructor.
                 // ROSE_ASSERT(dslAstAttribute->dslChildren.size() == 1);
                    if (dslAstAttribute->dslChildren.size() == 1)
                      {
                         SgNode* childNode = dslAstAttribute->dslChildren[0];
                         ROSE_ASSERT(childNode != NULL);
                      ...
                      {
                        ROSE_ASSERT(dslAstAttribute->dslChildren.size() == 0);
                      }
#if 0
                    printf ("Exiting as a test! \n");
                    ROSE_ASSERT(false);
                    ...
                    if (binaryOperator != NULL)
                       {
                         if (valueExp == NULL)
                            {
                           // Find the constant value for the expression associated with this attribute.
#if 1
                              printf ("Make up a constant value 42 until we have better support in the DSL specification \n");
#endif
                           // valueExp = buildIntVal(42);
                              valueExp = SageBuilder::buildIntVal(42);
                            }
                         ROSE_ASSERT(valueExp != NULL);

                      // value_Attribute->value = value;
                         constantExpressionAstAttribute->constantExpression = valueExp;
                       }
                    ...
                    for (size_t k = 0; k < stencil.m_coef.size(); k++)
                       {
                         ...
                         cout << " ; " ;
#endif
                         printf ("add stencil element: k = %zu \n",k);
