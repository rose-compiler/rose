/* 
 * This utility configures a NPB to be built for a specific classc. 
 ...
 * the number of nodes and classc for which a benchmark is being built. 

 * The utility takes 3 arguments: 
 *       setparams benchmark-name classc
 *    benchmark-name is "sp", "bt", etc
 *    classc is the size of the benchmark
 ...
 * 
 * This program makes use of two extra benchmark "classces"
 * classc "X" means an invalid specification. It is returned if
 * there is an error parsing the config file. 
 * classc "U" is an external specification meaning "unknown class"
 * 
...
void check_info(int type, char classc);
void read_info(int type, char *classp);
void write_info(int type, char classc);
void write_sp_info(FILE *fp, char classc);
void write_bt_info(FILE *fp, char classc);
void write_lu_info(FILE *fp, char classc);
void write_mg_info(FILE *fp, char classc);
void write_cg_info(FILE *fp, char classc);
void write_ft_info(FILE *fp, char classc);
void write_ep_info(FILE *fp, char classc);
void write_is_info(FILE *fp, char classc);
...
{
  int type;
  char classc, class_old;
  
  if (argc != 3) {
    printf("Usage: %s benchmark-name classc\n", argv[0]);
    exit(1);
  }

  /* Get command line arguments. Make sure they're ok. */
  get_info(argv, &type, &classc);
  if (classc != 'U') {
#ifdef VERBOSE
    printf("setparams: For benchmark %s: classc = %c\n", 
	   argv[1], classc); 
#endif
    check_info(type, classc);
  }
  ...
  if (classc != 'U') {
    if (class_old != 'X') {
#ifdef VERBOSE
      printf("setparams:     old settings: classc = %c\n", 
	     class_old); 
#endif
    }
  } else {
  ...
  * For example, to build a classc A benchmark, type                   *\n\
...
void check_info(int type, char classc) 
{
  ...
  if (classc != 'S' && 
      ...
      classc != 'W' && 
      classc != 'C') {
    printf("setparams: Unknown benchmark classc %c\n", classc); 
    printf("setparams: Allowed classces are \"S\", \"A\", \"B\" and \"C\"\n");
    exit(1);
  }
...
*/\n");
          ...
          break;
      case IS:
          fprintf(fp, DEF_CLASS_LINE, classc);
          fprintf(fp, "\
/*\n\
  ...
  switch(type) {
  case SP:
    write_sp_info(fp, classc);
    break;	      
  case BT:	      
    write_bt_info(fp, classc);
    break;	      
  case LU:	      
    write_lu_info(fp, classc);
    break;	      
  case MG:	      
    write_mg_info(fp, classc);
    break;	      
  case IS:	      
    write_is_info(fp, classc);  
    break;	      
  case FT:	      
    write_ft_info(fp, classc);
    break;	      
  case EP:	      
    write_ep_info(fp, classc);
    break;	      
  case CG:	      
    write_cg_info(fp, classc);
    break;
  default:
    printf("setparams: (Internal error): Unknown benchmark type %d\n", type);
    exit(1);
...
void write_sp_info(FILE *fp, char classc) 
{
  ...
  char *dt;
  if      (classc == 'S') { problem_size = 12;  dt = "0.015";   niter = 100; }
  else if (classc == 'W') { problem_size = 36;  dt = "0.0015";  niter = 400; }
  else if (classc == 'A') { problem_size = 64;  dt = "0.0015";  niter = 400; }
  else if (classc == 'B') { problem_size = 102; dt = "0.001";   niter = 400; }
  else if (classc == 'C') { problem_size = 162; dt = "0.00067"; niter = 400; }
  else {
    printf("setparams: Internal error: invalid classc %c\n", classc);
    exit(1);
  }
...
void write_bt_info(FILE *fp, char classc) 
{
  int problem_size, niter;
  char *dt;
  if      (classc == 'S') { problem_size = 12;  dt = "0.010";   niter = 60; }
  else if (classc == 'W') { problem_size = 24;  dt = "0.0008";  niter = 200; }
  else if (classc == 'A') { problem_size = 64;  dt = "0.0008";  niter = 200; }
  else if (classc == 'B') { problem_size = 102; dt = "0.0003";  niter = 200; }
  else if (classc == 'C') { problem_size = 162; dt = "0.0001";  niter = 200; }
  else {
    printf("setparams: Internal error: invalid classc %c\n", classc);
    exit(1);
  }
...
void write_lu_info(FILE *fp, char classc) 
{
  ...
  if      (classc == 'S') { problem_size = 12;  dt_default = "0.5"; itmax = 50; }
  else if (classc == 'W') { problem_size = 33;  dt_default = "1.5e-3"; itmax = 300; }
  else if (classc == 'A') { problem_size = 64;  dt_default = "2.0"; itmax = 250; }
  else if (classc == 'B') { problem_size = 102; dt_default = "2.0"; itmax = 250; }
  else if (classc == 'C') { problem_size = 162; dt_default = "2.0"; itmax = 250; }
  else {
    printf("setparams: Internal error: invalid classc %c\n", classc);
    exit(1);
  }
...
void write_mg_info(FILE *fp, char classc) 
{
  ...
  if      (classc == 'S') { problem_size = 32; nit = 4; }
  else if (classc == 'W') { problem_size = 64; nit = 40; }
  else if (classc == 'A') { problem_size = 256; nit = 4; }
  else if (classc == 'B') { problem_size = 256; nit = 20; }
  else if (classc == 'C') { problem_size = 512; nit = 20; }
  else {
    printf("setparams: Internal error: invalid classc type %c\n", classc);
    exit(1);
  }
...
void write_ft_info(FILE *fp, char classc) 
{
  ...
  if      (classc == 'S') { nx = 64; ny = 64; nz = 64; niter = 6;}
  else if (classc == 'W') { nx = 128; ny = 128; nz = 32; niter = 6;}
  else if (classc == 'A') { nx = 256; ny = 256; nz = 128; niter = 6;}
  else if (classc == 'B') { nx = 512; ny = 256; nz = 256; niter =20;}
  else if (classc == 'C') { nx = 512; ny = 512; nz = 512; niter =20;}
  else {
    printf("setparams: Internal error: invalid classc type %c\n", classc);
    exit(1);
  }
...
void write_ep_info(FILE *fp, char classc)
{
  ...
  int m;
  if      (classc == 'S') { m = 24; }
  else if (classc == 'W') { m = 25; }
  else if (classc == 'A') { m = 28; }
  else if (classc == 'B') { m = 30; }
  else if (classc == 'C') { m = 32; }
  else {
    printf("setparams: Internal error: invalid classc type %c\n", classc);
    exit(1);
  }
