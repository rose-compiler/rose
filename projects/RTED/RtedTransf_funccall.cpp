...
SgFunctionCallExp* RtedTransformation::convertIntToString(SgExpression* i)
{
  ...
  SgFunctionRefExp*  memRef_r = SB::buildFunctionRefExp(symbols.roseConvertIntToString);
  SgExprListExp*     arg_list = SB::buildExprListExp();

  SI::appendExpression(arg_list, i);
  return SB::buildFunctionCallExp(memRef_r, arg_list);
}
...
void RtedTransformation::insertFuncCall(RtedArguments& args)
{
  ...
  SgExpression*     callNameExp = SB::buildStringVal(args.f_name);
  SgExprListExp*    arg_list = SB::buildExprListExp();
  ...
  SI::appendExpression(arg_list, args.leftHandSideAssignmentExprStr);
  std::cerr << " ... Left hand side variable : " << args.leftHandSideAssignmentExprStr << std::endl;
  ...
  SgIntVal*            sizeExp = SB::buildIntVal(size);

  SI::appendExpression(arg_list, sizeExp);
  ...
  SgExprListExp*                      vararg_list = SB::buildExprListExp();
  ...
  {
     SgExpression* exp = SI::deepCopy(*it);
     ...
     if (isSgVarRefExp(exp))
     {
        ...
        std::cerr << " isSgVarRefExp :: type : " << vartype->class_name() << std::endl;
        std::cerr << "     base_type: " << base_type->class_name() << std::endl;

        // --------- varRefExp is TypeChar -----------------------
        if (isSgTypeChar(vartype) || isSgTypeChar(base_type)) {
           std::string        name = var->get_symbol()->get_declaration()->get_name().str();
           ...
           if (array)
           {
              ...
              std::cerr << " Found 00 - btype : " << array->get_base_type()->class_name() << "  index type: " << expr << std::endl;

              // this is the variable that we pass
              SI::appendExpression(vararg_list, var);
              if (dimIsTwo)
              {
                expr = ( expr == 0 ? SB::buildStringVal("00")
                                   : SB::buildStringVal(expr->unparseToString() + "\0")
                       );

                SI::appendExpression(vararg_list, expr);
              }

              std::cerr << ">>> Found variable : " << name << "  with size : " << expr->unparseToString() << "  and val : " << var << std::endl;
           }
           else if (isSgPointerType(inittype))
           {
              std::cerr << "Cant determine the size of the following object : " << var->class_name() << "  with base type : "
                        << base_type->class_name() << std::endl;
              ...
              if (dimIsTwo)
              {
                ...
                SgExpression* manglName = SB::buildStringVal(var->get_symbol()->get_declaration()->get_mangled_name().str());

                SI::appendExpression(vararg_list, manglName);
              }
           }
           else
           {
              std::cerr << "Cant determine the size of the following object : " << var->class_name() << "  with type : "
                        << inittype->class_name() << std::endl;
              std::cerr << "   and base type : " << base_type->class_name() << std::endl;

              // this is most likely a single char, the size of it is 1
              SgExpression* andSign = SB::buildAddressOfOp(var);
              SgExpression* charCast = SB::buildCastExp(andSign, SB::buildPointerType(SB::buildCharType()));

              SI::appendExpression(vararg_list, charCast);

              if (dimIsTwo)
              {
                SgExpression* numberToString = SB::buildStringVal("001");

                SI::appendExpression(vararg_list, numberToString);
              }
           }
        }
        ...
        else if (isSgPointerType(vartype))
        {
           ...
           std::cerr << "RtedTransformation - unknown type : " << vartype->class_name() << std::endl;
           std::cerr << "   and base type : " << base_type->class_name() << "  var:" << var->unparseToString() << "   stmt:"
           ...
           if (var) {
              ...
              SgExpression* manglName = SB::buildStringVal(initName->get_mangled_name().str());
              if (isSgFunctionParameterList(initName->get_parent())) {
                 //       cerr << "\n!!!!!!!!!!parent of initName = " << initName->get_parent()->class_name() << endl;
                 manglName = SB::buildStringVal(initName->get_name().str());
              }
        ...
        else
        {
           ...
           SI::appendExpression(vararg_list, convertIntToString(var));
           std::cerr << " Created Function call  convertToString" << std::endl;
        }
     }
     // --------- this is not a varRefExp ----
     else if (isSgStringVal(exp))
     {
        std::string theString = isSgStringVal(exp)->get_value();

        SI::appendExpression(vararg_list, SB::buildStringVal(theString + "\0"));

        if (dimIsTwo)
        {
           size_t        sizeString = theString.size() + 1; // add the '\0' to it
           std::string   sizeStringStr = RoseBin_support::ToString(sizeString);
           SgExpression* numberToString = SB::buildStringVal(sizeStringStr);
     ...
     {
       SI::appendExpression(vararg_list, convertIntToString(exp));

       if (dimIsTwo)
       {
         SI::appendExpression(vararg_list, SB::buildStringVal("-1"));
       }
     }
     else
     {
        // default create a string
        std::string   theString = exp->unparseToString();
        SgExpression* stringExp = SB::buildStringVal(theString + "\0");

        SI::appendExpression(vararg_list, stringExp);
  ...
  SgType*                 constCharPtrArr = SB::buildArrayType(constCharPtr, NULL);
  SgAggregateInitializer* tmparrInit = genAggregateInitializer(vararg_list, constCharPtr);
  SgVariableDeclaration*  tmparr = SB::buildVariableDeclaration( tmpName, constCharPtrArr, tmparrInit, scope );

  SageInterface::insertStatement(stmt, tmparr, true /* before */);

  SI::appendExpression( arg_list, SB::buildVarRefExp(tmparr) );
...
void RtedTransformation::insertFreeCall(SgExpression* freeExp, AllocKind ak)
{
   ...
   SI::appendExpression( arg_list, mkAddress(address_expression, upcShared) );
   ...
   if (upcShared)
   {
     ...
     insertCheck(ilBefore, stmt, symbols.roseUpcBeginExclusive, SB::buildExprListExp());
     insertCheck(ilAfter,  stmt, symbols.roseUpcEndExclusive,   SB::buildExprListExp());
   }
}
...
void RtedTransformation::insertReallocateCall(SgFunctionCallExp* realloc_call)
{
   ...
   SI::appendExpression( arg_list, address_expression );
   SI::appendExpression( arg_list, size_expression );
   appendFileInfo( arg_list, stmt );
...
getMangledNameOfExpression(SgExpression* expr)
{
  ...
  if (var_refs.size()==0)
  {
    ...
    std::cerr << " getMangledNameOfExpression: varRef on left hand side not found : " << expr->unparseToString() << std::endl;
  }
  else if (var_refs.size()==1)
  {
    ...
    std::cerr << " getMangledNameOfExpression: found varRef: " << manglName << std::endl;
  }
  else if (var_refs.size()>1)
  {
    // error
    std::cerr << " getMangledNameOfExpression: Too many varRefs on left hand side : " << var_refs.size() << std::endl;
  }
  ...
  if (handled)
  {
    if (isStringModifyingFunctionCall(callee.name) || isFileIOFunctionCall(callee.name))
    {
       ...
       if (varOnLeft) {
          ...
          varOnLeftStr = SB::buildStringVal(getMangledNameOfExpression(varOnLeft));
       } else {
          varOnLeftStr = SB::buildStringVal("NoAssignmentVar");
       }
