   ...
   BOOST_FOREACH( SgDeclarationStatement* decl, constructors )
            {
               ...
               SgSymbolTable* sym_tab = cdef -> get_declaration() -> get_scope() -> get_symbol_table();

               SgClassSymbol* csym = isSgClassSymbol(sym_tab -> find_class(cdef -> get_declaration() -> get_name()));
               ROSE_ASSERT( csym );
               ...
               SgExprStatement* fncall = buildExprStatement(buildFunctionCallExp(buildFunctionRefExp(symbols->roseCreateObject),
                     arg_list));
               attachComment(fncall, "", PreprocessingInfo::before);
               attachComment(fncall, "RS: Create Variable, parameters: "
                  "type, basetype, indirection_level, "
                  "address, size, filename, line, linetransformed", PreprocessingInfo::before);
...
void RtedTransformation::insertVariableCreateCall(SgInitializedName* initName) {
   ...
   if (externQual) {
      cerr << "Skipping this insertVariableCreateCall because it probably occurs multiple times (with and without extern)."
            << endl;
      return;
   }
   ...
   if (isSgStatement(stmt)) {
      ...
      if (isSgForInitStatement(stmt -> get_parent())) {
         ...
         if (buildVar == NULL)
            return;
         prependPseudoForInitializerExpression(buildVar, isSgForStatement(stmt -> get_parent() -> get_parent()));
...
SgFunctionCallExp*
RtedTransformation::buildVariableCreateCallExpr(SgInitializedName* initName, SgStatement* stmt, bool forceinit) {
    ...
    * the constructor.
    */
   bool initb = (initializer && !(isSgConstructorInitializer(initializer))) || forceinit;
...
RtedTransformation::buildVariableCreateCallStmt(SgInitializedName* initName, SgStatement* stmt, bool forceinit) {
   ...
   SgFunctionCallExp* fn_call = buildVariableCreateCallExpr(initName, stmt, forceinit);
   if (fn_call == NULL)
      return NULL;
...
SgExpression*
RtedTransformation::buildVariableInitCallExpr(SgInitializedName* initName, SgExpression* varRefE, SgStatement* stmt,
      bool ismalloc) {
   ...
   if (unionclass && unionclass->get_declaration()->get_class_type() == SgClassDeclaration::e_union)
      isUnionClass = true;
   //if (!isUnionClass)
   unionclass = NULL;
   ...
   if (lval)
      is_pointer_change = isSgExprStatement(stmt) && isSgPointerType(lval-> get_type());
#else
...
void RtedTransformation::insertInitializeVariable(SgInitializedName* initName, SgExpression* varRefE, bool ismalloc) {
   ...
   if (isSgStatement(stmt)) {
      ...
      // what is there is an array creation in a global scope
         ...
         cerr << " ------->....... RuntimeInstrumentation :: WARNING - Scope not handled!!! : " << name << " : "
               << scope->class_name() << "\n\n\n\n" << endl;
         // We need to add this new statement to the beginning of main
         // get the first statement in main as stmt
         stmt = mainFirst;
      ...
      if (isSgForInitStatement(stmt -> get_parent())) {
         ...
         prependPseudoForInitializerExpression(funcCallExp_vec, isSgForStatement(stmt -> get_parent() -> get_parent()));
      } else if (isSgIfStmt(scope)) {
         SgExpression* funcCallExp = buildVariableInitCallExpr(initName, varRefE, stmt, ismalloc);
         ...
         if (exprStmt) {
            ...
            if (partOfTrue == false && partOfFalse == false)
               partOfCondition = true;
            cerr << "@@@@ If cond : partOfTrue: " << partOfTrue << "   partOfFalse:" << partOfFalse << "  partOfCondition:"
         ...
         cerr << " -----------> RuntimeInstrumentation :: Surrounding Block is not Block! : " << name << " : "
               << scope->class_name() << "  - " << stmt->unparseToString() << endl;
         ROSE_ASSERT(false);
      }
   } else {
      cerr << "RuntimeInstrumentation :: Surrounding Statement could not be found! " << stmt->class_name() << "  "
...
void RtedTransformation::insertCheckIfThisNull(SgThisExp* texp) {
   ...
   if (isSgStatement(stmt)) {
      ...
      if (isNormalScope(scope)) {
         ...
         SgFunctionRefExp* memRef_r = buildFunctionRefExp(symbols->roseCheckIfThisNULL);
         ...
         string empty_comment = "";
         attachComment(exprStmt, empty_comment, PreprocessingInfo::before);
         string comment = "RS : roseCheckIfThisNULL, paramaters : (ThisExp, filename, line, line transformed, error Str)";
         attachComment(exprStmt, comment, PreprocessingInfo::before);
      } // basic block
      else if (isSgNamespaceDefinitionStatement(scope)) {
...
void RtedTransformation::insertAccessVariable(SgScopeStatement* initscope, SgExpression* derefExp, SgStatement* stmt,
      SgExpression* varRefE) {
   if (stmt->get_file_info()->isCompilerGenerated())
      return;
   if (isSgVarRefExp(varRefE) && !isInInstrumentedFile(isSgVarRefExp(varRefE)->get_symbol()->get_declaration()))
      return;
   if (isSgStatement(stmt)) {
      SgScopeStatement* scope = stmt->get_scope();
      //string name = initName->get_mangled_name().str();
      ...
      if (isNormalScope(scope)) {
         ...
         if (derefExp) {
            ...
            if (arrow_op) {
               ...
               if (isUsedAsLvalue(arrow_op)) {
                  bool isReadOnly = isthereAnotherDerefOpBetweenCurrentAndAssign(derefExp);
         ...
         //cerr << " >>>>>>>> Symbol Member: " << symbolName2 << endl;
         SgFunctionRefExp* memRef_r = buildFunctionRefExp(symbols->roseAccessVariable);
         SgFunctionCallExp* funcCallExp = buildFunctionCallExp(memRef_r, arg_list);
         SgExprStatement* exprStmt = buildExprStatement(funcCallExp);
         // insert new stmt (exprStmt) before (old) stmt
         insertStatementBefore(isSgStatement(stmt), exprStmt);
...
void RtedTransformation::visit_isAssignInitializer(SgNode* n) {
   ...
   // we do so
   cerr << ">>>>>>> Setting this var to be assign initialized : " << initName->unparseToString() << "  and assignInit: "
   ...
   if (isSgGlobal(initName->get_scope()) || isSgEnumDeclaration(initName->get_parent())) {
      ...
      if (ismalloc) {
         ...
         SgExpression* sizeExp = buildSizeOfOp(oldnewExp->get_specified_type()
