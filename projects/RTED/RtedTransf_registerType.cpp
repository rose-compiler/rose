...
void RtedTransformation::visit_isClassDefinition(SgClassDefinition* const cdef)
{
  ...
  std::string               qualname = cdef->get_qualified_name();
  ...
  if (   (  cdef -> get_file_info() -> isCompilerGenerated()
     ...
     ||  boost::starts_with(qualname, std::string("::rted_"))
     )
  {
  ...
  RtedClassDefinition*& entry = class_definitions[cdef];

  // Nothing left to do, if we have already seen the class
  ...
  cerr << ">>>> Found ClassDefinition " << qualname << endl;
  cerr << ">> cdef = " << cdef << endl;

  entry = new RtedClassDefinition( cdef,
...
void RtedTransformation::insertRegisterTypeCall(RtedClassDefinition* const rtedClass)
{
  ...
  ROSE_ASSERT( stmt );
  std::string typeC = rtedClass->classType;
  const bool  isUnionType = (rtedClass->classDef->get_declaration()->get_class_type() == SgClassDeclaration::e_union);
  ...
  if (isNormalScope(scope)) // || isSgClassDefinition(scope))
  {
    ...
    // \pp \todo why is the following test here?
