	...
	if (isSgStatement(stmt)) {
		...
		if (isSgBasicBlock(scope)) {
			...
			for (; it != value.end(); ++it) {
				...
				//				cerr << "       arg : " << expr << "   " << expr->unparseToString() << endl;
			}
...
void RtedTransformation::insertArrayAccessCall(SgStatement* stmt,
	...
	if (isSgStatement(stmt)) {
		...
		if (isSgIfStmt(stmt->get_parent())
				&& isSgIfStmt(stmt->get_parent())->get_conditional() == stmt) {
			cerr << "       >> Current stmt == if-conditional " << endl;
			//return;
		} else if (isSgWhileStmt(stmt->get_parent()) && isSgWhileStmt(
				stmt->get_parent())->get_condition() == stmt) {
			cerr << "       >> Current stmt == while-conditional " << endl;
			//return;
		}
		scope = isSgBasicBlock(ensureBasicBlockAsParent(stmt));
		cerr << " SCOPE :: " << scope->class_name() << endl;
		...
		SgExpression* expr = buildIntVal(0);
		if (array->indx2)
		...
		#if 0
		std::vector<SgExpression*>::const_iterator it = value.begin();
		cerr << " .. Creating Array Access Call :: " << initName->unparseToString() << "   size : " << value.size() << endl;
		for (; it != value.end(); ++it) {
			SgExpression* expr = isSgExpression(*it);
			if (expr==NULL)
			expr = buildIntVal(-1);
				...
				if (func && size) {
					...
					if (ismalloc) {
						...
						if (indx1 == NULL && indx2 == NULL) {
							// array initialized to pointer: pntr = malloc
							indx1 = size;
