...
CodeThorn::CommandLineOptions& parseCommandLine(int argc, char* argv[], Sawyer::Message::Facility logger, std::string version) {
  ...
  experimentalOptions.add_options()
    ...
    ("normalize-all", po::value< bool >()->default_value(true)->implicit_value(true),"Normalize all expressions before analysis.")
    ...
    ("abstraction-mode",po::value< int >()->default_value(1),"Select abstraction mode (0: equality merge (explicit model checking), 1: approximating merge (abstract model checking).")
