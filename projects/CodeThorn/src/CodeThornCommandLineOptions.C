...
CodeThorn::CommandLineOptions& parseCommandLine(int argc, char* argv[], Sawyer::Message::Facility logger, std::string version,
  ...
  experimentalOptions.add_options()
    ("ast-symbol-check",po::value< bool >(&ctOpt.astSymbolCheckFlag)->default_value(true),"Runs AST symbol check and exits on error.")
    ...
    ("program-stats-file",po::value< std::string >(&ctOpt.programStatsFileName),"print some basic program statistics about used language constructs.")
    ...
    ("context-sensitive",po::value< bool >(&ctOpt.contextSensitive)->default_value(false)->implicit_value(true),"Perform context sensitive analysis. Uses call strings with arbitrary length, recursion is not supported yet.")
    ...
    ("filter-initial-state", po::value< bool > (&ctOpt.initialStateFilterUnusedVariables)->implicit_value(true),"Filter unused variables when creating initial state.")
  ...
  visibleOptions.add_options()            
    ...
    ("register-range-start",po::value< string > (&ctOpt.registerAddressStartString)," set start address of registers (hex, octal, or dezimal).")
    ("register-range-end",po::value< string > (&ctOpt.registerAddressEndString)," set end address of registers (hex, octal, or dezimal).")
