...
#include "LTLThornCommandLineOptions.h"
...
CodeThorn::CommandLineOptions& parseCommandLine(int argc, char* argv[], Sawyer::Message::Facility logger, std::string version,
  ...
  visualizationOptions.add_options()
    ...
    ("tg1-memory-subgraphs", po::value< bool >(&ctOpt.visualization.tg1EStateMemorySubgraphs)->default_value(false)->implicit_value(true), "Transition graph 1: Show estate as memory graphs.")
  ...
  experimentalOptions.add_options()
    ...
    ("call-string-length",po::value< int >(&ctOpt.callStringLength)->default_value(10),"Set the length of the callstring for context-sensitive analysis. Default value is 10.")
  ...
  visibleOptions.add_options()            
    ...
    ("max-memory-forced-top",po::value< long int >(&ctOpt.maxMemoryForcedTop)->default_value(-1),"Performs approximation after <arg> Gigabytes of physical memory have been used.")
