...
string FunctionId::toString(const FunctionIdMapping& idMapping) const {
...
void FunctionIdMapping::computeFunctionSymbolMapping(SgProject* project) {
  ...
  // determine mapping FunctionDefinition->FunctionSymbol
  ...
  for(auto funDef : funDefList) {
    ...
    if(funIdIter!=mappingFunctionIdToFunctionDef.end()) {
      ...
      if((*funIdIter).second==0) {
        ...
        cout<<" setting FunctionId to def: "<<funDef<<endl;
      } else {
        cout<<" Problem: FunctionId maps to more than one def: ";
        cout<<"FunctionId1: "<<(*funIdIter).first.toString()<<" ==>" <<(*funIdIter).second << "|" << funDef<<endl;
#if 0
        cout<<"- 1 ------------------"<<endl;
