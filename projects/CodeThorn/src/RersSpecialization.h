...
#define COPY_PSTATEPTR_TO_GLOBALPTR(VARNAME) VARNAME[thread_id] = analyzer->mapGlobalVarAddress[analyzer->getVarNameByIdCode(pstate.readFromMemoryLocation(analyzer->globalVarIdByName(STR_VALUE(VARNAME))).getIntValue())]
#define COPY_GLOBALPTR_TO_PSTATEPTR(VARNAME) pstate.writeToMemoryLocation(analyzer->globalVarIdByName(STR_VALUE(VARNAME)), CodeThorn::AbstractValue(analyzer->globalVarIdByName(analyzer->mapAddressGlobalVar[VARNAME[thread_id]]).getIdCode()));
