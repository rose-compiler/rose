       ...
       // Loop over all the attributes at this IR node
          for (AstAttributeMechanism::iterator i = astAttributeContainer->begin(); i != astAttributeContainer->end(); i++)
            ...
            // The attributes for Point, Shift, Stencil, etc are only put at variables.
               ...
               Shift_Attribute* shiftAstAttribute = dynamic_cast<Shift_Attribute*>(dslAstAttribute);
               if (shiftAstAttribute != NULL)
                  {
                    shiftValueState[variableIdMapping.variableId(initializedName)] = new ShiftValue_Attribute();
                  }
             }
        }
#endif
