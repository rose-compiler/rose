...
int choice( 0 ); // for menu choice in visit()
...
void visitorTraversal::visit(SgNode* n) {
    // allow for just displaying nodes for debug
    ...
    //SgOmpForStatement * ofs = isSgOmpForStatement( n );
    //SgForStatement * fs = isSgForStatement( n );
    //SgOmpReductionClause * rc = isSgOmpReductionClause( n );
    
    if ( ps ) {
        if ( choice == 1 ) showNodes = true; 
        else   
