...
void getPrivateVarRefs( const SgOmpClausePtrList & clauses, SgVarRefExpPtrList & vars )
{
   ...
   rClause = isSgOmpReductionClause( clauses.at( i) );
    ...
    /*
     for( int i = 0; i<pSize; i++)
      {
           std::cout<<" the first variable is: "<< vars.at(0)->unparseToString()<<endl;
      }
    */
...
}/* the end of getPrivateVarRefs functions */
 ...
 */
 ...
 if( clauseList.size() > 0 )
  {
    getPrivateVarRefs( clauseList , privateVariables );
