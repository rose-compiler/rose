...
int main()
{
  const int global_arraySizeX = 5120;
  const int global_arraySizeY = 5120;
  const int global_arraySizeZ = 1;
  const int arraySize[3] = {global_arraySizeX,global_arraySizeY,global_arraySizeZ};

// allocate arrays
  REAL globalArray[arraySize[2]][arraySize[1]][arraySize[0]]; 
  memset(globalArray, 0, sizeof(globalArray[0][0][0]) * arraySize[0] * arraySize[1] * arraySize[2]);
  REAL newArray[arraySize[2]][arraySize[1]][arraySize[0]]; 
  memset(newArray, 0, sizeof(newArray[0][0][0]) * arraySize[0] * arraySize[1] * arraySize[2]);
// sequneceFill
  REAL factor = 100.;
//  int count = 1;
...
#pragma omp parallel for
     ...
     {
       newArray[0][j][i] = (globalArray[0][j][i-1] + globalArray[0][j][i+1] + globalArray[0][j-1][i] + globalArray[0][j+1][i] ) /4.0;
     }
     }
