...
specificationTraversal::getTransformationWorkList()
{
...
void specificationTraversal::handleInterLeaveAcrossArrays( SgFunctionCallExp* funcCallExp )
{
	...
	// Add to worklist
	transformationWorklist.addToWorklist(TransformationOptions::InterleaveAcrossArrays, inputArgs, outputArgs);
}
     ...
     if (isSgFunctionCallExp(n) != NULL)
        {
          ...
          TransformationOptions::LayoutType type = TransformationOptions::getLayoutType(functionName);

          switch(type)
          {
          	  case TransformationOptions::InterleaveAcrossArrays:
          		  	  handleInterLeaveAcrossArrays(funcCallExp);
          		  	  break;

			  case TransformationOptions::UnknownTransformation:
				  	  break;
