...
functionNBytes(const P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    size_t nBytes = 0;
    if (function && !function->attribute<size_t>(ATTR_NBytes).assignTo(nBytes)) {
        nBytes = partitioner.functionExtent(function).size();
        function->attribute(ATTR_NBytes, nBytes);
    }
    return nBytes;
}
...
functionNInsns(const P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    size_t nInsns = 0;
    if (function && !function->attribute<size_t>(ATTR_NInsns).assignTo(nInsns)) {
        ...
        function->attribute(ATTR_NInsns, nInsns);
    }
    return nInsns;
}
...
functionNIntervals(const P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    size_t nIntervals = 0;
    if (function && !function->attribute<size_t>(ATTR_NIntervals).assignTo(nIntervals)) {
        nIntervals = partitioner.functionExtent(function).nIntervals();
        function->attribute(ATTR_NIntervals, nIntervals);
    }
    return nIntervals;
}
...
functionNDiscontiguousBlocks(const P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    size_t retval = 0;
    if (function && !function->attribute<size_t>(ATTR_NDiscontiguousBlocks).assignTo(retval)) {
        ...
        function->attribute(ATTR_NDiscontiguousBlocks, retval);
    }
    return retval;
}
...
functionCfgGraphvizFile(const P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    ...
    if (function && !function->attribute<boost::filesystem::path>(ATTR_CfgGraphVizFile).assignTo(fileName)) {
        ...
        system(dotCmd.c_str());
        function->attribute(ATTR_CfgGraphVizFile, fileName);
    }
    return fileName;
}
...
functionCfgImage(const P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    ...
    if (function && !function->attribute<boost::filesystem::path>(ATTR_CfgImage).assignTo(imageName)) {
        ...
        function->attribute(ATTR_CfgImage, imageName);
    }
    return imageName;
}
...
functionCfgVertexCoords(const P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    ...
    if (function && !function->attribute<CfgVertexCoords>(ATTR_CfgVertexCoords).assignTo(coords)) {
        ...
        function->attribute(ATTR_CfgVertexCoords, coords);
    }
    return coords;
}
...
functionCallGraph(P2::Partitioner &partitioner) {
    ...
    if (!partitioner.attribute<P2::FunctionCallGraph*>(ATTR_CallGraph).assignTo(cg)) {
        cg = new P2::FunctionCallGraph(partitioner.functionCallGraph());
        partitioner.attribute(ATTR_CallGraph, cg);
    }
    return cg;
}
...
functionNCallers(P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    size_t nCallers = 0;
    if (function && !function->attribute<size_t>(ATTR_NCallers).assignTo(nCallers)) {
        P2::FunctionCallGraph *cg = functionCallGraph(partitioner);
        ASSERT_not_null(cg);
        nCallers = cg->nCallsIn(function);
        function->attribute(ATTR_NCallers, nCallers);
    }
    return nCallers;
}
...
functionNCallees(P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    size_t nCallees = 0;
    if (function && !function->attribute<size_t>(ATTR_NCallees).assignTo(nCallees)) {
        P2::FunctionCallGraph *cg = functionCallGraph(partitioner);
        ASSERT_not_null(cg);
        nCallees = cg->nCallsOut(function);
        function->attribute(ATTR_NCallees, nCallees);
    }
    return nCallees;
}
...
functionNReturns(P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    size_t nReturns = 0;
    if (function && !function->attribute<size_t>(ATTR_NReturns).assignTo(nReturns)) {
        BOOST_FOREACH (rose_addr_t bblockVa, function->basicBlockAddresses()) {
        ...
        function->attribute(ATTR_NReturns, nReturns);
    }
    return nReturns;
}
...
functionStackDelta(P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    ...
    if (function && !function->attribute<int64_t>(ATTR_StackDelta).assignTo(result)) {
        ...
        function->attribute(ATTR_StackDelta, result);
    }
    return result;
}
...
functionAst(P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    SgNode *result = NULL;
    if (function && !function->attribute<SgNode*>(ATTR_Ast).assignTo(result)) {
        result = P2::Modules::buildFunctionAst(partitioner, function, true /*relaxed*/);
        function->attribute(ATTR_Ast, result);
    }
...
functionDataFlow(P2::Partitioner &partitioner, const P2::Function::Ptr &function) {
    ...
    if (function && !function->attribute<FunctionDataFlow>(ATTR_DataFlow).assignTo(result)) {
        ...
        if (BaseSemantics::DispatcherPtr cpu = partitioner.newDispatcher(ops)) {
            ...
            function->attribute(ATTR_DataFlow, result);
        }
    }
    return result;
}
