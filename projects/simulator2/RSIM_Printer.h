...
// See class Translate
#define TF(X)           {(0==X?unsigned(-1):X), (unsigned)(X), #X}  // Define a bit flag.
#define TF2(M,X)        {M, X, #X}                                  // Define a bit vector with possible zero bits.
#define TF3(M,V,X)      {(unsigned)(M), V, #X}                      // Define a masked flag when X is not defined.
#define TF_FMT(M,FMT)   {0, M, FMT}                                 // Format remaining bits; FMT is a printf format string.
#define TE(X)           {(uint32_t)-1, (unsigned)(X), #X}           // Define an enumerated constant.
