                    ...
                    j != (*i)->arg_block->statements.end(); ++j)
            {
                ...
                if (aa && isSgConstructorInitializer(aa->get_rhs()))
                {
                    ...
                    SgExpression *new_expr
                        = isSgExpression(aa->get_rhs()->copy(treecopy));
                    if (isSgVarRefExp(aa->get_lhs()))
                        new_expr->set_parent(isSgVarRefExp(aa->get_lhs())
                    ...
                    if (isSgClassType(aa->get_lhs()->get_type()))
                    {
                        ...
                        if (subtype_of(cc, ca))
                        {
                            ...
                            ArgumentAssignment *a
                                = dynamic_cast<ArgumentAssignment *>
                                (f->statements[0]);
                            if (a == NULL)
                                break;
                                        ...
                                        new SgVariableSymbol(
                                            new SgInitializedName(
                                                ...
                                                new SgPointerType(
                                                    ...
                                                    ->get_type()))));
                            f->statements[0]
                                = new ArgumentAssignment(a->get_lhs(),
                                        this_ref);
                        }
                    }
                }
...
void CFGTraversal::perform_goto_backpatching()
{
    ...
    for (pi = cfg->procedures->begin(); pi != cfg->procedures->end(); ++pi)
    {
        ...
        while (gotos != (*pi)->goto_blocks.end())
        {
            ...
            if (target != (*pi)->goto_targets.end())
                ...
                std::cout
                    << "warning: goto statement with unknown label!"
                    << std::endl;
            ++gotos;
        }
    }
...
expression_set *make_nonredundant (const expression_set &exprs)
{
...
void CFGTraversal::number_exprs()
{
    ...
    for (block = cfg->nodes.begin(); block != cfg->nodes.end(); ++block)
                ...
                stmt != (*block)->statements.end(); ++stmt)
        {
            ...
            if (isSgCaseOptionStmt(*stmt) || isSgExprStatement(*stmt)
                    || isSgScopeStatement(*stmt))
                est.traverse(*stmt, preorder);
            else if (dynamic_cast<ArgumentAssignment *>(*stmt))
            {
                est.traverse(
                        dynamic_cast<ArgumentAssignment *>(*stmt)->get_lhs(),
                        preorder);
                est.traverse(
                        dynamic_cast<ArgumentAssignment *>(*stmt)->get_rhs(),
                        preorder);
            }
            else if (dynamic_cast<ReturnAssignment *>(*stmt))
            {
                est.traverse(new SgVarRefExp(new Sg_File_Info(),
                        dynamic_cast<ReturnAssignment *>(*stmt)->get_lhs()),
                        preorder);
                est.traverse(new SgVarRefExp(new Sg_File_Info(),
                        dynamic_cast<ReturnAssignment *>(*stmt)->get_lhs()),
                        preorder);
            ...
            {
                est.traverse(new SgVarRefExp(new Sg_File_Info(),
                        dynamic_cast<ParamAssignment *>(*stmt)->get_lhs()),
                        preorder);
                est.traverse(new SgVarRefExp(new Sg_File_Info(),
                        dynamic_cast<ParamAssignment *>(*stmt)->get_lhs()),
                        preorder);
                ...
                est.traverse(
                        dynamic_cast<LogicalIf *>(*stmt)->get_condition(),
                        preorder);
        }
    unsigned int i = 0;
...
void CFGTraversal::visit(SgNode *node)
{
    ...
    if (isSgFunctionDeclaration(node))
    {
        ...
        if (decl->get_definition() != NULL)
        {
            ...
            BasicBlock *last_node = (proc->this_assignment ?
                    proc->this_assignment : proc->exit);
...
BasicBlock *CFGTraversal::transform_block(SgBasicBlock *block,
        ...
        BasicBlock *continue_target)
{
     ...
     */
    Sg_File_Info *info = new Sg_File_Info();
    SgTreeCopy treecopy;
    ...
    for (s = stmts.begin(); s != stmts.end(); ++s)
    {
        if (isSgVariableDeclaration(*s))
        {
                    ...
                    n != decl->get_variables().end(); ++n)
            {
                ...
                SgVariableSymbol *varsym = new SgVariableSymbol(*n);
    ...
    SgInitializedNamePtrList::iterator n;
    for (n = local_classvar_decls.begin(); n != local_classvar_decls.end();
            ++n)
    {
    ...
    for (i = stmts.rbegin(); i != stmts.rend(); ++i)
    {
        ...
        case V_SgIfStmt:
            {
                ...
                join_block->statements.push_back(new IfJoin());
                ...
                SgExpression *new_expr
                    = isSgExpression(cond->get_expression_root()
                ...
                if (et.get_root_var() != NULL)
                    new_expr = new SgVarRefExp(info, et.get_root_var());
                
                if_block->statements.push_front(
                        new SgIfStmt(ifs->get_file_info(),
        ...
        case V_SgForStatement:
            {
                ...
                for (it = init_list.rbegin(); it != init_list.rend(); ++it)
                {
                    ...
                    if (init_expr)
                    {
                        init_block = allocate_new_block(init_block,
                                init_block_after);
                        SgExpression *new_expr
                            = isSgExpression(init_expr->get_expression_root()
                                    ->get_operand_i()->copy(treecopy));
                        new_expr->set_parent(NULL);
                        ExprLabeler el(expnum);
                        el.traverse(new_expr, preorder);
                        ...
                        init_block->statements.push_front(
                                new SgExprStatement(info, new_expr));
                    ...
                    else if (init_decl)
                    {
                        ...
                        for (it = vars.rbegin(); it != vars.rend(); ++it)
                        {
                            ...
                            SgExpression *new_expr
                                = isSgExpression(init->get_operand_i()
                                        ->copy(treecopy));
                            new_expr->set_parent(NULL);
                            ExprLabeler el(expnum);
                            el.traverse(new_expr, preorder);
                            ...
                            if (et.get_root_var() != NULL)
                                new_expr
                                    = new SgVarRefExp(info, et.get_root_var());

                            SgVarRefExp *var = new SgVarRefExp(
                                    info, new SgVariableSymbol(*it));
                            init_block->statements.push_front(
                                    new SgExprStatement(info,
                                        new SgAssignOp(info, var, new_expr)));
                            init_block_after = et.get_after();
                ...
                SgExpression *new_expr
                    = isSgExpression(fors->get_test_expr()->copy(treecopy));
                new_expr->set_parent(NULL);
                ExprLabeler el(expnum);
                el.traverse(new_expr, preorder);
                expnum = el.get_expnum();
                ...
                for_block->statements.push_front(
                        new SgWhileStmt(info,
                            new SgExprStatement(info, new_expr), NULL));
#else
                for_block->statements.push_front(
                ...
                SgExpression *new_expr_inc
                    = isSgExpression(fors->get_increment_expr()
                            ->copy(treecopy));
                new_expr->set_parent(NULL);
                ...
                if (et_inc.get_root_var() != NULL)
                    new_expr_inc
                        = new SgVarRefExp(info, et_inc.get_root_var());
                
                incr_block->statements.push_front(
                        new SgExprStatement(info, new_expr_inc));
                BasicBlock *incr_block_after = et_inc.get_after();
                /* unfold the body */
                BasicBlock *body
                    = transform_block(fors->get_loop_body(), incr_block_after,
        ...
        case V_SgWhileStmt:
            {
                ...
                BasicBlock *join_block
                    = new BasicBlock(node_id++, INNER, proc->procnum);
                cfg->nodes.push_back(join_block);
                join_block->statements.push_back(new WhileJoin());
                add_link(join_block, after, TRUE_EDGE);
                after = join_block;
                ...
                while_block->statements.push_front(
                        new SgWhileStmt(whiles->get_file_info(),
                            new SgExprStatement(info, new_expr)));
                
                BasicBlock *body
                    = transform_block(whiles->get_body(), et.get_after(),
                            after, et.get_after());
        ...
        case V_SgDoWhileStmt:
            {
                ...
                dowhile_block->statements.push_front(
                        new SgDoWhileStmt(dowhiles->get_file_info(), NULL,
                            new SgExprStatement(info, new_expr)));
        ...
        case V_SgSwitchStatement:
            {
                ...
                if (isSgExprStatement(item_sel))
                    new_expr = isSgExprStatement(item_sel)->get_the_expr();
                else
                {
                    std::cout << "unsupported statement type in switch"
                        << std::endl;
                    new_expr = NULL;
                }
                new_expr->set_parent(NULL);
                ...
                switch_block->statements.push_front(
                        new SgSwitchStatement(info,
                            new SgExprStatement(info, new_expr), NULL));
        ...
        case V_SgBasicBlock:
            {
                ...
                BasicBlock *body = transform_block(block, after,
                        break_target, continue_target);
        ...
        case V_SgReturnStmt:
            {
                ...
                if (last_block_of_this_block != NULL)
                    ...
                    new_block = allocate_new_block(new_block, proc->exit);
                if (returns->get_return_expr() == NULL)
                {
                ...
                SgExpression *new_expr = isSgExpression(
                        returns->get_return_expr()->copy(treecopy));
                new_expr->set_parent(NULL);
                ExprLabeler el(expnum);
                el.traverse(new_expr, preorder);
                expnum = el.get_expnum();
                ...
                new_block->statements.push_front(
                        ...
                        new SgVarRefExp(info, proc->returnvar), new_expr)));
        ...
        case V_SgVariableDeclaration: 
            {
                ...
                {
                    SgVariableSymbol *declared_var = new SgVariableSymbol(
                        new SgInitializedName((*j)->get_name(),
                                (*j)->get_type()));
                    ...
                    else if (initializer)
                    {
                        SgTreeCopy treecopy;
                        SgExpression *new_expr
                            = isSgExpression(initializer->get_operand_i()
                                    ->copy(treecopy));
                        new_expr->set_parent(NULL);
        ...
        case V_SgExprStatement:
            ...
            if (exprs != NULL)
            {
                SgTreeCopy treecopy;
                SgExpression *new_expr
                    = isSgExpression(exprs->get_the_expr()->copy(treecopy));
                new_expr->set_parent(NULL);
...
BlockList *CFGTraversal::do_switch_body(SgBasicBlock *block,
        BasicBlock *after, BasicBlock *continue_target)
{
                ...
                */
                ...
                if (!cases->get_body()->get_statements().empty())
                {
                    AstAttribute *stmt_end =
                        cases->get_body()->get_statements().back()
                        ->getAttribute("PAG statement end");
        ...
        case V_SgDefaultOptionStmt:
            {
                ...
                defaults->get_body()
                    ->addNewAttribute("PAG statement end", stmt_end);
                /* transform this block */
...
int CFGTraversal::find_procnum(const char *name) const
{
    int num = 0;
    std::deque<Procedure *>::const_iterator i;
...
BasicBlock *CFGTraversal::allocate_new_block(BasicBlock *new_block,
        BasicBlock *after)
{
    /* if (new_block == NULL)
    {
...
BasicBlock *CFGTraversal::allocate_block_without_successor(BasicBlock *nb)
{
    if (nb == NULL)
    {
...
bool CFGTraversal::is_destructor_decl(SgFunctionDeclaration *fd) const
{
...
BasicBlock *CFGTraversal::call_base_destructors(Procedure *p,
        BasicBlock *after)
{
    ...
    for (bi = blocks.begin(); bi != blocks.end(); ++bi)
    {
        ...
        SgInitializedName *this_var = new SgInitializedName(
                this_var_name, new SgPointerType(
                    p->class_type->get_declaration()->get_type()));
        SgVariableSymbol *this_var_sym
            = new SgVariableSymbol(this_var);
        SgVariableSymbol *this_sym = new SgVariableSymbol(
                new SgInitializedName("this", this_var->get_type()));
        ...
        this_block->statements.push_back(new ArgumentAssignment(
                    ...
                    new SgVarRefExp(new Sg_File_Info(), this_sym)));
        /* set links */
    ...
    if (blocks.empty() && !p->class_type->get_inheritances().empty())
    {
        ...
        cfg->nodes.push_back(b);
        b->statements.push_back(new DestructorCall(
                    p->class_type->get_declaration()->get_name().str(),
                    p->class_type->get_declaration()->get_type()));
