...
wrong, and the overall structure is not fixed. All of this means that this
manual is the best documentation SATIrE has ever had :-)
...
The general \texttt{Program} (Section~\ref{sec:satire_program}) object used
by SATIrE analyzers is built by calling its
\begin{verbatim}
...
Finally, the SATIrE ICFG (Section~\ref{sec:satire_icfg}) is built from a
ROSE AST by calling:
\begin{verbatim}
    CFG *createICFG(SgProject *astRoot, AnalyzerOptions *opts);
\end{verbatim}

Annotated or transformed programs represented by a ROSE AST can be output by
calling the
\begin{verbatim}
    void outputProgramRepresentation(SgProject *astRoot,
...
\begin{verbatim}
    ...
    int main(int argc, char **argv)
    {
        ...
        SgProject *root = createRoseAst(options);
        outputProgramRepresentation(root, options);
    }
\end{verbatim}
...
using the \texttt{--run-pointsto-analysis} command line flag.
...
\end{verbatim}
to find out whether there might be some pointer pointing to a given
\texttt{Location}; and
\begin{verbatim}
...
intermingled with input file names. The following flags are known:
{\scriptsize
\begin{verbatim}
  Frond End options:
