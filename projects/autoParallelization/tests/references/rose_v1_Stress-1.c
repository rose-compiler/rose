...
void StressStrainWork(real8 *deltz,real8 *delts,const real8 *newSxx,const real8 *newSyy,const real8 *newSzz,const real8 *newTxy,const real8 *newTxz,const real8 *newTyz,const real8 *sxx,const real8 *syy,const real8 *txy,const real8 *txz,const real8 *tyz,const real8 *dxx,const real8 *dyy,const real8 *dzz,const real8 *dxy,const real8 *dxz,const real8 *dyz,real8 deltaTime,const int *zoneset,const real8 *vc,const real8 *vnewc,int length)
{
  ...
  for (i = 0; i <= length - 1; i += 1) {
    ...
    deltz[index] += quarterDelta * (vnewc[i] + vc[i]) * (dxx[index] * (sxx[index] + newSxx[i]) + dyy[index] * (syy[index] + newSyy[i]) + dzz[index] * (szz + newSzz[i]) + 2. * dxy[index] * (txy[index] + newTxy[i]) + 2. * dxz[index] * (txz[index] + newTxz[i]) + 2. * dyz[index] * (tyz[index] + newTyz[i]));
    delts[i] += quarterDelta * (vnewc[i] + vc[i]) * (dxx[index] * sxx[index] + dyy[index] * syy[index] + dzz[index] * szz + 2. * dxy[index] * txy[index] + 2. * dxz[index] * txz[index] + 2. * dyz[index] * tyz[index]);
  }
}
...
/* Quiet the compiler - unused argument */
  for (i = 0; i <= length - 1; i += 1) {
    ...
    if (shearMod[nz] > 0.) {
      ...
      deltrh[nz] = .25 * (vnewc[i] + vc[i]) * ((newSxx[i] * sheari - sxx[nz] * shearr) * (sxx[nz] + newSxx[i]) + (newSyy[i] * sheari - syy[nz] * shearr) * (syy[nz] + newSyy[i]) + (newSzz[i] * sheari + (syy[nz] + sxx[nz]) * shearr) * (newSzz[i] - sxx[nz] - syy[nz]) + 2. * (newTxy[i] * sheari - txy[nz] * shearr) * (txy[nz] + newTxy[i]) + 2. * (newTxz[i] * sheari - txz[nz] * shearr) * (txz[nz] + newTxz[i]) + 2. * (newTyz[i] * sheari - tyz[nz] * shearr) * (tyz[nz] + newTyz[i]));
    }
     else {
      deltrh[nz] = - .25 * (vnewc[i] + vc[i]) * (sxx[nz] * (sxx[nz] + newSxx[i]) + syy[nz] * (syy[nz] + newSyy[i]) - (syy[nz] + sxx[nz]) * (newSzz[i] - sxx[nz] - syy[nz]) + 2. * txy[nz] * (txy[nz] + newTxy[i]) + 2. * txz[nz] * (txz[nz] + newTxz[i]) + 2. * tyz[nz] * (tyz[nz] + newTyz[i])) * shearr;
    }
