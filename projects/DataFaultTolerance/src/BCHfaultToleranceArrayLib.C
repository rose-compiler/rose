...
#include "BCHfaultToleranceArrayLibUtility.C"
//#include "bch3.C"
...
/*
 * TODO: Insert capability for other data types
 */
void init(int chunk, int noBits, string type)
{
	int dataElemSize = 0;

	if( type == "int")
		dataElemSize = sizeof(int);

	cout << " dataElemSize:" << dataElemSize <<endl;
	/*
	 * chunk specifies the number of contigous element that user wishes
	 * to encode as a block.
	 * chunk is promoted to nearest degree value of 2 ( 2**m ) for BCH
	 * chunk = data_elements * data_size + Req. Error bit encoding
	 * noBits specifies the number of resilient bits that user wants
	 */
	dataLength = initBCH( ceil(log(chunk*dataElemSize+noBits)/log(2)) , noBits);
