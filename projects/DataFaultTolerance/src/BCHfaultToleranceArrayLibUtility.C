 ...
 */
void updateHashEntry(string arrayEntry, vector<int> checkbits)
{
	hashVal hval;

	hval.checkbits = checkbits;
...
{
	chunk = pow(2, ceil(log(chunk)/log(2))); // Round to next power of 2
	...
	int data[adjUB-adjLB];

	for( int i=adjLB, index=0; i<=adjUB ; i++, index++)
	{
		int value = 0;
		...
		{
			// Store bch encoded check bits into hash table
			int bch_check_size;
			int* bch_check_bits = encode_bch(data, &bch_check_size);
			checkbits.assign(bch_check_bits, bch_check_bits+bch_check_size );
			string name = getName(arrayName, i-chunk, i-1);
			updateHashEntry(name, checkbits);
