...
#define FAULT_LIB_HEADER "faultToleranceLib.C"
...
void visitLoops(vector<SgForStatement*> loops) {
	...
	for (vector<SgForStatement*>::iterator iter = loops.begin(); iter
		...
		/* TODO: Add support for parsing the pragma */
...
{
	insertHeader(FAULT_LIB_HEADER,PreprocessingInfo::after,false,sfile->get_globalScope());
}
