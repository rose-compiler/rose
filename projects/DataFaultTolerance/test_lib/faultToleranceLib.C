...
unsigned int calculateCheckBits(void *memLoc, string type, bool *parity)
{
	unsigned int checkbits;
	...
	if(type.compare("int") == 0)
	{
		...
		*parity = calculateParity(data, checkbits);
		...
		checkbits = calculateIntCheckBits(*static_cast<float*>(memLoc));
	else
	{
...
void verifyCheckBits(void *memLoc, string type)
{
	...
	bool calParity;
	unsigned int calCheckBits = calculateCheckBits(memLoc, type, &calParity);
