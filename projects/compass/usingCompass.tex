...
\begin{itemize}
   ...
   \item CompassGUI: \\
         ...
         reruning compass and sifting thorugh the output while relatiting them to the source
   ...
   \item ToolGear post-processing: \\
         Output in XML permits the use of ToolGear (LLNL tool availalbe on the web) for
...
with Compass erroneus lines can be highlighted while programming, and the relevant error messages displayed 
...
\begin{enumerate}
   ...
   \item Execute {\tt ROSE\_SRC\_DIR/projects/compass/compassRepository/bin/gen\_checker.sh $<$name of your checker$>$}
\end{enumerate}

The results of executing gen\_checker.sh script is that a new directory name 'multipleCasesOnSameLine' 
...
\label{howToIntegrateNewChecker}
...
\begin{itemize}
\item {\bf CHECKER\_LIST}: a file that keeps a running list of all checkers seen by
...
Usually the CHECKER\_LIST is only modified when a user or delevoper wants to 
add a new checker or select a subset of trusted checkers.  Checkers can be
commented out using a {\tt \#}, no space is allowed between the {tt \#} and the 
checker name.
...
Since it is not unhead of that rules implemented by different checkers can be  
...
Seperate projects of developers could easily have their own RULE\_SELECTION file
...
tools build using ROSE may also be relavant.



\section{Checking Security Properties of Checkers}

    Compass is designed for extensability while providing the security for
the codes being checked.  To support this Compass provides a simple
mechanism for verifiying specific properties of the checkers used in Compass.
Compass implements a specific small number of checkers that are used for checking
the checkers in Compass.  The directory compassVerity contains the implementation
...
integrity of this ubset of Compass.  To verify the compass checkers run:
\begin{itemize}
   \item {\tt make verify} \\
        This makefile rule runs the compassVerify/compassMain on all the source files in
    all the checkers directies in Compass.  Because it runs compass on so many seperate
