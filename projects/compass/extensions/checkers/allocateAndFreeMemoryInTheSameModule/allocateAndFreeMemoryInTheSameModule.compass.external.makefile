...
CHECKER_NAME = allocateAndFreeMemoryInTheSameModuleTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h allocateAndFreeMemoryInTheSameModuleTest
...
allocateAndFreeMemoryInTheSameModuleTest: allocateAndFreeMemoryInTheSameModule.C allocateAndFreeMemoryInTheSameModuleMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ allocateAndFreeMemoryInTheSameModuleMain.C allocateAndFreeMemoryInTheSameModule.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: allocateAndFreeMemoryInTheSameModuleTest allocateAndFreeMemoryInTheSameModuleTest1.C
	./allocateAndFreeMemoryInTheSameModuleTest allocateAndFreeMemoryInTheSameModuleTest1.C

clean:
	rm -f libcompass.so "allocateAndFreeMemoryInTheSameModuleTest"
