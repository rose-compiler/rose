...
ScopExtractor::inspectBottomUpFunctionBody(SgProject* project, SgNode* root)
{
  ...
  // specialized for loop optimization      
  if(pragmaNodes.size() == 0)  
  {
    SgForStatement* forStmt = isSgForStatement(root);
    if(forStmt != NULL)
    {
      ScopExtractorVisitor scopTraversal(forStmt, polyoptions);
      if (! polyoptions.getQuiet())
        std::cout << "[PolyOpt] Start SCoP extraction for For loop" << std::endl;
      scopTraversal.reinitializeAttributes();
      scopTraversal.traverse(forStmt);
  ...
  else // perform as original PolyOpt that requires pragma
