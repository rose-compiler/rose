...
namespace OmpSupport{
  ...
  OmpAttribute* buildOmpAttribute(omp_construct_enum directive_type, SgNode* node)
  {
   ...
   result = new OmpAttribute(directive_type,pragmadecl);
   ROSE_ASSERT(result);
   return result;
  }
...
{ 
  std::cout<<"----------------------"<<std::endl;
  if (pragma)
   std::cout<<"Original pragma is:\n\t\t#pragma "<<pragma->get_pragma()->get_pragma()<<std::endl;
  if (parentPragma)  
   std::cout<<parentPragma->get_pragma()->get_pragma()<<std::endl;
  std::cout<<"Reproduced pragma is:\n";
