#ifndef LISTOFFLOATSERIALARRAY_H
#define LISTOFFLOATSERIALARRAY_H "ListOfFloatSerialArray.h"
#include "A++.h"
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfFloatSerialArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class floatSerialArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          floatSerialArray::reference( const floatSerialArray & t ) 
...
//
//  This class keeps an array of pointers to objects of class floatSerialArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfFloatSerialArray<floatArray> list,list2;
...
class ListOfFloatSerialArray : public ReferenceCounting
{
  ...
  ListOfFloatSerialArray(const ListOfFloatSerialArray&, 
    const CopyType=DEEP  ); 

  ~ListOfFloatSerialArray();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfFloatSerialArray& operator=(const ListOfFloatSerialArray&);
  ...
  void addElement( const floatSerialArray & t, const int index ); // Add an object and reference to object t

  void addElement( const floatSerialArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  floatSerialArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const floatSerialArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfFloatSerialArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     floatSerialArray **aList;		// Pointer to a list of pointers to object floatSerialArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfFloatSerialArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfFloatSerialArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfFloatSerialArray(*this, ct); }
};
