#ifndef LISTOFCOMPOSITEGRID_H
#define LISTOFCOMPOSITEGRID_H "ListOfCompositeGrid.h"
class CompositeGrid;
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfCompositeGrid Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class CompositeGrid to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          CompositeGrid::reference( const CompositeGrid & t ) 
//          CompositeGrid::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class CompositeGrid. 
...
//
//     floatArray a(10),b(5);
//     ListOfCompositeGrid<floatArray> list,list2;
...
class ListOfCompositeGrid : public ReferenceCounting
{
  ...
  ListOfCompositeGrid(const ListOfCompositeGrid&, 
    const CopyType=DEEP  ); 

  ~ListOfCompositeGrid();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfCompositeGrid& operator=(const ListOfCompositeGrid&);
  ...
  void addElement( const CompositeGrid & t, const int index ); // Add an object and reference to object t

  void addElement( const CompositeGrid & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  CompositeGrid& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const CompositeGrid & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfCompositeGrid;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     CompositeGrid **aList;		// Pointer to a list of pointers to object CompositeGrid.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfCompositeGrid &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfCompositeGrid &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfCompositeGrid(*this, ct); }
};
