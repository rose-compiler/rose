#ifndef LISTOFINTSERIALARRAY_H
#define LISTOFINTSERIALARRAY_H "ListOfIntSerialArray.h"
#include "A++.h"
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfIntSerialArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class intSerialArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          intSerialArray::reference( const intSerialArray & t ) 
...
//
//  This class keeps an array of pointers to objects of class intSerialArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfIntSerialArray<floatArray> list,list2;
...
class ListOfIntSerialArray : public ReferenceCounting
{
  ...
  ListOfIntSerialArray(const ListOfIntSerialArray&, 
    const CopyType=DEEP  ); 

  ~ListOfIntSerialArray();			//   Destructor.
  ...
  void addElement( const intSerialArray & t, const int index ); // Add an object and reference to object t

  void addElement( const intSerialArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  intSerialArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const intSerialArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfIntSerialArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     intSerialArray **aList;		// Pointer to a list of pointers to object intSerialArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfIntSerialArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfIntSerialArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfIntSerialArray(*this, ct); }
};
