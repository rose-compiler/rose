#ifndef LISTOFGENERICGRID_H
#define LISTOFGENERICGRID_H "ListOfGenericGrid.h"
class GenericGrid;
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfGenericGrid Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class GenericGrid to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          GenericGrid::reference( const GenericGrid & t ) 
//          GenericGrid::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class GenericGrid. 
...
//
//     floatArray a(10),b(5);
//     ListOfGenericGrid<floatArray> list,list2;
...
class ListOfGenericGrid : public ReferenceCounting
{
  ...
  ListOfGenericGrid(const ListOfGenericGrid&, 
    const CopyType=DEEP  ); 

  ~ListOfGenericGrid();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfGenericGrid& operator=(const ListOfGenericGrid&);
  ...
  void addElement( const GenericGrid & t, const int index ); // Add an object and reference to object t

  void addElement( const GenericGrid & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  GenericGrid& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const GenericGrid & X );		     // Find an element on the list and delete it.
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfGenericGrid &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfGenericGrid &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfGenericGrid(*this, ct); }
};
