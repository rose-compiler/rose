#ifndef LISTOFFLOATDISTRIBUTEDARRAY_H
#define LISTOFFLOATDISTRIBUTEDARRAY_H "ListOfFloatDistributedArray.h"
#include "A++.h"
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfFloatDistributedArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class floatArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          floatArray::reference( const floatArray & t ) 
//          floatArray::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class floatArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfFloatDistributedArray<floatArray> list,list2;
...
class ListOfFloatDistributedArray : public ReferenceCounting
{
  ...
  ListOfFloatDistributedArray(const ListOfFloatDistributedArray&, 
    const CopyType=DEEP  ); 

  ~ListOfFloatDistributedArray();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfFloatDistributedArray& operator=(const ListOfFloatDistributedArray&);
  ...
  void addElement( const floatArray & t, const int index ); // Add an object and reference to object t

  void addElement( const floatArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  floatArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const floatArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfFloatDistributedArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     floatArray **aList;		// Pointer to a list of pointers to object floatArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfFloatDistributedArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfFloatDistributedArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfFloatDistributedArray(*this, ct); }
};
