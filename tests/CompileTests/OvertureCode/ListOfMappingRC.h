#ifndef LISTOFMAPPINGRC_H
#define LISTOFMAPPINGRC_H "ListOfMappingRC.h"
class MappingRC;
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfMappingRC Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class MappingRC to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          MappingRC::reference( const MappingRC & t ) 
//          MappingRC::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class MappingRC. 
...
//
//     floatArray a(10),b(5);
//     ListOfMappingRC<floatArray> list,list2;
...
class ListOfMappingRC : public ReferenceCounting
{
  ...
  ListOfMappingRC(const ListOfMappingRC&, 
    const CopyType=DEEP  ); 

  ~ListOfMappingRC();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfMappingRC& operator=(const ListOfMappingRC&);
  ...
  void addElement( const MappingRC & t, const int index ); // Add an object and reference to object t

  void addElement( const MappingRC & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  MappingRC& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const MappingRC & X );		     // Find an element on the list and delete it.
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfMappingRC &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfMappingRC &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfMappingRC(*this, ct); }
};
