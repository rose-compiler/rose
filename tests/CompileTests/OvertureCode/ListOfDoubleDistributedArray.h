#ifndef LISTOFDOUBLEDISTRIBUTEDARRAY_H
#define LISTOFDOUBLEDISTRIBUTEDARRAY_H "ListOfDoubleDistributedArray.h"
#include "A++.h"
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfDoubleDistributedArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class doubleArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          doubleArray::reference( const doubleArray & t ) 
//          doubleArray::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class doubleArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfDoubleDistributedArray<floatArray> list,list2;
...
class ListOfDoubleDistributedArray : public ReferenceCounting
{
  ...
  ListOfDoubleDistributedArray(const ListOfDoubleDistributedArray&, 
    const CopyType=DEEP  ); 

  ~ListOfDoubleDistributedArray();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfDoubleDistributedArray& operator=(const ListOfDoubleDistributedArray&);
  ...
  void addElement( const doubleArray & t, const int index ); // Add an object and reference to object t

  void addElement( const doubleArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  doubleArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const doubleArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfDoubleDistributedArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     doubleArray **aList;		// Pointer to a list of pointers to object doubleArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfDoubleDistributedArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfDoubleDistributedArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfDoubleDistributedArray(*this, ct); }
};
