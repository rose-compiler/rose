...
//  kill(sj->other, 18);
 ...
 if (!p) {
     ...
     if ((cp = ((((((__extension__ ({ union { __typeof(jn->procs->status) __in; int __i; } __u; __u.__in = (jn->procs->status); __u.__i; }))) & 0x7f) == 0) ||
   (((signed char) ((((__extension__ ({ union { __typeof(jn->procs->status) __in; int __i; } __u; __u.__in = (jn->procs->status); __u.__i; }))) & 0x7f) + 1) >> 1) > 0)) &&
...
// jn->stat |= (0x0002);
 ...
 #if 0
     if (p->status == -1 ||
  (!((((__extension__ ({ union { __typeof(p->status) __in; int __i; } __u; __u.__in = (p->status); __u.__i; }))) & 0x7f) == 0) && !(((signed char) ((((__extension__ ({ union { __typeof(p->status) __in; int __i; } __u; __u.__in = (p->status); __u.__i; }))) & 0x7f) + 1) >> 1) > 0)))
...
check_cursh_sig(int sig)
{
 ...
 return;
    for (i = 1; i <= maxjob; i++) {
 if ((jobtab[i].stat & ((0x0400)|(0x0008))) ==
     (0x0400)) {
     for (j = 0; j < 2; j++) {
...
update_job(Job jn)
{
 ...
 if ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) == 0xffff))
     pn->status = -1;
 ...
 if (((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff) == 0x7f))
     somestopped = 1;
 if (!pn->next)
     val = ((((signed char) ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f) + 1) >> 1) > 0)) ? 0200 | (((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f) :
  ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff00) >> 8);
 ...
 inerrflush();
    ...
    if (somestopped && jn->stat & (0x0080))
 return;
    jn->stat |= (somestopped) ? (0x0001) | (0x0002) :
 (0x0001) | (0x0008);
 ...
 numpipestats = i;
    ...
    if (!inforeground &&
 (jn->stat & ((0x0100) | (0x0008))) == ((0x0100) | (0x0008))) {
 int su;

 if ((su = super_job(jn - jobtab)))
     handle_sub(su, 0);
    }
    if ((jn->stat & ((0x0008) | (0x0002))) == (0x0002)) {
 prevjob = curjob;
 curjob = job;
    }
    if (((opts[NOTIFY]) || job == thisjob) && (jn->stat & (0x0010))) {
 if (printjob(jn, !!(opts[LONGLISTJOBS]), 0) &&
     zleactive)
     zleentry(ZLE_CMD_REFRESH);
    }
    if (sigtrapped[17] && job != thisjob)
 dotrap(17);
...
setprevjob(void)
{
    ...
    for (i = maxjob; i; i--)
 if ((jobtab[i].stat & (0x0040)) && (jobtab[i].stat & (0x0002)) &&
     !(jobtab[i].stat & (0x0100)) && i != curjob && i != thisjob) {
     prevjob = i;
     return;
 }

    for (i = maxjob; i; i--)
 if ((jobtab[i].stat & (0x0040)) && !(jobtab[i].stat & (0x0100)) &&
     i != curjob && i != thisjob) {
     prevjob = i;
     return;
 }
...
// # 676 "jobs.c"
 ...
 s = "%J  %U user %S system %P cpu %*E total";
...
printjob(Job jn, int lng, int synch)
{
 ...
 lng = !!(opts[LONGLISTJOBS]);
    ...
    for (pn = jn->procs; pn; pn = pn->next) {
 if (jn->stat & (0x0080) &&
     jn->procs->status == -1 && !pn->next)
     pn->status = -1;
 if (pn->status != -1) {
     if ((((signed char) ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f) + 1) >> 1) > 0)) {
  sig = (((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f);
  llen = strlen(((sig) <= 31 ? sig_msg[sig] : "unknown signal"));
  if ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x80))
      llen += 14;
  if (llen > len)
      len = llen;
  if (sig != 2 && sig != 13)
      sflag = 1;
  if (job == thisjob && sig == 2)
      doputnl = 1;
  if ((opts[PRINTEXITVALUE]) && (opts[SHINSTDIN])) {
      sflag = 1;
      skip_print = 0;
  }
     } else if (((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff) == 0x7f)) {
  sig = ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff00) >> 8);
  if ((int)strlen(((sig) <= 31 ? sig_msg[sig] : "unknown signal")) > len)
      len = strlen(((sig) <= 31 ? sig_msg[sig] : "unknown signal"));
  if (job == thisjob && sig == 20)
      doputnl = 1;
     } else if ((opts[PRINTEXITVALUE]) && (opts[SHINSTDIN]) &&
         ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff00) >> 8)) {
  sflag = 1;
  skip_print = 0;
     }
 ...
 return 0;
    ...
    if (synch == 2 ||
 ((((opts[INTERACTIVE])) || synch) && ((opts[MONITOR])) &&
  ((jn->stat & (0x0002)) || sflag || job != thisjob))) {
 int len2, fline = 1;

 int plainfmt = (synch == 3) && (opts[POSIXJOBS]);
 ...
 for (pn = jn->procs; pn;) {
  ...
  else if (((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f) == 0)) {
      if (((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff00) >> 8))
   fprintf(fout, "exit %-4d%*s", ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff00) >> 8),
    len - 9 + 2, "");
      else
   fprintf(fout, "done%*s", len - 4 + 2, "");
  } else if (((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff) == 0x7f))
      fprintf(fout, "%-*s", len + 2,
       ((((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff00) >> 8)) <= 31 ? sig_msg[((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0xff00) >> 8)] : "unknown signal"));
  else if ((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x80))
      fprintf(fout, "%s (core dumped)%*s",
       (((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f)) <= 31 ? sig_msg[(((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f)] : "unknown signal"),
       (int)(len - 14 + 2 -
      strlen((((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f)) <= 31 ? sig_msg[(((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f)] : "unknown signal"))), "");
  else
      fprintf(fout, "%-*s", len + 2,
       (((((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f)) <= 31 ? sig_msg[(((__extension__ ({ union { __typeof(pn->status) __in; int __i; } __u; __u.__in = (pn->status); __u.__i; }))) & 0x7f)] : "unknown signal"));
     }
 ...
 fflush(fout);
    } else if (doputnl && ((opts[INTERACTIVE])) && !synch) {
 doneprint = 1;
 _IO_putc ('\n', fout);
 fflush(fout);
    }
    ...
    if ((lng & 4) || (((opts[INTERACTIVE])) && job == thisjob &&
...
freejob(Job jn, int deleting)
{
 ...
 jn = jobtab + job;
    ...
    if (maxjob == jn - jobtab) {
 while (maxjob && !(jobtab[maxjob].stat & (0x0040)))
     maxjob--;
    }
}
// # 1179 "jobs.c"
...
{
    int first = 1, q = queueing_enabled;


    do { queueing_enabled = 0; do { while (queue_front != queue_rear) { sigset_t oset; queue_front = (queue_front + 1) % 128; oset = signal_setmask(signal_mask_queue[queue_front]); zhandler(signal_queue[queue_front]); signal_setmask(oset); } } while (0); } while (0);
    signal_block(sigchld_mask);
    queue_traps(wait_cmd);
    while (!errflag && (kill(pid, 0) >= 0 || (*__errno_location ()) != 3)) {
 if (first)
     first = 0;
 else
     kill(pid, 18);
 ...
 if (last_signal != 17 && wait_cmd && last_signal >= 0 &&
     (sigtrapped[last_signal] & (1<<0))) {
...
zwaitjob(int job, int wait_cmd)
{
 ...
 while (!errflag && jn->stat &&
        !(jn->stat & (0x0008)) &&
        !(((opts[INTERACTIVE])) && (jn->stat & (0x0002)))) {
     signal_suspend(17, wait_cmd);
     if (last_signal != 17 && wait_cmd && last_signal >= 0 &&
  (sigtrapped[last_signal] & (1<<0)))
     {
...
clearjobtab(int monitor)
{
 ...
 if (monitor && !(opts[POSIXJOBS]) && jobtab[i].stat)
     oldmaxjob = i+1;
 else if (jobtab[i].stat & (0x0040))
     freejob(jobtab + i, 0);
    }
 ...
 if (thisjob != -1 && thisjob < oldmaxjob)
    ...
    memset(jobtab, 0, jobtabsize * sizeof(struct job));
    maxjob = 0;
// # 1433 "jobs.c"
    thisjob = initjob();
}
...
spawnjob(void)
{
    ...
    if (!subsh) {
 if (curjob == -1 || !(jobtab[curjob].stat & (0x0002))) {
     curjob = thisjob;
     setprevjob();
 } else if (prevjob == -1 || !(jobtab[prevjob].stat & (0x0002)))
     prevjob = thisjob;
 if (((opts[MONITOR])) && jobtab[thisjob].procs) {
...
scanjobs(void)
{
    ...
    for (i = 1; i <= maxjob; i++)
        if (jobtab[i].stat & (0x0001))
            printjob(jobtab + i, !!(opts[LONGLISTJOBS]), 1);
}
// # 1569 "jobs.c"
static int
isanum(char *s)
{
    while (*s == '-' || (typtab[((unsigned char)(*s))] & (1 << 0)))
 s++;
    return *s == '\0';
}
...
{
    if (curjob == thisjob ||
 (curjob != -1 && !(jobtab[curjob].stat & (0x0040)))) {
 curjob = prevjob;
 setprevjob();
 if (curjob == thisjob ||
     (curjob != -1 && !((jobtab[curjob].stat & (0x0040)) &&
...
{
    char *p, *q;
    size_t init_bytes = (50)*sizeof(struct job);
...
expandjobtab(void)
{
 ...
 return 0;
    ...
    memset(newjobtab + jobtabsize, 0, (50) * sizeof(struct job));
...
bin_fg(char *name, char **argv, Options ops, int func)
{
 ...
 return 0;
    ...
    if (func == 3) {
 lng = (((ops)->ind['l'] != 0)) ? 1 : (((ops)->ind['p'] != 0)) ? 2 : 0;
 if (((ops)->ind['d'] != 0))
     lng |= 4;
    } else {
 lng = !!(opts[LONGLISTJOBS]);
 ...
 scanjobs();
    ...
    if (func != 3 || (opts[MONITOR]) || !oldmaxjob)
 setcurjob();
 ...
 stopmsg = 2;
  ...
  if (func == 2 || func == 1 || func == 5) {


     if (curjob == -1 || (jobtab[curjob].stat & (0x0020))) {
  ...
  if (job != ignorejob && jobptr->stat) {
      if ((!((ops)->ind['r'] != 0) && !((ops)->ind['s'] != 0)) ||
   (((ops)->ind['r'] != 0) && ((ops)->ind['s'] != 0)) ||
   (((ops)->ind['r'] != 0) &&
    !(jobptr->stat & (0x0002))) ||
   (((ops)->ind['s'] != 0) && jobptr->stat & (0x0002)))
 ...
 case 4:
     if (func == 1)
  jobtab[job].stat |= (0x0800);
     if ((stopped = (jobtab[job].stat & (0x0002)))) {
  ...
  if (jobtab[job].pwd && strcmp(jobtab[job].pwd, pwd)) {
      FILE *fout = (func == 3 || !shout) ? stdout : shout;
      fprintf(fout, "(pwd : ");
  ...
  if (func != 4) {
      thisjob = job;
      if ((jobtab[job].stat & (0x0080)) &&
   ((!jobtab[job].procs->next ||
     (jobtab[job].stat & (0x2000)) ||
...
int
bin_kill(char *nam, char **argv, Options ops __attribute__((__unused__)), int func __attribute__((__unused__)))
{
    int sig = 15;
    int returnval = 0;
    ...
    if (*argv && **argv == '-') {
 if ((typtab[((unsigned char)((*argv)[1]))] & (1 << 0))) {
     char *endp;
  ...
  if (argv[1]) {
   ...
   if (signame == *argv) {
    ...
    if ((((signed char) ((((__extension__ ({ union { __typeof(sig) __in; int __i; } __u; __u.__in = (sig); __u.__i; }))) & 0x7f) + 1) >> 1) > 0))
        sig = (((__extension__ ({ union { __typeof(sig) __in; int __i; } __u; __u.__in = (sig); __u.__i; }))) & 0x7f);
    else if (((((__extension__ ({ union { __typeof(sig) __in; int __i; } __u; __u.__in = (sig); __u.__i; }))) & 0xff) == 0x7f))
        sig = ((((__extension__ ({ union { __typeof(sig) __in; int __i; } __u; __u.__in = (sig); __u.__i; }))) & 0xff00) >> 8);
    if (1 <= sig && sig <= 31)
        printf("%s\n", sigs[sig]);
    else
        printf("%d\n", sig);
 ...
 if (**argv == '%') {
  ...
  zwarnnam("kill", "kill %s failed: %e", *argv, (*__errno_location ()));
  returnval++;
  continue;
     }
     ...
     if (jobtab[p].stat & (0x0002)) {





  if (sig != 9 && sig != 18 && sig != 20
      && sig != 22 && sig != 21 && sig != 19)
      killjb(jobtab + p, 18);
     }
 } else if (!isanum(*argv)) {
...
getsignum(const char *s)
{
    ...
    x = atoi(s);
    if ((typtab[((unsigned char)(*s))] & (1 << 0)) && x >= 0 && x < (31 +3))
 return x;
...
int
bin_suspend(char *name, char **argv __attribute__((__unused__)), Options ops, int func __attribute__((__unused__)))
{

    if (((opts[LOGINSHELL])) && !((ops)->ind['f'] != 0)) {
...
findjobnam(const char *s)
{
 ...
 if (!(jobtab[jobnum].stat & ((0x0100) | (0x0020))) &&
