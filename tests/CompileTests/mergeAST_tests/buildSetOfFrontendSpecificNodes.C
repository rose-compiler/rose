...
getSetOfFrontendSpecificNodes()
   {
  // Note that this function builds two list and uses set difference to 
...
#if 0
     printf ("At base of getSetOfFrontendSpecificNodes(): t.nodeList = %ld \n",t.nodeList.size());
     return t.nodeList;
#else
     printf ("Calling compute set difference() to subtract the required IR nodes (t.nodeList = %ld) \n",t.nodeList.size());
  ...
  // Computer the set difference between the nodes to delete and the nodes that are required!
     ...
     printf ("At base of getSetOfFrontendSpecificNodes(): finalList = %ld \n",finalList.size());
     return finalList;
#endif
   }
