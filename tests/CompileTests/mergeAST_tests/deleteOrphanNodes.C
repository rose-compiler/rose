...
deleteOrphanIRnodesInMemoryPool()
  ...
  // Others are build by the copy mechanism.
     ...
     printf ("In deleteOrphanIRnodesInMemoryPool(): t.parentList.size() = %ld t.childList.size() = %ld \n",t.parentList.size(),t.childList.size());
     unsigned long size = t.parentList.size() > t.childList.size() ? t.parentList.size() : t.childList.size();
  ...
  // std::vector<SgNode*>::iterator end = set_symmetric_difference(t.parentList.begin(),t.parentList.end(),t.childList.begin(),t.childList.end(),orphanList.begin());
     ...
     printf ("In deleteOrphanIRnodesInMemoryPool(): deleting orphanList size = %ld \n",orphanList.size());
