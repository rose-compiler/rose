...
#if 0
                              printf ("Insert symbol %p into symbol table %p in scope = %p \n",sharedFunctionSymbol,sharedFunctionScope->get_symbol_table(),sharedFunctionScope);
#endif
...
#if 0
                              printf ("Removing symbol = %p = %s = %s from symbol table %p in scope = %p = %s \n",symbol,symbol->class_name().c_str(),SageInterface::get_name(symbol).c_str(),scopeSymbolTable,scopeStatement,scopeStatement->class_name().c_str());
#endif
                              scopeSymbolTable->remove(symbol);
                            }
                      ...
                      // SgSymbol* sharedSymbol = sharedClassScope->lookup_class_symbol(sharedClassDeclaration->get_name());
                         ...
                         if (sharedSymbol != NULL)
                           ...
                           // parent set to the symbol table that is eliminated).
                            ...
                            {
                              printf ("Note that sharedClassScope->lookup_class_symbol(sharedClassDeclaration->get_name()) == NULL \n");
                            }
          ...
          case V_SgEnumDeclaration:
            ...
            // if (matchingNodeInMergedAST != NULL)
               if (matchingNodeInMergedAST != NULL && isSgEnumDeclaration(matchingNodeInMergedAST)->get_scope() != scopeStatement)
                 ...
                 // into place when the second symbol is evaluated (becuase the sharing via name mangling 
