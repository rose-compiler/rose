...
// # 81 "src/event/ngx_event.c"
static ngx_command_t ngx_events_commands[] = {

    { { sizeof("events") - 1, (u_char *) "events" },
      0x01000000|0x00000100|0x00000001,
      ngx_events_block,
      0,
      0,
      ((void *)0) },
...
static ngx_str_t event_core_name = { sizeof("event_core") - 1, (u_char *) "event_core" };


static ngx_command_t ngx_event_core_commands[] = {

    { { sizeof("worker_connections") - 1, (u_char *) "worker_connections" },
    ...
    { { sizeof("connections") - 1, (u_char *) "connections" },
      0x02000000|0x00000002,
      ngx_event_connections,
      0,
      0,
    ...
    { { sizeof("multi_accept") - 1, (u_char *) "multi_accept" },
      0x02000000|0x00000200,
      ngx_conf_set_flag_slot,
      0,
      __builtin_offsetof (ngx_event_conf_t, multi_accept),
      ((void *)0) },

    { { sizeof("accept_mutex") - 1, (u_char *) "accept_mutex" },
      0x02000000|0x00000200,
      ngx_conf_set_flag_slot,
      0,
      __builtin_offsetof (ngx_event_conf_t, accept_mutex),
      ((void *)0) },

    { { sizeof("accept_mutex_delay") - 1, (u_char *) "accept_mutex_delay" },
      0x02000000|0x00000002,
      ngx_conf_set_msec_slot,
      0,
      __builtin_offsetof (ngx_event_conf_t, accept_mutex_delay),
      ((void *)0) },

    { { sizeof("debug_connection") - 1, (u_char *) "debug_connection" },
      0x02000000|0x00000002,
...
// # 221 "src/event/ngx_event.c"
    ...
    if (ngx_use_accept_mutex) {
        if (ngx_accept_disabled > 0) {
            ...
            if (ngx_trylock_accept_mutex(cycle) == -1) {
                return;
            }
...
ngx_handle_read_event(ngx_event_t *rev, ngx_uint_t flags)
{
    if (ngx_event_flags & 0x00000004) {
        ...
        if (!rev->active && !rev->ready) {
            if (ngx_event_actions.add(rev, EPOLLIN, EPOLLET)
                == -1)
            {
                return -1;
            }
    ...
    } else if (ngx_event_flags & 0x00000001) {



        if (!rev->active && !rev->ready) {
            if (ngx_event_actions.add(rev, EPOLLIN, 0)
                == -1)
            {
                return -1;
            }
        ...
        if (rev->active && (rev->ready || (flags & 1))) {
            if (ngx_event_actions.del(rev, EPOLLIN, 0 | flags)
                == -1)
            {
                return -1;
            }
    ...
    } else if (ngx_event_flags & 0x00001000) {



        if (!rev->active && !rev->ready) {
            if (ngx_event_actions.add(rev, EPOLLIN, 0) == -1) {
                return -1;
            }
        ...
        if (rev->oneshot && !rev->ready) {
            if (ngx_event_actions.del(rev, EPOLLIN, 0) == -1) {
                return -1;
            }
...
ngx_handle_write_event(ngx_event_t *wev, size_t lowat)
{
    ...
    if (ngx_event_flags & 0x00000004) {
        ...
        if (!wev->active && !wev->ready) {
            if (ngx_event_actions.add(wev, EPOLLOUT,
                              EPOLLET | (lowat ? 0 : 0))
                == -1)
            {
                return -1;
        ...
        if (!wev->active && !wev->ready) {
            if (ngx_event_actions.add(wev, EPOLLOUT, 0)
                == -1)
            {
                return -1;
            }
        ...
        if (wev->active && wev->ready) {
            if (ngx_event_actions.del(wev, EPOLLOUT, 0)
                == -1)
            {
                return -1;
            }
        ...
        if (!wev->active && !wev->ready) {
            if (ngx_event_actions.add(wev, EPOLLOUT, 0) == -1) {
                return -1;
            }
        ...
        if (wev->oneshot && wev->ready) {
            if (ngx_event_actions.del(wev, EPOLLOUT, 0) == -1) {
                return -1;
            }
...
{
    if (cycle->conf_ctx[ngx_events_module.index] == ((void *)0)) {
        if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "no \"events\" section in configuration");
...
ngx_event_module_init(ngx_cycle_t *cycle)
{
    ...
    if (!ngx_test_config && ngx_process <= 1) {
        if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "using the \"%s\" event method", ecf->name);
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlmt) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "getrlimit(RLIMIT_NOFILE) failed, ignored");
        ...
        {
            limit = (ccf->rlimit_nofile == -1) ?
                         (ngx_int_t) rlmt.rlim_cur : ccf->rlimit_nofile;

            if ((cycle->log)->log_level >= 5) ngx_log_error_core(5, cycle->log, 0, "%ui worker_connections exceed " "open file resource limit: %i", ecf->connections, limit);
...
// # 518 "src/event/ngx_event.c"
    ...
    (void) __sync_bool_compare_and_swap(ngx_connection_counter, 0, 1);
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    ccf = (ngx_core_conf_t *) cycle->conf_ctx[ngx_core_module.index];
    ecf = (*(cycle->conf_ctx[ngx_events_module.index])) [ngx_event_core_module.ctx_index];;
...
// # 613 "src/event/ngx_event.c"
    if (ngx_event_timer_init(cycle->log) == -1) {
        return -1;
    }

    for (m = 0; ngx_modules[m]; m++) {
        if (ngx_modules[m]->type != 0x544E5645) {
            continue;
        }
        ...
        if (module->actions.init(cycle, ngx_timer_resolution) != 0) {
    ...
    if (ngx_timer_resolution && !(ngx_event_flags & 0x00000800)) {
        struct sigaction sa;
        ...
        if (sigaction(14, &sa, ((void *)0)) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "sigaction(SIGALRM) failed");
        ...
        if (setitimer(ITIMER_REAL, &itv, ((void *)0)) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setitimer() failed");

        }
    }

    if (ngx_event_flags & 0x00000400) {
        struct rlimit rlmt;

        if (getrlimit(RLIMIT_NOFILE, &rlmt) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "getrlimit(RLIMIT_NOFILE) failed");
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (!(ngx_event_flags & 0x00000200)) {
            if (ls[i].previous) {
                ...
                if (ngx_event_actions.del(old->read, EPOLLIN, 1)
                    == -1)
                {
                    return -1;
                }
    ...
    *(void **) conf = ctx;
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        if (m->create_conf) {
            ...
            if ((*ctx)[ngx_modules[i]->ctx_index] == ((void *)0)) {
                return (void *) -1;
            }
        }
    }
    ...
    pcf = *cf;
    cf->ctx = ctx;
    cf->module_type = 0x544E5645;
    cf->cmd_type = 0x02000000;
...
ngx_event_connections(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ecf->connections != (ngx_uint_t) -1) {
        return "is duplicate";
    }

    if (strcmp((const char *) cmd->name.data, (const char *) "connections") == 0) {
...
ngx_event_use(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cf->cycle->old_cycle->conf_ctx) {
        old_ecf = (*(cf->cycle->old_cycle->conf_ctx[ngx_events_module.index])) [ngx_event_core_module.ctx_index];;
    ...
    for (m = 0; ngx_modules[m]; m++) {
        ...
        if (module->name->len == value[1].len) {
            if (strcmp((const char *) module->name->data, (const char *) value[1].data) == 0) {
...
// # 1258 "src/event/ngx_event.c"
    if (module == ((void *)0)) {
        for (i = 0; ngx_modules[i]; i++) {
            ...
            if (strcmp((const char *) event_module->name->data, (const char *) event_core_name.data) == 0)
            {
                continue;
            }
    ...
    if (module == ((void *)0)) {
        if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "no events module found");
        return (void *) -1;
    }

    if (ecf->connections == (ngx_uint_t) -1) { ecf->connections = 512; };
    cycle->connection_n = ecf->connections;

    if (ecf->use == (ngx_uint_t) -1) { ecf->use = module->ctx_index; };

    event_module = module->ctx;
    if (ecf->name == (void *) -1) { ecf->name = event_module->name->data; };

    if (ecf->multi_accept == -1) { ecf->multi_accept = 0; };
    if (ecf->accept_mutex == -1) { ecf->accept_mutex = 1; };
    if (ecf->accept_mutex_delay == (ngx_msec_t) -1) { ecf->accept_mutex_delay = 500; };
// # 1318 "src/event/ngx_event.c"
    return ((void *)0);
