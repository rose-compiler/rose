...
// # 10 "src/core/nginx.c" 2
// # 1 "src/core/nginx.h" 1
// # 11 "src/core/nginx.c" 2
...
static ngx_command_t ngx_core_commands[] = {
    ...
    { { sizeof("daemon") - 1, (u_char *) "daemon" },
      0x01000000|0x00010000|0x00000200,
      ngx_conf_set_flag_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, daemon),
      ((void *)0) },

    { { sizeof("master_process") - 1, (u_char *) "master_process" },
      0x01000000|0x00010000|0x00000200,
      ngx_conf_set_flag_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, master),
      ((void *)0) },

    { { sizeof("timer_resolution") - 1, (u_char *) "timer_resolution" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_msec_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, timer_resolution),
      ((void *)0) },

    { { sizeof("pid") - 1, (u_char *) "pid" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_str_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, pid),
      ((void *)0) },

    { { sizeof("lock_file") - 1, (u_char *) "lock_file" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_str_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, lock_file),
      ((void *)0) },

    { { sizeof("worker_processes") - 1, (u_char *) "worker_processes" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_num_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, worker_processes),
      ((void *)0) },

    { { sizeof("debug_points") - 1, (u_char *) "debug_points" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_enum_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, debug_points),
      &ngx_debug_points },

    { { sizeof("user") - 1, (u_char *) "user" },
      0x01000000|0x00010000|(0x00000002|0x00000004),
      ngx_set_user,
      0,
      0,
      ((void *)0) },

    { { sizeof("worker_priority") - 1, (u_char *) "worker_priority" },
      0x01000000|0x00010000|0x00000002,
      ngx_set_priority,
      0,
      0,
    ...
    { { sizeof("worker_cpu_affinity") - 1, (u_char *) "worker_cpu_affinity" },
      0x01000000|0x00010000|0x00000800,
      ngx_set_cpu_affinity,
      0,
      0,
      ((void *)0) },

    { { sizeof("worker_rlimit_nofile") - 1, (u_char *) "worker_rlimit_nofile" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_num_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, rlimit_nofile),
      ((void *)0) },

    { { sizeof("worker_rlimit_core") - 1, (u_char *) "worker_rlimit_core" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_off_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, rlimit_core),
      ((void *)0) },

    { { sizeof("worker_rlimit_sigpending") - 1, (u_char *) "worker_rlimit_sigpending" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_num_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, rlimit_sigpending),
      ((void *)0) },

    { { sizeof("working_directory") - 1, (u_char *) "working_directory" },
      0x01000000|0x00010000|0x00000002,
      ngx_conf_set_str_slot,
      0,
      __builtin_offsetof (ngx_core_conf_t, working_directory),
      ((void *)0) },

    { { sizeof("env") - 1, (u_char *) "env" },
      0x01000000|0x00010000|0x00000002,
      ngx_set_env,
      0,
      0,
      ((void *)0) },
// # 158 "src/core/nginx.c"
      { { 0, ((void *)0) }, 0, ((void *)0), 0, 0, ((void *)0) }
};


static ngx_core_module_t ngx_core_module_ctx = {
    { sizeof("core") - 1, (u_char *) "core" },
...
main(int argc, char *const *argv)
{
    ...
    if (ngx_show_version) {
        ngx_write_stderr("nginx version: " "nginx/" "1.2.3" "\x0a");

        if (ngx_show_help) {
            ngx_write_stderr(
                "Usage: nginx [-?hvVtq] [-s signal] [-c filename] "
                             "[-p prefix] [-g directives]" "\x0a"
                             "\x0a"
                "Options:" "\x0a"
                "  -?,-h         : this help" "\x0a"
                ...
                "  -V            : show version and configure options then exit"
                                   "\x0a"
                "  -t            : test configuration and exit" "\x0a"
                "  -q            : suppress non-error messages "
                                   "during configuration testing" "\x0a"
                "  -s signal     : send signal to a master process: "
                                   "stop, quit, reopen, reload" "\x0a"
        ...
        if (ngx_show_configure) {
            ngx_write_stderr(
// # 260 "src/core/nginx.c"
                "configure arguments:" "" "\x0a");
        }
...
// # 293 "src/core/nginx.c"
    ...
    if (cycle->log->file->fd != 2) {

        if (dup2(cycle->log->file->fd, 2) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, (*__errno_location ()), "dup2(STDERR_FILENO)" " failed");
    ...
    if (log->file->fd != 2) {
        if (close(log->file->fd) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "close()" " built-in log failed");
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    inherited = (u_char *) getenv("NGINX");

    if (inherited == ((void *)0)) {
        return 0;
    }

    if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "using inherited sockets from \"%s\"", inherited);
    ...
    for (p = inherited, v = p; *p; p++) {
        if (*p == ':' || *p == ';') {
            s = ngx_atoi(v, p - v);
            if (s == -1) {
                if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "invalid socket number \"%s\" in " "NGINX" " environment variable, ignoring the rest" " of the variable", v);
...
ngx_set_environment(ngx_cycle_t *cycle, ngx_uint_t *last)
{
    ...
    for (i = 0; i < ccf->env.nelts; i++) {
        if (strcmp((const char *) var[i].data, (const char *) "TZ") == 0
            || strncmp((const char *) var[i].data, (const char *) "TZ=", 3) == 0)
        {
            goto tz_found;
        }
    }
...
tz_found:
    ...
    for (i = 0; i < ccf->env.nelts; i++) {
        ...
        for (p = ngx_os_environ; *p; p++) {
            ...
            if (strncmp((const char *) *p, (const char *) var[i].data, var[i].len) == 0
                && (*p)[var[i].len] == '=')
            {
                n++;
                break;
...
ngx_exec_new_binary(ngx_cycle_t *cycle, char *const *argv)
{
    ...
    (void) memset(&ctx, 0, sizeof(ngx_exec_ctx_t));
    ...
    var = ngx_alloc(sizeof("NGINX")
                    + cycle->listening.nelts * (sizeof("-2147483648") - 1 + 1) + 2,
                    cycle->log);

    p = (((u_char *) memcpy(var, "NGINX" "=", sizeof("NGINX"))) + (sizeof("NGINX")));
    ...
    *p = '\0';
    ...
    env[n] = ((void *)0);
// # 630 "src/core/nginx.c"
    ctx.envp = (char *const *) env;

    ccf = (ngx_core_conf_t *) cycle->conf_ctx[ngx_core_module.index];

    if (rename((const char *) ccf->pid.data, (const char *) ccf->oldpid.data) != 0) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "rename()" " %s to %s failed " "before executing new binary process \"%s\"", ccf->pid.data, ccf->oldpid.data, argv[0]);
    ...
    if (pid == -1) {
        if (rename((const char *) ccf->oldpid.data, (const char *) ccf->pid.data) != 0) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "rename()" " %s back to %s failed after " "an attempt to execute new binary process \"%s\"", ccf->oldpid.data, ccf->pid.data, argv[0]);
...
ngx_process_options(ngx_cycle_t *cycle)
{
    ...
    if (ngx_prefix) {
        len = strlen((const char *) ngx_prefix);
        p = ngx_prefix;
...
// # 880 "src/core/nginx.c"
        (&cycle->conf_prefix)->len = sizeof("conf/") - 1; (&cycle->conf_prefix)->data = (u_char *) "conf/";



        (&cycle->prefix)->len = sizeof("/usr/local/nginx/") - 1; (&cycle->prefix)->data = (u_char *) "/usr/local/nginx/";


    }

    if (ngx_conf_file) {
        cycle->conf_file.len = strlen((const char *) ngx_conf_file);
        cycle->conf_file.data = ngx_conf_file;

    } else {
        (&cycle->conf_file)->len = sizeof("conf/nginx.conf") - 1; (&cycle->conf_file)->data = (u_char *) "conf/nginx.conf";
    }

    if (ngx_conf_full_name(cycle, &cycle->conf_file, 0) != 0) {
        return -1;
    }
    ...
    if (ngx_conf_params) {
        cycle->conf_param.len = strlen((const char *) ngx_conf_params);
        cycle->conf_param.data = ngx_conf_params;
    }

    if (ngx_test_config) {
        cycle->log->log_level = 7;
    }
...
ngx_core_module_init_conf(ngx_cycle_t *cycle, void *conf)
{
    ...
    if (ccf->daemon == -1) { ccf->daemon = 1; };
    if (ccf->master == -1) { ccf->master = 1; };
    if (ccf->timer_resolution == (ngx_msec_t) -1) { ccf->timer_resolution = 0; };

    if (ccf->worker_processes == -1) { ccf->worker_processes = 1; };
    if (ccf->debug_points == -1) { ccf->debug_points = 0; };
    ...
    {
        if ((cycle->log)->log_level >= 5) ngx_log_error_core(5, cycle->log, 0, "the number of \"worker_processes\" is not equal to " "the number of \"worker_cpu_affinity\" masks, " "using last mask for remaining worker processes");



    }
// # 1009 "src/core/nginx.c"
    if (ccf->pid.len == 0) {
        (&ccf->pid)->len = sizeof("logs/nginx.pid") - 1; (&ccf->pid)->data = (u_char *) "logs/nginx.pid";
    }

    if (ngx_conf_full_name(cycle, &ccf->pid, 0) != 0) {
        return (void *) -1;
    }
    ...
    if (ccf->oldpid.data == ((void *)0)) {
        return (void *) -1;
    }

    (void) memcpy((((u_char *) memcpy(ccf->oldpid.data, ccf->pid.data, ccf->pid.len)) + (ccf->pid.len)), ".oldbin", sizeof(".oldbin"));





    if (ccf->user == (uid_t) (ngx_uint_t) -1 && geteuid() == 0) {
        struct group *grp;
        struct passwd *pwd;
        ...
        pwd = getpwnam("nobody");
        if (pwd == ((void *)0)) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, (*__errno_location ()), "getpwnam(\"" "nobody" "\") failed");
        ...
        grp = getgrnam("nobody");
        if (grp == ((void *)0)) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, (*__errno_location ()), "getgrnam(\"" "nobody" "\") failed");
    ...
    if (ccf->lock_file.len == 0) {
        (&ccf->lock_file)->len = sizeof("logs/nginx.lock") - 1; (&ccf->lock_file)->data = (u_char *) "logs/nginx.lock";
    }

    if (ngx_conf_full_name(cycle, &ccf->lock_file, 0) != 0) {
        return (void *) -1;
    }
    ...
    if (lock_file.len) {
        ...
        if (ccf->lock_file.len != lock_file.len
            || strncmp((const char *) ccf->lock_file.data, (const char *) lock_file.data, lock_file.len)
               != 0)
        {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, 0, "\"lock_file\" could not be changed, ignored");
        ...
        if (cycle->lock_file.data == ((void *)0)) {
            return (void *) -1;
        }
        ...
        (void) memcpy((((u_char *) memcpy(cycle->lock_file.data, ccf->lock_file.data, ccf->lock_file.len)) + (ccf->lock_file.len)), ".accept", sizeof(".accept"));
...
// # 1121 "src/core/nginx.c"
    ...
    if (ccf->user != (uid_t) (ngx_uint_t) -1) {
        return "is duplicate";
    }
    ...
    if (pwd == ((void *)0)) {
        ngx_conf_log_error(1, cf, (*__errno_location ()),
