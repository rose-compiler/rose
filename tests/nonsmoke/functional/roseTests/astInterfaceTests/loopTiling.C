...

{
  ...
  if (!CommandlineProcessing::isOptionWithParameter (argvList,"-rose:loopTiling:","abstract_handle",handle, true)
   ...
   {
     cout<<"Usage: loopTiling inputFile.c -rose:loopTiling:abstract_handle <loop_handle_string> -rose:loopTiling:depth D -rose:loopTiling:tilesize N"<<endl;
     return 0;
   }
 ...
 if (isSgForStatement(stmt))
  {
    bool result=false;
    result = SageInterface::loopTiling(isSgForStatement(stmt),depth, tilesize);
