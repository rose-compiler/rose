...
// # 598 "zftp.c"
 ...
 switch(ch) {
 case (-1):
     if (ferror(zfsess->cin) && (*__errno_location ()) == 4) {
  clearerr(zfsess->cin);
  continue;
     }
     zcfinish = 2;
...
zfgetmsg(void)
{
 ...
 return 6;
    ...
    ptr = line;
    if (zfdrrrring || !(typtab[((unsigned char)(*ptr))] & (1 << 0)) || !(typtab[((unsigned char)(ptr[1]))] & (1 << 0)) || !(typtab[((unsigned char)(ptr[2]))] & (1 << 0))) {
...
zfsendcmd(char *cmd)
{
 ...
 return 6;
    ...
    if (ret <= 0) {
 zwarnnam("zftp send", "failure sending control message: %e", (*__errno_location ()));
 return 6;
    }
...
zfopendata(char *name, union tcp_sockaddr *zdsockp, int *is_passivep)
{
 ...
 return 1;
    ...
    if (zfsess->dfd < 0) {
 zwarnnam(name, "can't get data socket: %e", (*__errno_location ()));
 return 1;
    }
 ...
 if(zfsess->control->peer.a.sa_family == 10)
     psv_cmd = "EPSV\r\n";
 else
 ...
 if(zfsess->control->peer.a.sa_family == 10) {
  ...
  goto bad_epsv;
     while(ptr != end && *ptr == '0')
  ptr++;
     if(ptr == end || (end-ptr) > 5 || !(typtab[((unsigned char)(*ptr))] & (1 << 0)))
  goto bad_epsv;
...

 {
  ...
  if ((typtab[((unsigned char)(*ptr))] & (1 << 0)))
      break;
 ...
 } while (err && (*__errno_location ()) == 4 && !errflag);

 if (err) {
     zwarnnam(name, "connect failed: %e", (*__errno_location ()));
     zfclosedata();
     return 1;
 }
 ...
 *zdsockp = zfsess->control->sock;
 ...
 if(zdsockp->a.sa_family == 10) {
     zdsockp->in6.sin6_port = 0;
     len = sizeof(struct sockaddr_in6);
 } else

 {
     zdsockp->in.sin_port = 0;
 ...
 if(zdsockp->a.sa_family == 10) {
     ...
     inet_ntop(10, &zdsockp->in6.sin6_addr,
     portcmd+8, 46);
...
// # 1085 "zftp.c"
 ...
 if (ptr) {
     while (ptr > lastmsg && !(typtab[((unsigned char)(*ptr))] & (1 << 0)))
  ptr--;
     while (ptr > lastmsg && (typtab[((unsigned char)(ptr[-1]))] & (1 << 0)))
  ptr--;
     if ((typtab[((unsigned char)(*ptr))] & (1 << 0))) {
  zfstatusp[zfsessno] &= ~ZFST_NOSZ;
  if (getsize) {
      off_t sz = zstrtol(ptr, ((void *)0), 10);
...
// # 1128 "zftp.c"
 ...
 newfd = zfmovefd(accept(zfsess->dfd, (struct sockaddr *)&zdsock,
    &len));
 if (newfd < 0)
     zwarnnam(name, "unable to accept data: %e", (*__errno_location ()));
 zfclosedata();
 if (newfd < 0)
     return 1;
 zfsess->dfd = newfd;
...
// # 1157 "zftp.c"
 ...
 setsockopt(zfsess->dfd, 1, 13,
    ...
    {
 int arg = 0x08;
 setsockopt(zfsess->dfd, IPPROTO_IP, 1, (char *)&arg, sizeof(arg));
    }



    fcntl(zfsess->dfd, 2, 1);
 ...
 *retmdtm = ((void *)0);
 ...
 zfsettype((zfstatusp[zfsessno] & ZFST_TMSK));
 if (retsize) {
 ...
 if (retmdtm) {
     ...
     tm = gmtime(&statbuf.st_mtim.tv_sec);
...
zfread_block(int fd, char *bf, off_t sz, int tmout)
{
 ...
 while (cnt) {
  ...
  bfptr += n;
  cnt -= n;
     } else if (n < 0 && (errflag || zfdrrrring || (*__errno_location ()) != 4))
  return n;
     else
  break;
 }
...
zfsenddata(char *name, int recv, int progress, off_t startat)
{
    ...
    char lsbuf[32768], *ascbuf = ((void *)0), *optr;
 ...
 if (((zfstatusp[zfsessno] >> ZFST_TBIT) & ZFST_TMSK) == ZFST_ASCI)
     fromasc = 1;
 if ((zfstatusp[zfsessno] & ZFST_MMSK) == ZFST_BLOC)
 ...
 if ((zfstatusp[zfsessno] & ZFST_MMSK) == ZFST_BLOC)
    ...
    if (toasc)
 ascbuf = zalloc((32768/2));
    zfpipe();
    zfread_eof = 0;
    while (!ret && !zfread_eof) {
 n = (toasc) ? read_ptr(fdin, ascbuf, (32768/2), rtmout)
     : read_ptr(fdin, lsbuf, 32768, rtmout);
 if (n > 0) {
     char *iptr;
     if (toasc) {
  ...
  if (newn < 0) {
// # 1585 "zftp.c"
      if ((*__errno_location ()) != 4 || errflag || zfdrrrring) {
   if (!zfdrrrring &&
       (!((opts[INTERACTIVE])) || (!errflag && (*__errno_location ()) != 32))) {
       ret = recv ? 2 : 1;
       zwarnnam(name, "write failed: %e", (*__errno_location ()));
   } else
       ret = recv ? 3 : 1;
   break;
      }
  ...
  if (!zfdrrrring &&
      (!((opts[INTERACTIVE])) || (!errflag && (*__errno_location ()) != 32))) {
      ret = recv ? 1 : 2;
      zwarnnam(name, "read failed: %e", (*__errno_location ()));
  } else
      ret = recv ? 1 : 3;
  break;
     }
 ...
 if (!ret && sofar != last_sofar && progress &&
    ...
    zfdrrrring = 0;
    if (!errflag && !ret && !recv &&
 (zfstatusp[zfsessno] & ZFST_MMSK) == ZFST_BLOC) {

 ret = (zfwrite_block(fdout, lsbuf, 0, wtmout) < 0);
    }
    if (errflag || ret > 1) {
// # 1649 "zftp.c"
 unsigned char msg[4] = { 255, 244, 255, 242 };
 ...
 *tmpptr++ = '\0';
 ...
 zfsess->control = tcp_socket(af, SOCK_STREAM, 0, 16);

 if (!(zfsess->control) || (zfsess->control->fd < 0)) {
     if (zfsess->control) {
  tcp_close(zfsess->control);
  zfsess->control = ((void *)0);
     }
     freehostent(zhostp);
     zfunsetparam("ZFTP_HOST");
     zfunsetparam("ZFTP_PORT");
     if(af == 2) { } else continue;
     zwarnnam(name, "socket failed: %e", (*__errno_location ()));
     alarm(0);
     return 1;
 }
 ...
 return 1;
    ...
    len = 1;
    setsockopt(zfsess->control->fd, 1, 10,
        (char *)&len, sizeof(len));



    len = 0x10;
    setsockopt(zfsess->control->fd, IPPROTO_IP, 1, (char *)&len, sizeof(len));
 ...
 return 1;
    ...
    if (zfstatfd == -1) {
 zfstatfd = gettempfile(((void *)0), 1, &fname);
 ;
...
// # 2031 "zftp.c"
 ...
 fflush(stderr);
    ...
    if (fgets(instr, 256, stdin) == ((void *)0))
 instr[len = 0] = '\0';
...
static int
zftp_params(char *name __attribute__((__unused__)), char **args, int flags __attribute__((__unused__)))
{
...
static int
zftp_login(char *name, char **args, int flags __attribute__((__unused__)))
{
    char *ucmd, *passwd = ((void *)0), *acct = ((void *)0);
    char *user, tbuf[2] = "X";
    int stopit;
 ...
 zfstatusp[zfsessno] |= ZFST_SYST;
    ...
    tbuf[0] = ((zfstatusp[zfsessno] & ZFST_TMSK) == ZFST_ASCI) ? 'A' : 'I';
...
// # 2257 "zftp.c"
static int
zftp_test(char *name __attribute__((__unused__)), char **args __attribute__((__unused__)), int flags __attribute__((__unused__)))
{
 ...
 return 1;
    ...
    pfd.events = 0x001;
    if ((ret = poll(&pfd, 1, 0)) < 0 && (*__errno_location ()) != 4 && (*__errno_location ()) != 11)
 zfclose(0);
...
zftp_dir(char *name, char **args, int flags)
{
    ...
    ret = zfgetdata(name, ((void *)0), cmd, 0);
    zsfree(cmd);
    if (ret)
 return 1;
...
static int
zftp_cd(char *name __attribute__((__unused__)), char **args, int flags)
{
...
{
    char buf[] = "TYPE X\r\n";
    if ((type & ZFST_TMSK) == ((zfstatusp[zfsessno] >> ZFST_TBIT) & ZFST_TMSK))
 return 0;
    buf[5] = ((type & ZFST_TMSK) == ZFST_ASCI) ? 'A' : 'I';
    if (zfsendcmd(buf) > 2)
 return 1;
...
zftp_type(char *name, char **args, int flags)
{
 ...
 printf("%c\n", ((zfstatusp[zfsessno] & ZFST_TMSK) == ZFST_ASCI) ?
        'A' : 'I');
 fflush(stdout);
 return 0;
    } else {
...
static int
zftp_mode(char *name, char **args, int flags __attribute__((__unused__)))
{
    char *str, cmd[] = "MODE X\r\n";
    int nt;

    if (!(str = *args)) {
 printf("%c\n", ((zfstatusp[zfsessno] & ZFST_MMSK) == ZFST_STRE) ?
        'S' : 'B');
 fflush(stdout);
 return 0;
    }
...
static int
zftp_local(char *name __attribute__((__unused__)), char **args, int flags)
{
...
// # 2582 "zftp.c"
 ...
 if (flags & ZFTP_REST) {
     startat = zstrtol(args[1], ((void *)0), 10);
...
static int
zftp_delete(char *name __attribute__((__unused__)), char **args, int flags __attribute__((__unused__)))
{
    int ret = 0;
    char *cmd, **aptr;
    for (aptr = args; *aptr; aptr++) {
...
static int
zftp_mkdir(char *name __attribute__((__unused__)), char **args, int flags)
{
    int ret;
...
static int
zftp_rename(char *name __attribute__((__unused__)), char **args, int flags __attribute__((__unused__)))
{
    int ret;
    char *cmd;
...
static int
zftp_quote(char *name __attribute__((__unused__)), char **args, int flags)
{
    int ret = 0;
    char *cmd;
...
static int
zftp_close(char *name __attribute__((__unused__)), char **args __attribute__((__unused__)), int flags __attribute__((__unused__)))
{
    zfclose(0);
    return 0;
}
...
newsession(char *nm)
{
    ...
    for (zfsessno = 0, nptr = ((zfsessions)->list.first);
  nptr; zfsessno++, (nptr = ((nptr)->next))) {
 ...
 zinsertlinknode(zfsessions,((zfsessions)->list.last),zfsess);
...
static int
zftp_session(char *name __attribute__((__unused__)), char **args, int flags __attribute__((__unused__)))
{
    if (!*args) {
 LinkNode nptr;

 for (nptr = ((zfsessions)->list.first); nptr; (nptr = ((nptr)->next)))
...
static int
zftp_rmsession(char *name __attribute__((__unused__)), char **args, int flags __attribute__((__unused__)))
{
    int no;
    LinkNode nptr;
    Zftp_session sptr = ((void *)0);
    char *newsess = ((void *)0);


    for (no = 0, nptr = ((zfsessions)->list.first); nptr; no++, (nptr = ((nptr)->next))) {
 ...
 if (zfsesscnt > 1) {
     LinkNode newn = ((zfsessions)->list.first);
     if (newn == nptr)
  (newn = ((newn)->next));
...
static int
bin_zftp(char *name, char **args, Options ops __attribute__((__unused__)), int func __attribute__((__unused__)))
{
 ...
 if (zfsess->control && (zfstatusp[zfsessno] & ZFST_CLOS)) {
     ...
     if ((oldstatus & ZFST_TMSK) != (zfstatusp[zfsessno] & ZFST_TMSK))
  zfsetparam("ZFTP_TYPE",
      ztrdup((zfstatusp[zfsessno] & ZFST_TMSK) == ZFST_ASCI ?
      "A" : "I"), ZFPM_READONLY);
     if ((oldstatus & ZFST_MMSK) != (zfstatusp[zfsessno] & ZFST_MMSK))
  zfsetparam("ZFTP_MODE",
      ztrdup((zfstatusp[zfsessno] & ZFST_MMSK) == ZFST_BLOC ?
      "B" : "S"), ZFPM_READONLY);
 }
    }
 ...
 ret = zftp_test("zftp test", ((void *)0), 0);
    }
...
zftp_cleanup(void)
{
    ...
    for (zfsessno = 0, nptr = ((zfsessions)->list.first); nptr;
  zfsessno++, (nptr = ((nptr)->next))) {
 zfsess = (Zftp_session)nptr->dat;
 zfclosedata();
...
static int
zftpexithook(Hookdef d __attribute__((__unused__)), void *dummy __attribute__((__unused__)))
{
    zftp_cleanup();
    return 0;
}
...
int
setup_(Module m __attribute__((__unused__)))
{
    return (require_module("zsh/net/tcp", ((void *)0)) == 1);
}
...
int
boot_(Module m __attribute__((__unused__)))
{
...
cleanup_(Module m)
{
    ...
    zftp_cleanup();
    return setfeatureenables(m, &module_features, ((void *)0));
}


int
finish_(Module m __attribute__((__unused__)))
{
    return 0;
}
