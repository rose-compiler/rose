...
// # 11 "src/core/ngx_connection.c" 2
...
ngx_create_listening(ngx_conf_t *cf, void *sockaddr, socklen_t socklen)
{
    ...
    ngx_listening_t *ls;
    struct sockaddr *sa;
    u_char text[(sizeof("unix:") - 1 + (sizeof(struct sockaddr_un) - __builtin_offsetof (struct sockaddr_un, sun_path)))];
    ...
    (void) memset(ls, 0, sizeof(ngx_listening_t));
    ...
    (void) memcpy(sa, sockaddr, socklen);

    ls->sockaddr = sa;
    ls->socklen = socklen;

    len = ngx_sock_ntop(sa, text, (sizeof("unix:") - 1 + (sizeof(struct sockaddr_un) - __builtin_offsetof (struct sockaddr_un, sun_path))), 1);
    ...
    case 1:
         ls->addr_text_max_len = (sizeof(struct sockaddr_un) - __builtin_offsetof (struct sockaddr_un, sun_path));
         len++;
         break;

    case 2:
         ls->addr_text_max_len = (sizeof("255.255.255.255") - 1);
         break;
    default:
         ls->addr_text_max_len = (sizeof("unix:") - 1 + (sizeof(struct sockaddr_un) - __builtin_offsetof (struct sockaddr_un, sun_path)));
         break;
    }
    ...
    (void) memcpy(ls->addr_text.data, text, len);
...
ngx_set_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        ls[i].sockaddr = ngx_palloc(cycle->pool, sizeof(struct sockaddr_un));
        ...
        if (getsockname(ls[i].fd, ls[i].sockaddr, &ls[i].socklen) == -1) {
            if ((cycle->log)->log_level >= 3) ngx_log_error_core(3, cycle->log, (*__errno_location ()), "getsockname() of the inherited " "socket #%d failed", ls[i].fd);
...
// # 131 "src/core/ngx_connection.c"
        case 1:
             ls[i].addr_text_max_len = (sizeof(struct sockaddr_un) - __builtin_offsetof (struct sockaddr_un, sun_path));
             len = (sizeof(struct sockaddr_un) - __builtin_offsetof (struct sockaddr_un, sun_path));
             break;


        case 2:
             ls[i].addr_text_max_len = (sizeof("255.255.255.255") - 1);
             len = (sizeof("255.255.255.255") - 1) + sizeof(":65535") - 1;
             break;

        default:
            if ((cycle->log)->log_level >= 3) ngx_log_error_core(3, cycle->log, (*__errno_location ()), "the inherited socket #%d has " "an unsupported protocol family", ls[i].fd);
        ...
        if (getsockopt(ls[i].fd, 1, 8, (void *) &ls[i].rcvbuf,
                       &olen)
            == -1)
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "getsockopt(SO_RCVBUF) %V failed, ignored", &ls[i].addr_text);
        ...
        if (getsockopt(ls[i].fd, 1, 7, (void *) &ls[i].sndbuf,
                       &olen)
            == -1)
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "getsockopt(SO_SNDBUF) %V failed, ignored", &ls[i].addr_text);



            ls[i].sndbuf = -1;
        }
// # 244 "src/core/ngx_connection.c"
        timeout = 0;
        olen = sizeof(int);

        if (getsockopt(ls[i].fd, IPPROTO_TCP, 9, &timeout, &olen)
            == -1)
        {
            if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, (*__errno_location ()), "getsockopt(TCP_DEFER_ACCEPT) for %V failed, ignored", &ls[i].addr_text);
...
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            if (s == -1) {
                if ((log)->log_level >= 1) ngx_log_error_core(1, log, (*__errno_location ()), "socket()" " %V failed", &ls[i].addr_text);
            ...
            if (setsockopt(s, 1, 2,
                ...
                == -1)
            {
                if ((log)->log_level >= 1) ngx_log_error_core(1, log, (*__errno_location ()), "setsockopt(SO_REUSEADDR) %V failed", &ls[i].addr_text);



                if (close(s) == -1) {
                    if ((log)->log_level >= 1) ngx_log_error_core(1, log, (*__errno_location ()), "close() socket" " %V failed", &ls[i].addr_text);
...
// # 356 "src/core/ngx_connection.c"
            if (!(ngx_event_flags & 0x00000100)) {
                if (ngx_nonblocking(s) == -1) {
                    if ((log)->log_level >= 1) ngx_log_error_core(1, log, (*__errno_location ()), "ioctl(FIONBIO)" " %V failed", &ls[i].addr_text);
            ...
            if (bind(s, ls[i].sockaddr, ls[i].socklen) == -1) {
                ...
                if (err == 98 && ngx_test_config) {
                    continue;
                }

                if ((log)->log_level >= 1) ngx_log_error_core(1, log, err, "bind() to %V failed", &ls[i].addr_text);
            ...
            if (ls[i].sockaddr->sa_family == 1) {
                ...
                mode = (0400|0200|(0400 >> 3)|(0200 >> 3)|((0400 >> 3) >> 3)|((0200 >> 3) >> 3));

                if (chmod((char *) name, mode) == -1) {
                    if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, (*__errno_location ()), "chmod() \"%s\" failed", name);

                }

                if (ngx_test_config) {
                    if (unlink((const char *) name) == -1) {
                        if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, (*__errno_location ()), "unlink()" " %s failed", name);
            ...
            if (listen(s, ls[i].backlog) == -1) {
                if ((log)->log_level >= 1) ngx_log_error_core(1, log, (*__errno_location ()), "listen() to %V, backlog %d failed", &ls[i].addr_text, ls[i].backlog);
        ...
        if ((log)->log_level >= 6) ngx_log_error_core(6, log, 0, "try again to bind() after 500ms");
    ...
    if (failed) {
        if ((log)->log_level >= 1) ngx_log_error_core(1, log, 0, "still could not bind()");
        return -1;
    }
...
ngx_configure_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ls[i].rcvbuf != -1) {
            if (setsockopt(ls[i].fd, 1, 8,
                           (const void *) &ls[i].rcvbuf, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setsockopt(SO_RCVBUF, %d) %V failed, ignored", ls[i].rcvbuf, &ls[i].addr_text);
        ...
        if (ls[i].sndbuf != -1) {
            if (setsockopt(ls[i].fd, 1, 7,
                           (const void *) &ls[i].sndbuf, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setsockopt(SO_SNDBUF, %d) %V failed, ignored", ls[i].sndbuf, &ls[i].addr_text);
        ...
        if (ls[i].keepalive) {
            ...
            if (setsockopt(ls[i].fd, 1, 9,
                           (const void *) &keepalive, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setsockopt(SO_KEEPALIVE, %d) %V failed, ignored", keepalive, &ls[i].addr_text);
        ...
        if (ls[i].keepidle) {
            if (setsockopt(ls[i].fd, IPPROTO_TCP, 4,
                           (const void *) &ls[i].keepidle, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setsockopt(TCP_KEEPIDLE, %d) %V failed, ignored", ls[i].keepidle, &ls[i].addr_text);
        ...
        if (ls[i].keepintvl) {
            if (setsockopt(ls[i].fd, IPPROTO_TCP, 5,
                           (const void *) &ls[i].keepintvl, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setsockopt(TCP_KEEPINTVL, %d) %V failed, ignored", ls[i].keepintvl, &ls[i].addr_text);
        ...
        if (ls[i].keepcnt) {
            if (setsockopt(ls[i].fd, IPPROTO_TCP, 6,
                           (const void *) &ls[i].keepcnt, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setsockopt(TCP_KEEPCNT, %d) %V failed, ignored", ls[i].keepcnt, &ls[i].addr_text);


            }
        }
// # 582 "src/core/ngx_connection.c"
        if (ls[i].listen) {



            if (listen(ls[i].fd, ls[i].backlog) == -1) {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "listen() to %V, backlog %d failed, ignored", &ls[i].addr_text, ls[i].backlog);


            }
        }
// # 647 "src/core/ngx_connection.c"
        if (ls[i].add_deferred || ls[i].delete_deferred) {
            ...
            if (setsockopt(ls[i].fd, IPPROTO_TCP, 9,
                           &timeout, sizeof(int))
                == -1)
            {
                if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "setsockopt(TCP_DEFER_ACCEPT, %d) for %V failed, " "ignored", timeout, &ls[i].addr_text);
...
ngx_close_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (c) {
            if (c->read->active) {
                if (ngx_event_flags & 0x00000080) {
                    ngx_event_actions.del_conn(c, 1);

                } else if (ngx_event_flags & 0x00000040) {
                    ...
                    ngx_event_actions.del(c->read, EPOLLIN, 0);

                } else {
                    ngx_event_actions.del(c->read, EPOLLIN, 1);
                }
            }
        ...
        if (close(ls[i].fd) == -1) {
            if ((cycle->log)->log_level >= 1) ngx_log_error_core(1, cycle->log, (*__errno_location ()), "close() socket" " %V failed", &ls[i].addr_text);
...
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    if (ngx_cycle->files && (ngx_uint_t) s >= ngx_cycle->files_n) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "the new socket has number %d, " "but only %ui files are available", s, ngx_cycle->files_n);
    ...
    if (c == ((void *)0)) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "%ui worker_connections are not enough", ngx_cycle->connection_n);
    ...
    (void) memset(c, 0, sizeof(ngx_connection_t));
    ...
    (void) memset(rev, 0, sizeof(ngx_event_t));
    (void) memset(wev, 0, sizeof(ngx_event_t));
...
ngx_close_connection(ngx_connection_t *c)
{
    ...
    if (c->fd == -1) {
        if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, 0, "connection already closed");
        return;
    }
    ...
    if (ngx_event_actions.del_conn) {
        ...
        if (c->write->active || c->write->disabled) {
            ngx_event_actions.del(c->write, EPOLLOUT, 1);
        }
    }
// # 911 "src/core/ngx_connection.c"
    if (c->read->prev) {
        *(c->read->prev) = c->read->next; if (c->read->next) { c->read->next->prev = c->read->prev; } c->read->prev = ((void *)0); ;;
    }

    if (c->write->prev) {
        *(c->write->prev) = c->write->next; if (c->write->next) { c->write->next->prev = c->write->prev; } c->write->prev = ((void *)0); ;;
    }
    ...
    if (close(fd) == -1) {
        ...
        if ((ngx_cycle->log)->log_level >= level) ngx_log_error_core(level, ngx_cycle->log, err, "close() socket" " %d failed", fd);
...
ngx_reusable_connection(ngx_connection_t *c, ngx_uint_t reusable)
{
    ...
    if (c->reusable) {
        (&c->queue)->next->prev = (&c->queue)->prev; (&c->queue)->prev->next = (&c->queue)->next;
    }
    ...
    if (reusable) {
        ...
        (&c->queue)->next = ((ngx_queue_t *) &ngx_cycle->reusable_connections_queue)->next; (&c->queue)->next->prev = &c->queue; (&c->queue)->prev = (ngx_queue_t *) &ngx_cycle->reusable_connections_queue; ((ngx_queue_t *) &ngx_cycle->reusable_connections_queue)->next = &c->queue;
...
ngx_drain_connections(void)
{
    ...
    for (i = 0; i < 32; i++) {
        if ((&ngx_cycle->reusable_connections_queue == (&ngx_cycle->reusable_connections_queue)->prev)) {
            break;
        }

        q = (&ngx_cycle->reusable_connections_queue)->prev;
        c = (ngx_connection_t *) ((u_char *) q - __builtin_offsetof (ngx_connection_t, queue));
...
// # 1036 "src/core/ngx_connection.c"
    default:
    ...
    if (addr == 0) {
        ...
        if (getsockname(c->fd, (struct sockaddr *) &sa, &len) == -1) {
            ngx_connection_error(c, (*__errno_location ()), "getsockname() failed");
            return -1;
        }
        ...
        (void) memcpy(c->local_sockaddr, &sa, len);
    }
...
ngx_connection_error(ngx_connection_t *c, ngx_err_t err, char *text)
{
    ...
    if ((c->log)->log_level >= level) ngx_log_error_core(level, c->log, err, text);
