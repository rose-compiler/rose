...
// # 1 "/nfs/casc/overture/ROSE/opt/rhel7/x86_64/gcc/4.8.4/mpc/1.0/mpfr/3.1.2/gmp/5.1.2/lib/gcc/x86_64-unknown-linux-gnu/4.8.4/include/stdbool.h" 1 3 4
// # 25 "fatal-signal.c" 2
// # 1 "./stdlib.h" 1
// # 20 "./stdlib.h"
...
get_handler (struct sigaction const *a)
{
  ...
  if (a->sa_flags & 4)
    return (sa_handler_t) a->__sigaction_handler.sa_sigaction;
...
{
  static _Bool fatal_signals_initialized = 0;
      ...
      for (i = 0; i < ((sizeof(fatal_signals) / sizeof(fatal_signals[0])) - 1); i++)
        {
          ...
          if (sigaction (fatal_signals[i], ((void *)0), &action) >= 0
              && get_handler (&action) == ((__sighandler_t) 1))
            fatal_signals[i] = -1;
        }

      fatal_signals_initialized = 1;
    }
}
    ...
    if (fatal_signals[i] >= 0)
      {
        ...
        if (saved_sigactions[sig].__sigaction_handler.sa_handler == ((__sighandler_t) 1))
          saved_sigactions[sig].__sigaction_handler.sa_handler = ((__sighandler_t) 0);
        sigaction (sig, &saved_sigactions[sig], ((void *)0));
      }
}
...
{
  static _Bool cleanup_initialized = 0;
  if (!cleanup_initialized)
    {
      init_fatal_signals ();
      install_handlers ();
      cleanup_initialized = 1;
    }
  ...
  if (actions_count == actions_allocated)
    {
      ...
      actions_entry_t *new_actions =
        ((actions_entry_t *) (sizeof (actions_entry_t) == 1 ? xmalloc (new_actions_allocated) : xnmalloc (new_actions_allocated, sizeof (actions_entry_t))));
      size_t k;
...
{
  static _Bool fatal_signal_set_initialized = 0;
  if (!fatal_signal_set_initialized)
    {
      ...
      fatal_signal_set_initialized = 1;
    }
}
...
block_fatal_signals (void)
{
  ...
  sigprocmask (0, &fatal_signal_set, ((void *)0));
}
...
{
  init_fatal_signal_set ();
  sigprocmask (1, &fatal_signal_set, ((void *)0));
}
