...
ngx_signal_t signals[] = {
    ...
    { 14, "SIGALRM", "", ngx_signal_handler },

    { 2, "SIGINT", "", ngx_signal_handler },

    { 29, "SIGIO", "", ngx_signal_handler },

    { 17, "SIGCHLD", "", ngx_signal_handler },

    { 31, "SIGSYS, SIG_IGN", "", ((__sighandler_t) 1) },

    { 13, "SIGPIPE, SIG_IGN", "", ((__sighandler_t) 1) },
    ...
    char *name, ngx_int_t respawn)
{
    ...
    if (respawn >= 0) {
        ...
        if (s == 1024) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, 0, "no more than %d processes can be spawned", 1024);
    ...
    if (respawn != -5) {
        ...
        if (socketpair(1, SOCK_STREAM, 0, ngx_processes[s].channel) == -1)
        {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "socketpair() failed while spawning \"%s\"", name);
        ...
        if (ngx_nonblocking(ngx_processes[s].channel[0]) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "ioctl(FIONBIO)" " failed while spawning \"%s\"", name);
        ...
        if (ioctl(ngx_processes[s].channel[0], 0x5452, &on) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "ioctl(FIOASYNC) failed while spawning \"%s\"", name);

            ngx_close_channel(ngx_processes[s].channel, cycle->log);
            return -1;
        }

        if (fcntl(ngx_processes[s].channel[0], 8, ngx_pid) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "fcntl(F_SETOWN) failed while spawning \"%s\"", name);

            ngx_close_channel(ngx_processes[s].channel, cycle->log);
            return -1;
        }

        if (fcntl(ngx_processes[s].channel[0], 2, 1) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "fcntl(FD_CLOEXEC) failed while spawning \"%s\"", name);
        ...
        if (fcntl(ngx_processes[s].channel[1], 2, 1) == -1) {
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "fcntl(FD_CLOEXEC) failed while spawning \"%s\"", name);
    ...
    case -1:
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "fork() failed while spawning \"%s\"", name);
    ...
    if ((cycle->log)->log_level >= 6) ngx_log_error_core(6, cycle->log, 0, "start %s %P", name, pid);
...
ngx_execute_proc(ngx_cycle_t *cycle, void *data)
{
    ...
    if (execve(ctx->path, ctx->argv, ctx->envp) == -1) {
        if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "execve() failed while executing %s \"%s\"", ctx->name, ctx->path);
...
ngx_init_signals(ngx_log_t *log)
{
    ...
    for (sig = signals; sig->signo != 0; sig++) {
        ...
        if (sigaction(sig->signo, &sa, ((void *)0)) == -1) {
            if ((log)->log_level >= 1) ngx_log_error_core(1, log, (*__errno_location ()), "sigaction(%s) failed", sig->signame);
...
// # 370 "src/os/unix/ngx_process.c"
    ...
    if ((ngx_cycle->log)->log_level >= 6) ngx_log_error_core(6, ngx_cycle->log, 0, "signal %d (%s) received%s", signo, sig->signame, action);


    if (ignore) {
        if ((ngx_cycle->log)->log_level >= 3) ngx_log_error_core(3, ngx_cycle->log, 0, "the changing binary signal is ignored: " "you should shutdown or terminate " "before either old or new binary's process");
...
// # 526 "src/os/unix/ngx_process.c"
        ...
        if (((((__extension__ ({ union { __typeof(status) __in; int __i; } __u; __u.__in = (status); __u.__i; }))) & 0xff00) >> 8) == 2 && ngx_processes[i].respawn) {
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "%s %P exited with fatal code %d " "and cannot be respawned", process, pid, ((((__extension__ ({ union { __typeof(status) __in; int __i; } __u; __u.__in = (status); __u.__i; }))) & 0xff00) >> 8));
...
ngx_unlock_mutexes(ngx_pid_t pid)
{
    ...
    for (i = 0; ; i++) {
        ...
        if (ngx_shmtx_force_unlock(&sp->mutex, pid)) {
            if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, 0, "shared memory zone \"%V\" was locked by %P", &shm_zone[i].shm.name, pid);
...
ngx_os_signal_process(ngx_cycle_t *cycle, char *name, ngx_int_t pid)
{
    ...
    for (sig = signals; sig->signo != 0; sig++) {
        if (strcmp((const char *) name, (const char *) sig->name) == 0) {
            ...
            if ((cycle->log)->log_level >= 2) ngx_log_error_core(2, cycle->log, (*__errno_location ()), "kill(%P, %d) failed", pid, sig->signo);
