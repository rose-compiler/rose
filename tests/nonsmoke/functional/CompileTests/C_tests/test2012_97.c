...
rsvp_print(register const u_char *pptr, register u_int len) {
    ...
    if (!(gndo->ndo_snapend - (sizeof(*rsvp_com_header)) <= gndo->ndo_snapend && (const u_char *)&(*rsvp_com_header) <= gndo->ndo_snapend - (sizeof(*rsvp_com_header)))) goto trunc;
 ...
 printf("ERROR: RSVP version %u packet not supported",
               (((rsvp_com_header->version_flags)&0xf0)>>4));
 return;
    }
    ...
    plen = tlen = ((u_int16_t)ntohs(((const unaligned_u_int16_t *)(rsvp_com_header->length))->val));

    printf("\n\tRSVPv%u %s Message (%u), Flags: [%s], length: %u, ttl: %u, checksum: 0x%04x",
           (((rsvp_com_header->version_flags)&0xf0)>>4),
           tok2str(rsvp_msg_type_values, "unknown, type: %u",rsvp_com_header->msg_type),
           rsvp_com_header->msg_type,
           bittok2str(rsvp_header_flag_values,"none",((rsvp_com_header->version_flags)&0x0f)),
           tlen,
           rsvp_com_header->ttl,
           ((u_int16_t)ntohs(((const unaligned_u_int16_t *)(rsvp_com_header->checksum))->val)));
    ...
    case 12:
        while(tlen > 0) {
            ...
            subtlen=((u_int16_t)ntohs(((const unaligned_u_int16_t *)(rsvp_com_header->length))->val));
