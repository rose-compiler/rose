 ...
 *extracted_ethertype = 0;
 ...
 if ((control & 3) == 3) {
  ...
  control = ((u_int16_t)((u_int16_t)*((const u_int8_t *)(p + 2) + 1) << 8 | (u_int16_t)*((const u_int8_t *)(p + 2) + 0)));
  is_u = 0;
 }
...
// # 215 "./print-llc.c"
 ...
 if (gndo->ndo_eflag) {
                printf("LLC, dsap %s (0x%02x) %s, ssap %s (0x%02x) %s",
                       ...
                       dsap,
                       tok2str(llc_ig_flag_values, "Unknown", dsap_field & 1),
                       tok2str(llc_values, "Unknown", ssap),
                       ssap,
                       tok2str(llc_flag_values, "Unknown", ssap_field & 1));
...
// # 286 "./print-llc.c"
 ...
 if (is_u) {
  printf("Unnumbered, %s, Flags [%s], length %u",
                       tok2str(llc_cmd_values, "%02x", ((control) & 0xef)),
                       tok2str(llc_flag_values,"?",(ssap_field & 1) | (control & 0x10)),
                       length);
  ...
  if ((control & 1) == 1) {
   (void)printf("Supervisory, %s, rcv seq %u, Flags [%s], length %u",
    tok2str(llc_supervisory_values,"?",(((control) >> 2) & 0x03)),
    (((control) >> 9) & 0x7f),
    tok2str(llc_flag_values,"?",(ssap_field & 1) | (control & 0x0100)),
                                length);
  } else {
...
snap_print(const u_char *p, u_int length, u_int caplen, u_int bridge_pad)
{
 ...
 if (!(gndo->ndo_snapend - (5) <= gndo->ndo_snapend && (const u_char *)&(*p) <= gndo->ndo_snapend - (5))) goto trunc;
 orgcode = ((u_int32_t)((u_int32_t)*((const u_int8_t *)(p) + 0) << 16 | (u_int32_t)*((const u_int8_t *)(p) + 1) << 8 | (u_int32_t)*((const u_int8_t *)(p) + 2)));
 et = ((u_int16_t)ntohs(((const unaligned_u_int16_t *)(p + 3))->val));
...
// # 435 "./print-llc.c"
 ...
 case 0x0080c2:
  ...
  case 0x0007:
   ...
   if (!(gndo->ndo_snapend - (bridge_pad) <= gndo->ndo_snapend && (const u_char *)&(*p) <= gndo->ndo_snapend - (bridge_pad))) goto trunc;
   caplen -= bridge_pad;
   length -= bridge_pad;
   p += bridge_pad;
   ...
   ether_print(gndo, p, length, caplen, ((void *)0), ((void *)0));
   return (1);
...
// # 495 "./print-llc.c"
  ...
  case 0x000a:
   ...
   if (!(gndo->ndo_snapend - (bridge_pad + 1) <= gndo->ndo_snapend && (const u_char *)&(*p) <= gndo->ndo_snapend - (bridge_pad + 1))) goto trunc;
   caplen -= bridge_pad + 1;
   length -= bridge_pad + 1;
   p += bridge_pad + 1;
