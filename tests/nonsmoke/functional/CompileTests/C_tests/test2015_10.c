...
static uint8_t get_xen_consumer(xen_event_channel_notification_t fn)
 ...
 // Expansion of Xen BUG_ON macro.
     do { 
       // if (__builtin_expect((i >= (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(xen_consumers), typeof(&xen_consumers[0]))); }))),0)) 
       // if ( (i >= sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(xen_consumers), typeof(&xen_consumers[0]))); }) ) )
       // if ( ( __builtin_types_compatible_p(typeof(xen_consumers), typeof(&xen_consumers[0])) ) )
          if ( sizeof(struct { typeof(&xen_consumers[0]); }) )
               do { 
                    do {
