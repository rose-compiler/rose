...
static int ptygettyinfo (int fd,struct ttyinfo*ti);
static void ptysettyinfo (int fd,struct ttyinfo*ti);
...
// # 75 "zpty.c"
    ...
    mode = fcntl(fd, 3, 0);
    if (mode != -1 && !(mode & (04000|04000)) &&
 !fcntl(fd, 4, mode | (04000|04000)))
 return 1;
...
// # 1 "/usr/include/sys/stropts.h" 2 3 4
// # 161 "zpty.c" 2
// # 182 "zpty.c"
static int
get_pty(int master, int *retfd)
{
    ...
    if (master) {
 if ((mfd = open("/dev/ptmx", 02|0400)) < 0)
     return 1;
...
newptycmd(char *nam, char *pname, char **args, int echo, int nblock)
{
 ...
 return 1;
    ...
    if (get_pty(1, &master)) {
 zwarnnam(nam, "can't open pseudo terminal: %e", (*__errno_location ()));
 scriptname = oscriptname;
 ineval = oineval;
 return 1;
    }
    if ((pid = fork()) == -1) {
 zwarnnam(nam, "can't create pty command %s: %e", pname, (*__errno_location ()));
 close(master);
 scriptname = oscriptname;
 ineval = oineval;
 return 1;
 ...
 if (setsid() != mypid) {
     zwarnnam(nam, "failed to create new session: %e", (*__errno_location ()));


     if (ioctl(SHTTY, 0x5422, 0))
  zwarnnam(nam, "%e", (*__errno_location ()));
     setpgid(0L, mypid);
 ...
 if (((opts[INTERACTIVE]))) {
     ...
     if (ioctl(slave, 0x5413, (char *) &info.winsize) == 0) {
  info.winsize.ws_row = zterm_lines;
  info.winsize.ws_col = zterm_columns;
  ioctl(slave, 0x5414, (char *) &info.winsize);
     }
 }
 ...
 close(coprocout);
 init_io();
 assignsparam("TTY",ztrdup(ttystrname),0);
 ...
 zexit(lastval, 0);
    ...
    if (master == -1) {
 zerrnam(nam, "cannot duplicate fd %d: %e", master, (*__errno_location ()));
 scriptname = oscriptname;
 ineval = oineval;
 return 1;
    }
...
ptyread(char *nam, Ptycmd cmd, char **args, int noblock, int mustmatch)
{
 ...
 tokenize(p);
 remnulargs(p);
 if (!(prog = patcompile(p, 0x0040, ((void *)0)))) {
 ...
 if (noblock && cmd->read == -1) {
     ...
     expire_tv.tv_sec = 0;
     expire_tv.tv_usec = 0;
     do { unsigned int __i; fd_set *__arr = (&foofd); for (__i = 0; __i < sizeof (fd_set) / sizeof (__fd_mask); ++__i) ((__arr)->fds_bits)[__i] = 0; } while (0);
     (((&foofd)->fds_bits)[((cmd->fd) / (8 * sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((cmd->fd) % (8 * sizeof (__fd_mask)))));
     pollret = select(cmd->fd+1,
    (fd_set *) &foofd, ((void *)0), ((void *)0), &expire_tv);
 ...
 return 1;
    ...
    if (*args)
 assignsparam(*args,ztrdup(metafy(buf, used, 7)),0);
    else if (used)
 write_loop(1, buf, used);
...
static int
bin_zpty(char *nam, char **args, Options ops, int func __attribute__((__unused__)))
{
    if ((((ops)->ind['r'] != 0) && ((ops)->ind['w'] != 0)) ||
 ((((ops)->ind['r'] != 0) || ((ops)->ind['w'] != 0)) &&
  (((ops)->ind['d'] != 0) || ((ops)->ind['e'] != 0) ||
   ((ops)->ind['b'] != 0) || ((ops)->ind['L'] != 0))) ||
 (((ops)->ind['w'] != 0) && (((ops)->ind['t'] != 0) || ((ops)->ind['m'] != 0))) ||
 (((ops)->ind['n'] != 0) && (((ops)->ind['b'] != 0) || ((ops)->ind['e'] != 0) ||
    ((ops)->ind['r'] != 0) || ((ops)->ind['t'] != 0) ||
    ((ops)->ind['d'] != 0) || ((ops)->ind['L'] != 0) ||
    ((ops)->ind['m'] != 0))) ||
 (((ops)->ind['d'] != 0) && (((ops)->ind['b'] != 0) || ((ops)->ind['e'] != 0) ||
    ((ops)->ind['L'] != 0) || ((ops)->ind['t'] != 0) ||
    ((ops)->ind['m'] != 0))) ||
 (((ops)->ind['L'] != 0) && (((ops)->ind['b'] != 0) || ((ops)->ind['e'] != 0) ||
    ((ops)->ind['m'] != 0)))) {
 ...
 return (((ops)->ind['r'] != 0) ?
  ptyread(nam, p, args + 1, ((ops)->ind['t'] != 0),
   ((ops)->ind['m'] != 0)) :
  ptywrite(p, args + 1, ((ops)->ind['n'] != 0)));
    } else if (((ops)->ind['d'] != 0)) {
 Ptycmd p;
 int ret = 0;
 ...
 return newptycmd(nam, *args, args + 1, ((ops)->ind['e'] != 0),
    ((ops)->ind['b'] != 0));
    } else {
 Ptycmd p;
 char **a;
...
static int
ptyhook(Hookdef d __attribute__((__unused__)), void *dummy __attribute__((__unused__)))
{
    deleteallptycmds();
    return 0;
}
...
static struct builtin bintab[] = {
    { { ((void *)0), "zpty", 0 }, bin_zpty, 0, -1, 0, "ebdmrwLnt", ((void *)0) },
};
