...
// # 51 "memcache_proto.c" 2
// # 1 "../../include/vstring_vstream.h" 1
// # 22 "../../include/vstring_vstream.h"
...
// # 52 "memcache_proto.c" 2



// # 1 "./memcache_proto.h" 1
// # 17 "./memcache_proto.h"
...
int memcache_get(VSTREAM *stream, VSTRING *vp, ssize_t bound)
{
   ...
   vstring_get_bound(vp, stream, bound));
    ...
    case '\r':
 if ((next_char = ((&(stream)->buf)->cnt < 0 ? ++(&(stream)->buf)->cnt, (int) *(&(stream)->buf)->ptr++ : vbuf_get(&(stream)->buf))) == '\n') {
     ((&(vp)->vbuf)->cnt > 0 ? --(&(vp)->vbuf)->cnt, (int) (*(&(vp)->vbuf)->ptr++ = ('\n')) : vbuf_put((&(vp)->vbuf),('\n')));

 } else {
     if (next_char != (-1))
  vbuf_unget(&(stream)->buf, (next_char));
    ...
    default:
     if (msg_verbose)
  msg_info("%s got %s", ((stream)->path ? (const char *) (stream)->path : "unknown_stream"),
    ((ssize_t) ((vp)->vbuf.ptr - (vp)->vbuf.data)) < bound ? "EOF" : "input too long");
     return (-1);
 }
    ...
    case '\n':
 vstring_truncate(vp, ((ssize_t) ((vp)->vbuf.ptr - (vp)->vbuf.data)) - 1);
 if (((ssize_t) ((vp)->vbuf.ptr - (vp)->vbuf.data)) > 0 && ((char *) (vp)->vbuf.ptr)[-1] == '\r')
     vstring_truncate(vp, ((ssize_t) ((vp)->vbuf.ptr - (vp)->vbuf.data)) - 1);
 do { if ((vp)->vbuf.cnt <= 0) (((vp))->vbuf.space(&((vp))->vbuf, 1)); *(vp)->vbuf.ptr = 0; } while (0);
 if (msg_verbose)
     msg_info("%s got: %s", ((stream)->path ? (const char *) (stream)->path : "unknown_stream"), ((char *) (vp)->vbuf.data));
 return (0);
    }
}
...
int memcache_fwrite(VSTREAM *stream, const char *cp, ssize_t todo)
{
 ...
 msg_panic("memcache_fwrite: negative todo %ld", (long) todo);
    ...
    if (msg_verbose)
 msg_info("%s write: %.*s", ((stream)->path ? (const char *) (stream)->path : "unknown_stream"), (int) todo, cp);
    if (vbuf_write(&(stream)->buf, (cp), (todo)) != todo
 || vstream_fputs("\r\n", stream) == (-1))
 return (-1);
    else
 return (0);
...
int memcache_fread(VSTREAM *stream, VSTRING *buf, ssize_t todo)
{
 ...
 msg_panic("memcache_fread: negative todo %ld", (long) todo);
    ...
    ((buf)->vbuf.space(&(buf)->vbuf, todo));
    do { (buf)->vbuf.ptr = (buf)->vbuf.data + (todo); (buf)->vbuf.cnt = (buf)->vbuf.len - (todo); } while (0);
    if (vbuf_read(&(stream)->buf, (((char *) (buf)->vbuf.data)), (todo)) != todo
 || ((&(stream)->buf)->cnt < 0 ? ++(&(stream)->buf)->cnt, (int) *(&(stream)->buf)->ptr++ : vbuf_get(&(stream)->buf)) != '\r'
 || ((&(stream)->buf)->cnt < 0 ? ++(&(stream)->buf)->cnt, (int) *(&(stream)->buf)->ptr++ : vbuf_get(&(stream)->buf)) != '\n') {
 if (msg_verbose)
     msg_info("%s read: error", ((stream)->path ? (const char *) (stream)->path : "unknown_stream"));
 return (-1);
    } else {
 vstring_truncate(buf, todo);
 do { if ((buf)->vbuf.cnt <= 0) (((buf))->vbuf.space(&((buf))->vbuf, 1)); *(buf)->vbuf.ptr = 0; } while (0);
 if (msg_verbose)
     msg_info("%s read: %s", ((stream)->path ? (const char *) (stream)->path : "unknown_stream"), ((char *) (buf)->vbuf.data));
 return (0);
    }
}
...
int memcache_vprintf(VSTREAM *stream, const char *fmt, va_list ap)
{
    ...
    vstream_fputs("\r\n", stream);
    if (((&(stream)->buf)->flags & (((1<<0) | (1<<1)) | ((1<<3) | (1<<4)))))
 return (-1);
    else
 return (0);
}
...
int memcache_printf(VSTREAM *stream, const char *fmt,...)
{
 ...
 __builtin_va_end(ap);
 msg_info("%s write: %s", ((stream)->path ? (const char *) (stream)->path : "unknown_stream"), ((char *) (buf)->vbuf.data));
 vstring_free(buf);
    }
