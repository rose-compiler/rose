...
// This should unparse as: "typedef __builtin_va_list __gnuc_va_list;"
...
static ngx_str_t err_levels[] = {
    { 0, ((void *)0) },
    { sizeof("emerg") - 1, (u_char *) "emerg" },
    { sizeof("alert") - 1, (u_char *) "alert" },
    { sizeof("crit") - 1, (u_char *) "crit" },
    { sizeof("error") - 1, (u_char *) "error" },
    { sizeof("warn") - 1, (u_char *) "warn" },
    { sizeof("notice") - 1, (u_char *) "notice" },
    { sizeof("info") - 1, (u_char *) "info" },
    { sizeof("debug") - 1, (u_char *) "debug" }
};
...
// # 87 "src/core/ngx_log.c"
{
    ...
    (void) memcpy(errstr, ngx_cached_err_log_time.data, ngx_cached_err_log_time.len);
    ...
    *p++ = (u_char) 10;;
    ...
    (void) ngx_write_fd(2, msg, p - msg);
}
// # 189 "src/core/ngx_log.c"
void
ngx_log_abort(ngx_err_t err, const char *fmt, ...)
{
    ...
    if ((ngx_cycle->log)->log_level >= 2) ngx_log_error_core(2, ngx_cycle->log, err, "%*s", p - errstr, errstr);
...
ngx_log_stderr(ngx_err_t err, const char *fmt, ...)
{
    ...
    *p++ = (u_char) 10;;
    ...
    (void) ngx_write_fd(2, errstr, p - errstr);
}
...
ngx_log_init(u_char *prefix)
{
    ...
    if (name[0] != '/') {
        ...
        if (plen) {
            ...
            p = (((u_char *) memcpy(name, prefix, plen)) + (plen));

            if (!((*(p - 1)) == '/')) {
                *p++ = '/';
            }

            ngx_cpystrn(p, (u_char *) "logs/error.log", nlen + 1);

            p = name;
        }
    }

    ngx_log_file.fd = open((const char *) name, 01|02000|0100, 0644);



    if (ngx_log_file.fd == -1) {
        ngx_log_stderr((*__errno_location ()),
...
ngx_log_set_levels(ngx_conf_t *cf, ngx_log_t *log)
{
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        for (n = 1; n <= 8; n++) {
            if (strcmp((const char *) value[i].data, (const char *) err_levels[n].data) == 0) {
...
ngx_error_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (strcmp((const char *) value[1].data, (const char *) "stderr") == 0) {
        (&name)->len = 0; (&name)->data = ((void *)0);
    ...
    if (cf->cycle->new_log.file == ((void *)0)) {
        return ((void *)0);
    }

    if (cf->args->nelts == 2) {
        cf->cycle->new_log.log_level = 4;
        return ((void *)0);
    }
