...
static void closemn (struct multio**mfds,int fd);
...
static void execcmd (Estate state,int input,int output,int how,int last1);
...
static int execfuncdef (Estate state,int do_exec __attribute__((__unused__)));
...
static int doneps4;
static char *STTYval;
static char *blank_env[] = { ((void *)0) };



static int (*execfuncs[21 -8]) (Estate, int) = {
...
static struct builtin commandbn =
    { { ((void *)0), 0, 0 }, bin_whence, 0, -1, 27, "vV", ((void *)0) };
...
parse_string(char *s, int reset_lineno)
{
 ...
 struct rlimit current_limits[__RLIM_NLIMITS], limits[__RLIM_NLIMITS];
...
zsetlimit(int limnum, char *nam)
{
 ...
 if (setrlimit(limnum, limits + limnum)) {
     if (nam)
  zwarnnam(nam, "setrlimit failed: %e", (*__errno_location ()));
     return -1;
 }
...
setlimits(char *nam)
{
    ...
    for (limnum = 0; limnum < __RLIM_NLIMITS; limnum++)
...
zfork(struct timeval *tv)
{
 ...
 gettimeofday(tv, &dummy_tz);
    ...
    if (pid == -1) {
 zerr("fork failed: %e", (*__errno_location ()));
 return -1;
    }
...
{
    Wordcode end = state->pc + ((state->pc[-1]) >> 5);
...
zexecve(char *pth, char **argv, char **newenvp)
{
 ...
 sprintf(buf + 2, "%s/%s", pwd, pth);
    ...
    if ((eno = (*__errno_location ())) == 8 || eno == 2) {
 char execvebuf[64 + 1], *ptr, *ptr2, *argv0;
 int fd, ct, t0;

 if ((fd = open(pth, 00|0400)) >= 0) {
     argv0 = *argv;
     *argv = pth;
     ct = read(fd, execvebuf, 64);
     close(fd);
     if (ct > 0) {
  if (execvebuf[0] == '#') {
      if (execvebuf[1] == '!') {
   for (t0 = 0; t0 != ct; t0++)
       if (execvebuf[t0] == '\n')
    break;
   while ((typtab[((unsigned char)(execvebuf[t0]))] & (1 << 3)))
       execvebuf[t0--] = '\0';
   execvebuf[64] = '\0';
...
commandnotfound(char *arg0, LinkList args)
{
 ...
 return 127;
    ...
    insertlinknode(args,&(args)->node,arg0);
...
{
    Cmdnam cn;
    char buf[(4096*4)], buf2[(4096*4)];
    char *s, *z, *arg0;
    char **argv, **pp, **newenvp = ((void *)0);
    int eno = 0, ee;

    arg0 = (char *) (((args)->list.first)->dat);
    if ((opts[RESTRICTED]) && (strchr(arg0, '/') || defpath)) {
 ...
 zsfree(s);
    ...
    if ((!opts[RESTRICTED]) && (z = zgetenv("ARGV0"))) {
 ((((args)->list.first))->dat = ((void *) ztrdup(z)));
 ...
 sprintf(buf2, "-%s", arg0);
 ((((args)->list.first))->dat = ((void *) ztrdup(buf2)));
    }
 ...
 if (*s == '/') {
     ...
     if (arg0 == s || (!opts[PATHDIRS]) ||
  (arg0[0] == '.' && (arg0 + 1 == s ||
        (arg0[1] == '.' && arg0 + 2 == s)))) {
  zerr("%e: %s", lerrno, arg0);
  _exit((lerrno == 13 || lerrno == 8) ? 126 : 127);
     }
     break;
 }
    ...
    if (defpath) {
 char *s, pbuf[4096];
 char *dptr, *pe, *ps = "/bin:/usr/bin";

 for(;ps;ps = pe ? pe+1 : ((void *)0)) {
     pe = strchr(ps, ':');
     if (*ps == '/') {
  s = pbuf;
  if (pe)
...
findcmd(char *arg0, int docopy)
{
 ...
 return ((void *)0);
    for (s = arg0; *s; s++)
 if (*s == '/') {
     { if (iscom(arg0)) return docopy ? dupstring(arg0) : arg0; };
     if (arg0 == s || (!opts[PATHDIRS])) {
  return ((void *)0);
     }
     break;
 ...
 if (cn->node.flags & (1<<1))
   ...
   *z++ = '/';
      ...
      strcpy(z, arg0);
      { if (iscom(buf)) return docopy ? dupstring(buf) : arg0; };
  }
     strcpy(nn, cn->u.name ? *(cn->u.name) : "");
     strcat(nn, "/");
     strcat(nn, cn->node.nam);
 }
 { if (iscom(nn)) return docopy ? dupstring(nn) : arg0; };
    }
    for (pp = path; *pp; pp++) {
 z = buf;
 if (**pp) {
...
iscom(char *s)
{
    ...
    return (access(us, 1) == 0 && stat(us, &statbuf) >= 0 &&
     ((((statbuf.st_mode)) & 0170000) == (0100000)));
}
...
entersubsh(int flags)
{
 ...
 for (sig = 0; sig < (31 +3); sig++)
  ...
  unsettrap(sig);
    ...
    job_control_ok = monitor && (flags & ESUB_JOB_CONTROL) && (opts[POSIXJOBS]);
 ...
 if (flags & ESUB_ASYNC) {
     ...
     if (isatty(0)) {
  close(0);
  if (open("/dev/null", 02 | 0400)) {
      zerr("can't open /dev/null: %e", (*__errno_location ()));
      _exit(1);
  }
     }
 }
...
execode(Eprog p, int dont_change_job, int exiting, char *context)
{
 ...
 if (zsh_eval_context_len == alen + 1) {
    ...
    zsh_eval_context[alen+1] = ((void *)0);
...
execsimple(Estate state)
{
 ...
 return (lastval = 1);
    ...
    if (!(intrap && !trapisfunc && traplocallevel == locallevel) && !ineval && code)
 lineno = code - 1;

    code = ((*state->pc++) & ((wordcode) ((1 << 5) - 1)));
...
execlist(Estate state, int dont_change_job, int exiting)
{
    ...
    if (sourcelevel && (!opts[SHINSTDIN]))
 pline_level = list_pipe = 0;



    code = *state->pc++;
    if (((code) & ((wordcode) ((1 << 5) - 1))) != 1) {

 lastval = 0;
    }
    while (((code) & ((wordcode) ((1 << 5) - 1))) == 1 && !breaks && !retflag && !errflag) {
 int donedebug;

 ltype = ((code) >> 5);
 csp = cmdsp;

 if (!(intrap && !trapisfunc && traplocallevel == locallevel) && !ineval) {
// # 1150 "exec.c"
     wordcode code2 = *state->pc, lnp1 = 0;
     if (ltype & (1<<5)) {
  lnp1 = code2;
     } else if (((code2) & ((wordcode) ((1 << 5) - 1))) == 2) {
  if ((((code2) >> 5) & ((wordcode) 0x1c)) == 16)
      lnp1 = state->pc[1];
  else
      lnp1 = (((state->pc[1]) >> 5) >> 1);
     }
 ...
 if (sigtrapped[(31 +2)] && (opts[DEBUGBEFORECMD]) && !intrap) {
 ...
 if (donedebug == 2) {
     ...
     while (((code) & ((wordcode) ((1 << 5) - 1))) == 2) {
  state->pc = state->pc + (((code) >> 5) >> (5));
  if ((((code) >> 5) & ((wordcode) 3)) == 0)
      break;
  code = *state->pc++;
     }
     donetrap = 1;
 ...
 while (((code) & ((wordcode) ((1 << 5) - 1))) == 2) {
     next = state->pc + (((code) >> 5) >> (5));
     if (!oldnoerrexit)
  noerrexit = ((((code) >> 5) & ((wordcode) 3)) != 0);
     switch ((((code) >> 5) & ((wordcode) 3))) {
     case 0:

  if ((((code) >> 5) & ((wordcode) 0x1c)) & 16)
      execsimple(state);
  else
      execpline(state, code, ltype, (ltype & (1<<4)) && exiting);
  state->pc = next;
  goto sublist_done;
  break;
     case 1:
  ...
  if ((ret = (((((code) >> 5) & ((wordcode) 0x1c)) & 16) ?
       execsimple(state) :
       execpline(state, code, (1<<1), 0)))) {
      state->pc = next;
      code = *state->pc++;
      next = state->pc + (((code) >> 5) >> (5));
      while (((code) & ((wordcode) ((1 << 5) - 1))) == 2 &&
   ...
   next = state->pc + (((code) >> 5) >> (5));
      ...
      if (((code) & ((wordcode) ((1 << 5) - 1))) != 2) {



   donetrap = 1;
   goto sublist_done;
      } else if ((((code) >> 5) & ((wordcode) 3)) == 0) {
   donetrap = 1;
...
sublist_done:
 ...
 if (sigtrapped[(31 +2)] && !(opts[DEBUGBEFORECMD]) && !donedebug) {
 ...
 if (!noerrexit && !donetrap) {
     if (sigtrapped[(31 +1)] && lastval) {
  dotrap((31 +1));
  donetrap = 1;
     }
     if (lastval) {
  int errreturn = (opts[ERRRETURN]) &&
      ((opts[INTERACTIVE]) || locallevel || sourcelevel);
  int errexit = (opts[ERREXIT]) ||
      ((opts[ERRRETURN]) && !errreturn);
  if (errexit) {
      if (sigtrapped[0])
   dotrap(0);
      if (mypid != getpid())
...
execpline(Estate state, wordcode slcode, int how, int last1)
{
    ...
    int slflags = (((slcode) >> 5) & ((wordcode) 0x1c));
    wordcode code = *state->pc++;
    static int lastwj, lpforked;

    if (((code) & ((wordcode) ((1 << 5) - 1))) != 3)
 return lastval = (slflags & 8) != 0;
    else if (slflags & 8)
 last1 = 0;
 ...
 if (mpipe(ipipe) < 0) {
    ...
    if (!pline_level++) {
        list_pipe_pid = 0;
 nowait = 0;
 simple_pline = ((((code) >> 5) & ((wordcode) 1)) == 0);
 ...
 if (newjob != lastwj) {
  ...
  _exit(0);
     ...
     if (list_pipe || (pline_level && !(how & (1<<0))))
  jn->stat |= (0x0020);
  ...
  if(!pline_level) {
   ...
   qn->status = pn->status;
      ...
      jn->stat &= ~((0x0008) | (0x0020));
      jn->stat |= (0x0002) | (0x0001) | (0x0010);
      printjob(jn, !!(opts[LONGLISTJOBS]), 1);
  }
  ...
  if (!updated &&
      ...
      !(jobtab[list_pipe_job].stat & (0x0002))) {
      signal_unblock(sigchld_mask);
      signal_block(sigchld_mask);
  }
  if (list_pipe_child &&
  ...
  if (!list_pipe_child && !lpforked && !subsh && ((opts[MONITOR])) &&
   ...
   if (pid < 0) {
       ...
       zerr("pipe failed: %e", (*__errno_location ()));
   ...
   if (!(jn->stat & (0x0008))) {
       ...
       jobtab[list_pipe_job].stat |= (0x0080);
       jn->stat |= (0x0100) | (0x0020);
       jn->other = pid;
   }
   ...
   if (jobtab[list_pipe_job].procs) {
       if (setpgid(0L, mypgrp = jobtab[list_pipe_job].gleader)
    == -1) {
  ...
  if (jn->gleader)
     ...
     if (list_pipe_child ||
  ((jn->stat & (0x0008)) &&
   (list_pipe || (pline_level && !(jn->stat & (0x0100))))))
  deletejob(jn, 0);
     thisjob = pj;
 }
 if ((slflags & 8) && !errflag)
     lastval = !lastval;
    }
    if (!pline_level)
    ...
    int how, int input, int output, int last1)
{
 ...
 return;
    ...
    if (!(intrap && !trapisfunc && traplocallevel == locallevel) && !ineval && (((pcode) >> 5) >> 1))
 lineno = (((pcode) >> 5) >> 1) - 1;

    if (pline_level == 1) {
 if ((how & (1<<2)) || (!sfcontext && !sourcelevel))
     strcpy(list_pipe_text,
     getjobtext(state->prog,
         state->pc + ((((pcode) >> 5) & ((wordcode) 1)) == 0 ?
        0 : 1)));
 else
     list_pipe_text[0] = '\0';
    }
    if ((((pcode) >> 5) & ((wordcode) 1)) == 0)
 ...
 state->pc++;
 for (pc = state->pc; ((code = *pc) & ((wordcode) ((1 << 5) - 1))) == 4;
      pc += ((((int)(((code) >> 5) & (0x20))) ? 4 : 3) + (((int)(((code) >> 5) & (0x40))) ? 2 : 0)));
 ...
 if (((code) & ((wordcode) ((1 << 5) - 1))) >= 8 && (how & (1<<1))) {
     int synch[2];
     struct timeval bgtime;
  ...
  zclose(pipes[0]);
  close(synch[0]);
  entersubsh(((how & (1<<2)) ? ESUB_ASYNC : 0)
...
makecline(LinkList list)
{
 ...
 for (node = ((list)->list.first); node; (node = ((node)->next))) {
     *ptr++ = (char *)((node)->dat);
     quotedzputs(((node)->dat), xtrerr);
     if (((node)->next))
  fputc(' ', xtrerr);
 }
 fputc('\n', xtrerr);
...
untokenize(char *s)
{
 ...
 while ((c = *s++))
     if ((typtab[((unsigned char)(c))] & (1 << 4))) {
  char *p = s - 1;

  if (c != ((char) 0x9d))
      *p++ = ztokens[c - ((char) 0x84)];
...
quote_tokenized_output(char *str, FILE *file)
{
 ...
 default:
     if ((typtab[((unsigned char)(*s))] & (1 << 4))) {
  _IO_putc (ztokens[*s - ((char) 0x84)], file);
  continue;
     }
     break;
 }
...
checkclobberparam(struct redir *f)
{
 ...
 return 0;
    ...
    if (!(opts[CLOBBER]) && (fd = (int)getintvalue(v)) &&
 fd <= max_zsh_fd && fdtable[fd] == 2) {
...
clobber_open(struct redir *f)
{
    ...
    if ((opts[CLOBBER]) || (((f->type)>=REDIR_WRITE && (f->type)<=REDIR_READWRITE) && ((f->type) & 1)))
 return open(unmeta(f->name),
  01 | 0100 | 01000 | 0400, 0666);


    if ((fd = open(unmeta(f->name),
     01 | 0100 | 0200 | 0400, 0666)) >= 0)
 return fd;
    ...
    if ((fd = open(unmeta(f->name), 01 | 0400)) != -1) {
 if(!fstat(fd, &buf) && !((((buf.st_mode)) & 0170000) == (0100000)))
     return fd;
 close(fd);
    }
...
closemn(struct multio **mfds, int fd)
{
 ...
 if (mn->rflag) {
  ...
  for (i = 0; i < mn->ct; i++)
     ...
     for (i = 0; i < mn->ct; i++)
  while ((len = read(mn->fds[i], buf, 4092)) != 0) {
      if (len < 0) {
   if ((*__errno_location ()) == 4)
       continue;
   else
      ...
      char *varid)
{
 ...
 fd1 = movefd(fd2);
 if (fd1 == -1) {
     zerr("cannot moved fd %d: %e", fd2, (*__errno_location ()));
     return;
 } else {
     fdtable[fd1] = 2;
 ...
 if(!mfds[fd1]) {
  ...
  if (fd1 == fd2)
      ...
      if (fdN < 0 && (*__errno_location ()) != 9) {
   zerr("cannot duplicate fd %d: %e", fd1, (*__errno_location ()));
   mfds[fd1] = ((void *)0);
   closemnodes(mfds);
   return;
      }
 ...
 if (mfds[fd1]->ct == 1) {
     int fdN = movefd(fd1);
     if (fdN < 0) {
  zerr("multio failed for fd %d: %e", fd1, (*__errno_location ()));
  closemnodes(mfds);
  return;
     }
     mfds[fd1]->fds[0] = fdN;
     fdN = movefd(fd2);
     if (fdN < 0) {
  zerr("multio failed for fd %d: %e", fd2, (*__errno_location ()));
  closemnodes(mfds);
  return;
     }
     mfds[fd1]->fds[1] = fdN;
  ...
  return;
    ...
    if (subsh_close >= 0 && fdtable[subsh_close] == 0)
 subsh_close = -1;
}
...
addvars(Estate state, Wordcode pc, int addflags)
{
    ...
    wordcode ac;
    union linkroot svl; struct linknode __n0;
    ...
    flags = (!(addflags & ADDVAR_RESTORE) &&
      locallevel > 0 && (opts[WARNCREATEGLOBAL])) ?
 ASSPM_WARN_CREATE : 0;
    xtr = (opts[XTRACE]);
    if (xtr) {
 printprompt4();
 doneps4 = 1;
    ...
    state->pc = pc;
    while (((ac = *state->pc++) & ((wordcode) ((1 << 5) - 1))) == 5) {
 int myflags = flags;
 name = ecgetstr(state, 2, &htok);
 if (htok)
     untokenize(name);
 if (((((ac) >> 5) & ((wordcode) 2)) >> 1) == 1)
     myflags |= ASSPM_AUGMENT;
 if (xtr)
     fprintf(xtrerr,
  ((((ac) >> 5) & ((wordcode) 2)) >> 1) == 1 ? "%s+=" : "%s=", name);
 if ((isstr = ((((ac) >> 5) & ((wordcode) 1)) == 0))) {
     do { (svl).list.first = &__n0; (svl).list.last = &__n0; (svl).list.flags = 0; __n0.next = ((void *)0); __n0.prev = &(svl).node; __n0.dat = (void *) (ecgetstr(state, 2, &htok)); } while (0);
     vl = &svl;
 } else
     vl = ecgetlist(state, (((ac) >> 5) >> 2), 2, &htok);

 if (vl && htok) {
     prefork(vl, (isstr ? (0x04|0x02) :
    0x02));
     if (errflag) {
  state->pc = opc;
  return;
     }
     if ((opts[GLOBASSIGN]) || !isstr)
  globlist(vl, 0);
     if (errflag) {
  state->pc = opc;
  return;
 ...
 if (isstr && ((((vl)->list.first) == ((void *)0)) || !((((vl)->list.first))->next))) {
     Param pm;
     char *val;
     int allexp;

     if ((((vl)->list.first) == ((void *)0)))
  val = ztrdup("");
     else {
  untokenize((((vl)->list.first)->dat));
  ...
  if ((addflags & ADDVAR_RESTRICT) && (opts[RESTRICTED]) &&
...
execcmd(Estate state, int input, int output, int how, int last1)
{
    ...
    FILE *oxtrerr = xtrerr, *newxtrerr = ((void *)0);

    doneps4 = 0;
    redir = (((*state->pc) & ((wordcode) ((1 << 5) - 1))) == 4 ? ecgetredirs(state) : ((void *)0));
    if (((*state->pc) & ((wordcode) ((1 << 5) - 1))) == 5) {
 cmdoutval = 0;
 varspc = state->pc;
 while ((((code = *state->pc)) & ((wordcode) ((1 << 5) - 1))) == 5)
     state->pc += ((((code) >> 5) & ((wordcode) 1)) == 0 ?
     3 : (((code) >> 5) >> 2) + 2);
    } else
 varspc = ((void *)0);
    ...
    type = ((code) & ((wordcode) ((1 << 5) - 1)));




    args = (type == 6 ?
     ecgetlist(state, ((code) >> 5), 1, &htok) : ((void *)0));
 ...
 mfds[i] = ((void *)0);
    ...
    if (type == 6 && args && (((args)->list.first) != ((void *)0)) &&
 *(char *)(((args)->list.first)->dat) == '%') {
        if (how & (1<<3)) {
     oautocont = opts[AUTOCONTINUE];
            opts[AUTOCONTINUE] = 1;
 }
 insertlinknode(args,&(args)->node,dupstring((how & (1<<3)) ? "disown" : (how & (1<<2)) ? "bg" : "fg"));
 ...
 how = (1<<1);
    ...
    if ((opts[AUTORESUME]) && type == 6 && (how & (1<<1)) &&
 args && (((args)->list.first) != ((void *)0)) && (!redir || (((redir)->list.first) == ((void *)0))) && !input &&
 !((((args)->list.first))->next)) {
 if ((!opts[NOTIFY]))
     scanjobs();
 if (findjobnam((((args)->list.first)->dat)) != -1)
     insertlinknode(args,&(args)->node,dupstring("fg"));
    }
    ...
    if (type == 6) {
 while (args && (((args)->list.first) != ((void *)0))) {
     char *cmdarg = (char *) (((args)->list.first)->dat);
  ...
  break;
     ...
     checked = 0;
     if ((cflags & (1<<8)) && ((((args)->list.first))->next)) {

  char *next = (char *) ((((((args)->list.first))->next))->dat);
  char *cmdopt;
          ...
          (cmdopt = strchr("pvV", next[1])))
  {
   ...
   if (((((args)->list.first))->next))
       next = (char *) ((((((args)->list.first))->next))->dat);
      } else {
   hn = &commandbn.node;
   is_builtin = 1;
   break;
  ...
  if (!strcmp(next, "--"))
     ...
     if ((cflags & (1<<9)) && ((((args)->list.first))->next)) {
...
// # 2492 "exec.c"
  while (next && *next == '-' && strlen(next) >= 2) {
   ...
   default:
      ...
      if (((args)->list.first) && ((((args)->list.first))->next))
   next = (char *) ((((((args)->list.first))->next))->dat);
  }
  if (exec_argv0) {
      char *str, *s;
     ...
     uremnode(args, ((args)->list.first));
     hn = ((void *)0);
     if ((cflags & (1<<8)) && (!opts[POSIXBUILTINS]))
  break;
 }
    }


    esprefork = (assign || (opts[MAGICEQUALSUBST])) ? 0x01 : 0;
    if (args && htok)
 ...
 for (;;) {
     ...
     if (!(cflags & (1<<10)))
  while (!checked && !errflag && args && (((args)->list.first) != ((void *)0)) &&
         has_token((char *) (((args)->list.first)->dat)))
      zglob(args, ((args)->list.first), 0);
     else if (!unglobbed) {
  for (node = ((args)->list.first); node; (node = ((node)->next)))
  ...
  do_exec = 1;
     ...
     if (!args || (((args)->list.first) == ((void *)0))) {
  if (redir && (((redir)->list.first) != ((void *)0))) {
      if (do_exec) {
   ...
   if (!args)
       args = newlinklist();
   insertlinknode(args,((args)->list.last),dupstring(":"));
      } else if (readnullcmd && *readnullcmd &&
          ((Redir) (((redir)->list.first)->dat))->type == REDIR_READ &&
          !((((redir)->list.first))->next)) {
   if (!args)
       args = newlinklist();
   insertlinknode(args,((args)->list.last),dupstring(readnullcmd));
      } else {
   if (!args)
       args = newlinklist();
   insertlinknode(args,((args)->list.last),dupstring(nullcmd));
      }
  } else if ((cflags & (1<<5)) && (cflags & (1<<8))) {
      lastval = 0;
      return;
  } else {
   ...
   fflush(xtrerr);
     ...
     } else if ((opts[RESTRICTED]) && (cflags & (1<<9)) && do_exec) {
...
// # 2653 "exec.c"
 ...
 return;
    ...
    if ((how & (1<<2)) ||
 (!sfcontext && !sourcelevel && (((opts[MONITOR])) || (how & (1<<0)))))
 ...
 text = ((void *)0);
    ...
    setunderscore((args && (((args)->list.first) != ((void *)0))) ? ((char *) ((((args)->list.last))->dat)) : "");


    if (type == 6 && ((opts[INTERACTIVE])) && (!opts[RMSTARSILENT]) &&
 (opts[SHINSTDIN]) && args && (((args)->list.first) != ((void *)0)) &&
 ((((args)->list.first))->next) && !strcmp((((args)->list.first)->dat), "rm")) {
 LinkNode node, next;

 for (node = ((((args)->list.first))->next); node && !errflag; node = next) {
     char *s = (char *) ((node)->dat);
     int l = strlen(s);

     next = ((node)->next);
     if (s[0] == ((char) 0x87) && !s[1]) {
  if (!checkrmall(pwd))
      uremnode(args, node);
     } else if (l > 2 && s[l - 2] == '/' && s[l - 1] == ((char) 0x87)) {
  char t = s[l - 2];
 ...
 return;
    ...
    if (type == 6 && !nullexec) {
 char *s;
 char trycd = ((opts[AUTOCD]) && (opts[SHINSTDIN]) &&
        (!redir || (((redir)->list.first) == ((void *)0))) && args && !(((args)->list.first) == ((void *)0)) &&
        !((((args)->list.first))->next) && *(char *)(((args)->list.first)->dat));
 ...
 if (!hn && trycd && (s = cancd((((args)->list.first)->dat)))) {
     (((args)->list.first)->dat) = (void *) s;
     insertlinknode(args,&(args)->node,dupstring("cd"));
  ...
  is_builtin = 1;
    ...
    is_cursh = (is_builtin || is_shfunc || nullexec || type >= 8);
// # 2804 "exec.c"
    if ((how & (1<<2)) ||
 (!do_exec &&
 ...
 if (pid) {
  ...
  while (((ac = *p) & ((wordcode) ((1 << 5) - 1))) == 5) {
      if (!strcmp(ecrawstr(state->prog, p + 1, ((void *)0)), "STTY")) {
 ...
 close(synch[0]);
 flags = ((how & (1<<2)) ? ESUB_ASYNC : 0) | ESUB_PGRP;
 if ((type != 7) && !(how & (1<<2)))
     flags |= ESUB_KEEPTRAP;
 if (type == 7 && !(how & (1<<2)))
 ...
 if ((how & (1<<2)) && (opts[BGNICE]))
     if (nice(5) < 0)
  zwarn("nice(5) failed: %e", (*__errno_location ()));
 ...
 jobtab[thisjob].stat |= (0x0400);
 if (!jobtab[thisjob].procs)
     jobtab[thisjob].stat |= (0x0020);
 if (is_builtin)
   jobtab[thisjob].stat |= (0x4000);
    } else {
 ...
 is_exec = 1;
    ...
    if ((esglob = !(cflags & (1<<10))) && args && htok) {
 LinkList oargs = args;
 globlist(args, 0);
 args = oargs;
    }
 ...
 if ((newxtrerr = fdopen(movefd(dup(fileno(stderr))), "w"))) {
    ...
    if (input)
 addfd(forked, save, mfds, 0, input, 0, ((void *)0));
    if (output)
 addfd(forked, save, mfds, 1, output, 1, ((void *)0));
 ...
 spawnpipes(redir, nullexec);
    ...
    while (redir && (((redir)->list.first) != ((void *)0))) {
 ...
 if (fn->type == REDIR_INPIPE) {
  ...
  closemnodes(mfds);
  fixfds(save);
  do { if (!forked) { redir_err = lastval = 1; goto done; } else { _exit(1); } } while (0);
     }
  ...
  do { if (!forked) { redir_err = lastval = 1; goto done; } else { _exit(1); } } while (0);
     ...
     if ((opts[RESTRICTED]) && ((fn->type)>=REDIR_WRITE && (fn->type)<=REDIR_READWRITE)) {
  ...
  if (!checkclobberparam(fn))
      ...
      fil = getherestr(fn);
  if (fil == -1) {
      if ((*__errno_location ()) && (*__errno_location ()) != 4)
  ...
  if (!checkclobberparam(fn))
      ...
      fil = open(unmeta(fn->name), 00 | 0400);
  else
  ...
  if (fil == -1) {
      ...
      fixfds(save);
      if ((*__errno_location ()) != 4)
   zwarn("%e: %s", (*__errno_location ()), fn->name);
      do { if (!forked) { redir_err = lastval = 1; goto done; } else { _exit(1); } } while (0);
  }
  addfd(forked, save, mfds, fn->fd1, fil, 0, fn->varid);
  ...
  if (nullexec == 1 && fn->fd1 == 0 &&
      (opts[SHINSTDIN]) && ((opts[INTERACTIVE])) && !zleactive)
      init_io();
  break;
     case REDIR_CLOSE:
  if (fn->varid) {
  ...
  if (!checkclobberparam(fn))
      fil = -1;
  else if ((((fn->type)>=REDIR_WRITE && (fn->type)<=REDIR_READWRITE) && ((fn->type) & 2)))
      fil = open(unmeta(fn->name),
          ((!opts[CLOBBER]) && !(((fn->type)>=REDIR_WRITE && (fn->type)<=REDIR_READWRITE) && ((fn->type) & 1))) ?
          01 | 02000 | 0400 :
          01 | 02000 | 0100 | 0400, 0666);
  else
      fil = clobber_open(fn);
  if(fil != -1 && ((fn->type)>=REDIR_ERRWRITE && (fn->type)<=REDIR_ERRAPPNOW))
      dfil = dup(fil);
  else
      dfil = 0;
  if (fil == -1 || dfil == -1) {
  ...
  if(((fn->type)>=REDIR_ERRWRITE && (fn->type)<=REDIR_ERRAPPNOW))
      addfd(forked, save, mfds, 2, dfil, 1, ((void *)0));
  break;
     }
 ...
 if (is_exec) {
     int flags = ((how & (1<<2)) ? ESUB_ASYNC : 0) |
  ESUB_PGRP | ESUB_FAKE;
     if (type != 7)
  flags |= ESUB_KEEPTRAP;
     if ((do_exec || (type >= 8 && last1 == 1))
  && !forked)
 ...
 if (type >= 8) {
     if (last1 == 1)
  do_exec = 1;
     lastval = (execfuncs[type - 8])(state, do_exec);
 } else if (is_builtin || is_shfunc) {
     LinkList restorelist = 0, removelist = 0;


     if (!forked && ((cflags & (1<<8)) ||
       ((!opts[POSIXBUILTINS]) && !assign) ||
       ((opts[POSIXBUILTINS]) && !is_shfunc &&
        !(hn->flags & (1<<11))))) {
  if (varspc)
      save_params(state, varspc, &restorelist, &removelist);
  else
      restorelist = removelist = ((void *)0);
     }
     if (varspc) {
  ...
  if (save[1] == -2) {
      if (ferror(stdout)) {
   zwarn("write error: %e", (*__errno_location ()));
   clearerr(stdout);
      }
  } else
      clearerr(stdout);
     }
     if ((opts[PRINTEXITVALUE]) && (opts[SHINSTDIN]) &&
  lastval && !subsh) {
  ...
  if ((opts[RCS]) && ((opts[INTERACTIVE])) && !nohistsave)
      savehistfile(((void *)0), 1, 0x8000);
  exit(lastval);
     }
     if (restorelist)
  restore_params(restorelist, removelist);
  ...
  if (!subsh && (opts[RCS]) && ((opts[INTERACTIVE])) && !nohistsave)
      savehistfile(((void *)0), 1, 0x8000);
     }
     if (type == 6) {
  if (varspc) {
...
save_params(Estate state, Wordcode pc, LinkList *restore_p, LinkList *remove_p)
{
    ...
    *remove_p = newlinklist();
    ...
    while (((ac = *pc) & ((wordcode) ((1 << 5) - 1))) == 5) {
 s = ecrawstr(state->prog, pc + 1, ((void *)0));
...
// # 3426 "exec.c"
  ...
  copyparam(tpm, pm, 0);
  pm = tpm;
     } else if (!(pm->node.flags & (1<<10)) &&
         ((!opts[RESTRICTED]) || !(pm->node.flags & (1<<24)))) {
  ...
  pm = tpm;
     ...
     insertlinknode(*remove_p,((*remove_p)->list.last),dupstring(s));
     insertlinknode(*restore_p,((*restore_p)->list.last),pm);
 } else
     insertlinknode(*remove_p,((*remove_p)->list.last),dupstring(s));

 pc += ((((ac) >> 5) & ((wordcode) 1)) == 0 ?
        3 : (((ac) >> 5) >> 2) + 2);
    }
}
...
restore_params(LinkList restorelist, LinkList removelist)
{
 ...
 while ((pm = (Param) ugetnode(restorelist))) {
  ...
  paramtab->addnode(paramtab, pm->node.nam, pm);
     ...
     if ((pm->node.flags & (1<<12)) && ((s = getsparam(pm->node.nam))))
  addenv(pm, s);
 }
    }
}
...
gethere(char **strp, int typ)
{
    ...
    for (s = str; *s; s++)
 if ((typtab[((unsigned char)(*s))] & (1 << 14))) {
     qt = 1;
     break;
 }
    str = quotesubst(str);
...
getherestr(struct redir *fn)
{
 ...
 t[len++] = '\n';
    if ((fd = gettempfile(((void *)0), 1, &s)) < 0)
 return -1;
    write_loop(fd, t, len);
    close(fd);
    fd = open(s, 00 | 0400);
...
simple_redir_name(Eprog prog, int redir_type)
{
    ...
    pc = prog->prog;
    if (prog != &dummy_eprog &&
 ((pc[0]) & ((wordcode) ((1 << 5) - 1))) == 1 && (((pc[0]) >> 5) & (1<<4)) &&
 ((pc[1]) & ((wordcode) ((1 << 5) - 1))) == 2 && !(((pc[1]) >> 5) & ((wordcode) 0x1c)) &&
 (((pc[1]) >> 5) & ((wordcode) 3)) == 0 &&
 ((pc[2]) & ((wordcode) ((1 << 5) - 1))) == 3 && (((pc[2]) >> 5) & ((wordcode) 1)) == 0 &&
 ((pc[3]) & ((wordcode) ((1 << 5) - 1))) == 4 && ((int)(((pc[3]) >> 5) & (0x1f))) == redir_type &&
 !((int)(((pc[3]) >> 5) & (0x20))) &&
 !pc[4] &&
 ((pc[6]) & ((wordcode) ((1 << 5) - 1))) == 6 && !((pc[6]) >> 5)) {
 return dupstring(ecrawstr(prog, pc + 5, ((void *)0)));
    }
...
getoutput(char *cmd, int qt)
{
 ...
 if (errflag)
     return ((void *)0);
 untokenize(s);
 if ((stream = open(unmeta(s), 00 | 0400)) == -1) {
     zerr("%e: %s", (*__errno_location ()), s);
     return ((void *)0);
 }
 ...
 return ((void *)0);
    ...
    signal_block(sigchld_mask);
    cmdoutval = 0;
    if ((cmdoutpid = pid = zfork(((void *)0))) == -1) {
...
readoutput(int in, int qt)
{
    ...
    while ((c = fgetc(fin)) != (-1) || (*__errno_location ()) == 4) {
 if (c == (-1)) {
     (*__errno_location ()) = 0;
     clearerr(fin);
     continue;
 }
 if ((typtab[((unsigned char)(c))] & (1 << 12))) {
     *ptr++ = ((char) 0x83);
     c ^= 32;
     cnt++;
 }
 ...
 *ptr++ = c;
 ...
 insertlinknode(ret,((ret)->list.last),buf);
    } else {
 ...
 while (*words) {
     if ((opts[GLOBSUBST]))
  shtokenize(*words);
     insertlinknode(ret,((ret)->list.last),*words++);
 }
    }
    return ret;
}
...
parsecmd(char *cmd, char **eptr)
{
    ...
    for (str = cmd + 2; *str && *str != ((char) 0x89); str++);
    if (!*str || cmd[1] != ((char) 0x88)) {
...
getoutputfile(char *cmd, char **eptr)
{
 ...
 jobtab[thisjob].filelist = znewlinklist();
    zinsertlinknode(jobtab[thisjob].filelist,((jobtab[thisjob].filelist)->list.last),nam);

    if (!s)
 signal_block(sigchld_mask);
    fd = open(nam, 01 | 0100 | 0200 | 0400, 0600);
 ...
 return nam;
    ...
    if (fd < 0 || (cmdoutpid = pid = zfork(((void *)0))) == -1) {
...
// # 3966 "exec.c"
    ...
    if (thisjob == -1)
 return ((void *)0);
    pnam = hcalloc(strlen("/proc/self/fd") + 6);
    if (!(prog = parsecmd(cmd, eptr)))
 return ((void *)0);
    if (mpipe(pipes) < 0)
 return ((void *)0);
    if ((pid = zfork(&bgtime))) {
 sprintf(pnam, "%s/%d", "/proc/self/fd", pipes[!out]);
 zclose(pipes[out]);
 if (pid == -1)
 {
     zclose(pipes[!out]);
...
static int
execcond(Estate state, int do_exec __attribute__((__unused__)))
{
    int stat;
...
static int
execarith(Estate state, int do_exec __attribute__((__unused__)))
{
    char *e;
...
static int
exectime(Estate state, int do_exec __attribute__((__unused__)))
{
    int jb;
 ...
 return 0;
    ...
    execpline(state, *state->pc++, (1<<0)|(1<<1), 0);
    thisjob = jb;
    return lastval;
}
...
static int
execfuncdef(Estate state, int do_exec __attribute__((__unused__)))
{
    Shfunc shf;
    char *s = ((void *)0);
    ...
    names = ecgetlist(state, *state->pc++, 2, &htok);
    nprg = end - beg;
    sbeg = *state->pc++;
    nstrs = *state->pc++;
    npats = *state->pc++;
 ...
 if (!names) {
     ...
     args = ecgetlist(state, *state->pc++, 2, &htok);
  ...
  args = newlinklist();
     shf->node.nam = "(anon)";
     insertlinknode(args,&(args)->node,shf->node.nam);
...
{
    LinkList last_file_list = ((void *)0);
    unsigned char *ocs;
    int ocsp, osfc;
 ...
 jobtab[thisjob].filelist = ((void *)0);
 deletejob(jobtab + thisjob, 0);
    }
 ...
 printprompt4();
 if (args)
     for (lptr = ((args)->list.first); lptr; (lptr = ((lptr)->next))) {
  if (lptr != ((args)->list.first))
      fputc(' ', xtrerr);
  quotedzputs((char *)((lptr)->dat), xtrerr);
     }
 fputc('\n', xtrerr);
 fflush(xtrerr);
    }
...
// # 4360 "exec.c"
static int
execautofn(Estate state, int do_exec __attribute__((__unused__)))
{
    Shfunc shf;
...
loadautofn(Shfunc shf, int fksh, int autol)
{
 ...
 ksh = fksh;
 if (ksh == 1)
     ksh = (shf->node.flags & (1<<17)) ? 2 :
    (shf->node.flags & (1<<18)) ? 0 : 1;
    }
 ...
 return ((void *)0);
    ...
    if (ksh == 2 || (ksh == 1 && (opts[KSHAUTOLOAD]))) {
 if (autol) {
     prog->flags |= 8;
...
// # 4544 "exec.c"
 ...
 runshfunc(prog, ((void *)0), fstack.name);
...
getfpfunc(char *s, int *ksh, char **fname)
{
 ...
 unmetafy(buf, ((void *)0));
 if (!access(buf, 4) && (fd = open(buf, 00 | 0400)) != -1) {
...
stripkshdef(Eprog prog, char *name)
{
 ...
 return ((void *)0);
    ...
    code = *pc++;
    if (((code) & ((wordcode) ((1 << 5) - 1))) != 1 ||
 (((code) >> 5) & ((1<<1)|(1<<4)|(1<<5))) != ((1<<1)|(1<<4)|(1<<5)))
 return prog;
    pc++;
    code = *pc++;
    if (((code) & ((wordcode) ((1 << 5) - 1))) != 10 ||
 *pc != 1 || strcmp(name, ecrawstr(prog, pc + 1, ((void *)0))))
 return prog;

    {
 Eprog ret;
 Wordcode end = pc + ((code) >> 5);
...
cancd(char *s)
{
 ...
 if (cancd2(s))
     return s;
 if (access(unmeta(s), 1) == 0)
     return ((void *)0);
 if (!nocdpath)
     for (cp = cdpath; *cp; cp++) {
  if (strlen(*cp) + strlen(s) + 1 >= 4096)
      continue;
  if (**cp)
...
cancd2(char *s)
{
    ...
    if (!(opts[CHASEDOTS]) && !(opts[CHASELINKS])) {
 if (*s != '/')
 ...
 us = unmeta(s);
    ret = !(access(us, 1) || stat(us, &buf) || !((((buf.st_mode)) & 0170000) == (0040000)));
    if (us2)
 free(us2);
    return ret;
}
