...
for header in ${STL_HEADERS}; do
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "----------------------------------------------------------------------------------------------------------------" >> $LOGFILE
        echo "$VALGRIND $TOOL2 $TOOL2_BACKEND $BS_INCLUDE ${TEST_HEADER_PP_C} -std=$LANG_STANDARD" >> $LOGFILE
        {
        ...
        if [ $? -eq 0 ]; then
            ...
            then
                ...
                echo -en " ${RED}FAIL${COLOREND} "
                echo -en "[no file generated by: $VALGRIND $TOOL2 $TOOL2_BACKEND $BS_INCLUDE --edg:no_warnings ${TEST_HEADER_PP_C} -std=$LANG_STANDARD ]"
                ((T1_FAIL+=1))
            fi
        else
            echo -en "${RED}FAIL ----${COLOREND}" # 1
            # reinvoke the rose tool to show the failing front end issue
            echo -en " [ $VALGRIND $TOOL2 $TOOL2_BACKEND $BS_INCLUDE --edg:no_warnings ${TEST_HEADER_PP_C} -std=$LANG_STANDARD ]"
            ((T1_FAIL+=1))
        fi
    else
