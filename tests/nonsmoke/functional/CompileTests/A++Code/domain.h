...
class Array_Domain_Type
  ...
  // friend class Partitioning_Type;
     ...
     */
     ...
     // intArray indexing support for each array dimension (required for inirect addressing support)!
        intArray *Index_Array[MAX_ARRAY_DIMENSION];
#if defined(PPP)
     ...
     private:
        static Array_Domain_Type *Current_Link;
        Array_Domain_Type        *freepointer;
     ...
     // (12/13/2000) Note: These are set to ZERO on processors where the SerialArray is a NullArray.
        int Left_Number_Of_Points  [MAX_ARRAY_DIMENSION];
        int Right_Number_Of_Points [MAX_ARRAY_DIMENSION];
...
// end of defined(USE_EXPRESSION_TEMPLATES)
     ...
     public:
       ...
       // Support functions for reference counting (inlined for performance)
          ...
          inline void incrementReferenceCount() const { ((Array_Domain_Type*) this)->referenceCount++; }
	...
	static bool exitFromGlobalMemoryRelease;
        ...
        inline void incrementRawDataReferenceCount() const
           {
             ...
             if (APP_DEBUG > 0)
                  printf ("Inside of Array_Domain_Type::incrementRawDataReferenceCount() rc = %d + 1 \n",
...
#if COMPILE_DEBUG_STATEMENTS
             if (APP_DEBUG > 0)
                  printf ("Inside of Array_Domain_Type::decrementRawDataReferenceCount() rc = %d - 1 \n",
                       Array_Reference_Count_Array [Array_ID()]);
#endif
             APP_ASSERT (getRawDataReferenceCount() >= 0);
             Array_Reference_Count_Array [Array_ID()]--;
     ...
     // static int Pop_Array_ID ();
        ...
        void Initialize_Indirect_Addressing_View_Of_Target ( int Axis , const intArray & X , 
                                         const Array_Domain_Type & Target_Domain );
#if defined(PPP)
        void Compute_Local_Index_Arrays ( const Array_Domain_Type & Original_Domain );
        void Initialize_Parallel_Parts_Of_Domain ( const Array_Domain_Type & X );
     ...
     // void Fixup_Array_Domain_For_Copy_Constructor ( const Array_Domain_Type & Target_Domain );

     public:
     // Used to free memory-in-use internally in Array_Domain_Type objects! 
     ...
     // ********************************************************************************
        ...
        void Initialize_Non_Indexed_Dimension ( int x , const Array_Domain_Type & X );
        void Initialize_Indirect_Addressing_View_Of_Target_Non_Indexed_Dimension ( int x , const Array_Domain_Type & X );

        void Preinitialize_Domain ( const Array_Domain_Type & X );
     ...
     // ********************************************************************************
     // ********************************************************************************
     // *******************  Array_Domain_Type CONSTRUCTORS  *******************
     // ********************************************************************************
     // ********************************************************************************
     ...
     // ********************************************************************************
        Array_Domain_Type();
        Array_Domain_Type( int Number_Of_Valid_Dimensions , 
                             const Integer_Array_MAX_ARRAY_DIMENSION_Type Array_Size );
        Array_Domain_Type( int Number_Of_Valid_Dimensions ,
                             const Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Internal_Index_List );

        Array_Domain_Type    ( ARGUMENT_LIST_MACRO_INTEGER );
  ...
  // This simplifies the preinitialization lists.
        Array_Domain_Type ( const Internal_Index & I );
#if (MAX_ARRAY_DIMENSION >= 2)
        Array_Domain_Type ( const Internal_Index & I, const Internal_Index & J );
#endif
#if (MAX_ARRAY_DIMENSION >= 3)
        Array_Domain_Type ( const Internal_Index & I, const Internal_Index & J,
...
#error MAX_ARRAY_DIMENSION exceeded!
     ...
     //                           Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type* Index_List );
        ...
        Array_Domain_Type ( const Integer_Array_MAX_ARRAY_DIMENSION_Type Integer_List );
...
#error MAX_ARRAY_DIMENSION exceeded!
     ...
     // ********************************************************************************
#if defined(PPP)
        Array_Domain_Type    ( ARGUMENT_LIST_MACRO_INTEGER_AND_CONST_REF_RANGE );
#endif
     ...
     // ********************************************************************************
#if defined(APP) || defined(PPP)
     // Array_Domain_Type( int Array_Size_I , int Array_Size_J , 
     ...
     // Array_Domain_Type ( const char* );
        Array_Domain_Type ( const AppString & X );
     ...
     // These are used as part of error checking support (for test codes)
        ...
        bool operator== ( const Array_Domain_Type & X ) const;
     ...
     // This builds a descriptor to represent a 1D (linear) view any data described by the input descriptor.
        static Array_Domain_Type *Vectorizing_Domain ( const Array_Domain_Type & X );
        ...
        void Update_Parallel_Information_Using_Old_Domain ( const Array_Domain_Type & Old_Array_Domain_Type );
#endif

        static Array_Domain_Type* Build_Temporary_By_Example ( const Array_Domain_Type & X );
     ...
     // PADRE required functions
        inline void setLeftNumberOfPoints (int value, int i) { Left_Number_Of_Points [i] = value; }
        inline void setRightNumberOfPoints(int value, int i) { Right_Number_Of_Points[i] = value; }
     ...
     // Dimension independent query functions
        bool isSameBase   ( const Array_Domain_Type & X ) const;
        bool isSameBound  ( const Array_Domain_Type & X ) const;
        bool isSameStride ( const Array_Domain_Type & X ) const;
        bool isSameLength ( const Array_Domain_Type & X ) const;
#if defined(APP) || defined(PPP)
        bool isSameGhostBoundaryWidth (const Array_Domain_Type & X ) const;
        bool isSameDistribution ( const Array_Domain_Type & X ) const;
#endif

     // We have a definition of similar which means that the 
     // bases bounds and strides are the same.
        bool isSimilar ( const Array_Domain_Type & X ) const;
	...
	*/
	...
	int* setupProcessorListOnPosition 
     ...
     // static int computeArrayDimension ( int Array_Size_I, int Array_Size_J, int Array_Size_K, int Array_Size_L );
        ...
        static int computeArrayDimension ( const Array_Domain_Type & X );
...
#if !defined(PPP)
        static void Put_Domain_Into_Cache ( Array_Domain_Type* & X );

        static Array_Domain_Type *Get_Domain_From_Cache ( 
             int Array_Size_I , int Array_Size_J , int Array_Size_K , int Array_Size_L );
        static Array_Domain_Type *Get_Domain_From_Cache ( const Array_Domain_Type & X );
        static Array_Domain_Type *Get_Domain_From_Cache ();
        Array_Domain_Type *Get_Domain_From_Cache ( const Internal_Index & I );
        Array_Domain_Type *Get_Domain_From_Cache ( const Internal_Index & I , const Internal_Index & J );
        Array_Domain_Type *Get_Domain_From_Cache ( const Internal_Index & I , const Internal_Index & J , 
                                                             const Internal_Index & K );
        Array_Domain_Type *Get_Domain_From_Cache ( const Internal_Index & I , const Internal_Index & J , 
                                                             const Internal_Index & K , const Internal_Index & L );
