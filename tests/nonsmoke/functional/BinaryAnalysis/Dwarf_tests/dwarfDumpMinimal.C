     ...
     for (;;)
       ...
       /* child first: we are doing depth-first walk */
          if (cdres == DW_DLV_OK)
            ...
            // print_die_and_children(dbg, child, srcfiles, cnt);
               build_dwarf_IR_node_from_die_and_children(dbg, child, srcfiles, cnt);
...
void
build_dwarf_line_numbers_this_cu(Dwarf_Debug dbg, Dwarf_Die cu_die)
   {
     Dwarf_Signed linecount = 0;
     Dwarf_Line *linebuf = NULL;
     Dwarf_Signed i = 0;
...
/* process each compilation unit in .debug_info */
void
build_dwarf_IR_nodes(Dwarf_Debug dbg)
   {
     Dwarf_Unsigned cu_header_length = 0;
     Dwarf_Unsigned abbrev_offset = 0;
     Dwarf_Half version_stamp = 0;
          ...
          if (sres == DW_DLV_OK)
             {
               ...
               if (info_flag || cu_name_flag)
                 ...
                 // to the source code.
                    ...
                    printf ("In print_infos(): Calling print_die_and_children() \n");
                 // print_die_and_children(dbg, cu_die, srcfiles, cnt);
                    build_dwarf_IR_node_from_die_and_children(dbg, cu_die, srcfiles, cnt);
               ...
               if (line_flag)
                 ...
                 // print_line_numbers_this_cu(dbg, cu_die);
                    build_dwarf_line_numbers_this_cu(dbg, cu_die);
                  }
...
main(int argc, char** argv)
  ...
  // I am unclear about the functionality of these two functions!
     ...
     build_dwarf_IR_nodes(rose_dwarf_dbg);
...
#if 0
  // Output the dwarf information stored in the executable...
     Elf* elf = NULL;
     int archive = 0; // false -- this is not an archive (*.a file)
     string file_name = "dwarfReader";
  // dumpDwarf ( elf, file_name, archive, &global_config_file_data );
#endif
