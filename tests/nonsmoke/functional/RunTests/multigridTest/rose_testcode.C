...
// APP_smartReleaseOfInternalMemory = TRUE;
   Array_Domain_Type::smartReleaseOfInternalMemory = 1; 
 ...
 floatArray E(Ispan,Jspan,Kspan,Lspan,Mspan); 
 floatArray F(Ispan,Jspan,Kspan,Lspan,Mspan,Nspan); 
 ...
 float m1; float m2; float m3; float m4; float m5; float m6; 
 //TYPE m1,m2,m3,m4,m5;
  ...
  if (((m1 != 9.0 || m2 != 12.0) || m3 != -5.0) || m4 != 7.0)
  { 
    cout<<("ERROR in max(X), test1")<<(endl)<<("  m1,m2,m3,m4=")<<(m1)<<(" ")<<(m2)<<(" ")
    <<(m3)<<(" ")<<(m4)<<(endl); 
  ...
  F(7,5,-3,101,100,103) = 9; 
  where (B>6){ m5 = max(F(all,all,all,Internal_Index(101),Internal_Index(100),
  Internal_Index(103))); 
  ...
  if ((((m1 != 4.0 || m2 != 6.0) || m3 != -5.0) || m4 != 5.0)
   || m5 != 9.0){ 
    cout<<("ERROR in max(X), test2")<<(endl)<<("  m1,m2,m3,m4,m5=")<<(m1)<<(" ")<<(m2)
    <<(" ")<<(m3)<<(" ")<<(m4)<<(" ")<<(m5)<<(endl); 
 ...
 floatArray FF(3); 
  ...
  where (FF<7){ m1 = max((A(Internal_Indirect_Addressing_Index(I),Internal_Indirect_Addressing_Index(J),
  Internal_Indirect_Addressing_Index(-3),Internal_Indirect_Addressing_Index(103))+2)); 
  ...
  where (FF>6){ m2 = max((A(Internal_Indirect_Addressing_Index(I),Internal_Indirect_Addressing_Index(J),
  Internal_Indirect_Addressing_Index(-3),Internal_Indirect_Addressing_Index(103))+2)); 
  ...
  if (m1 != 6 || m2 != 3)
  { 
    cout<<("ERROR in max(X), test5")<<(endl)<<("  m1,m2=")<<(m1)<<(" ")<<(m2)<<(endl); 
 ...
 float sum1; float sum2; 
  ...
  { 
    ...
    /*------------------------------------------------------------*/
  T5=sqrt(A(Internal_Indirect_Addressing_Index(I),Internal_Indirect_Addressing_Index(J),
  Internal_Indirect_Addressing_Index(-6),Internal_Indirect_Addressing_Index(100)));   // all elements for k fixed at -6 are 1
 ...
 floatArray F1(A(all,all,Internal_Index(-6),Internal_Index(100))-C);  // view of A includes only
 ...
 floatArray F2(Ispan,Jspan,Kspan); 
  F2=0; 
  where (B>6){ F2=B-D; }   // B-D = B-(12-B) = 2*B-12 so masked 
 ...
 floatArray F3(Ispan,Jspan,Kspan); 
  F3=A(Is,Js,Ks,Internal_Index(103))-(B+D); 
  ...
  if (sum1 != m1)
  { 
    cout<<("ERROR in Lhs-Rhs,test3")<<(endl)<<("  sum1=")<<(sum1)<<(endl); 
 ...
 floatArray G1; 
  G1.redim(A(all,all,all,Internal_Index(103))); 
  G1=0; 
  ...
  //APP_DEBUG = 4;
  where (A(all,all,all,Internal_Index(102))>7){ G1=A(all,all,all,Internal_Index(104))
  +B; 
  }   //APP_DEBUG = 0;
 ...
 floatArray P5(11,11,6); 
  P5=D; 

 floatArray P6; 
 //P6 = -12 + (P5+P4)(all,all,3);
  P6=-12+floatArray((P5+P4))(all,all,Internal_Index(3)); 
  ...
  if (sum1 != m1)
  { 
    cout<<("ERROR in A=x+Rhs,test1")<<(endl)<<("  sum1=")<<(sum1)<<(endl); 
 ...
 floatArray N1(Ispan,11); 
  ...
  // N2 is -5
#else
 floatArray N2(N1.getDataPointer(),N1.getLength(0),N1.getLength(1),N1.getLength(2),N1.getLength(3)); 
 // N2 is -5
#endif
 floatArray N3; 
...
#else
 floatArray BB(bptr,Ispan,Jspan,Kspan); 
#endif
 ...
 floatArray H1; floatArray H2; 
  ...
  H1.reference(floatArray((B-D))(all,all,Internal_Index(-6)));   // H1 has elements -8,-6,...,10,12
 ...
 floatArray ONE(Ispan,Jspan,Kspan); 
  ONE=1; 
 ...
 floatArray W2; 
  ...
  W2=sum(floatArray(B(all,JJ,Internal_Index(-3))*ONE(all,JJ,Internal_Index(-3))),1); 
  ...
  if (sum1 != m1 || sum2 != m2)
  { 
    cout<<("ERROR in A=sum(X,axis),test1")<<(endl)<<("  sum1=")<<(sum1)<<(", sum2=")<<(sum2)
    <<(endl); 
    cout<<("  correct answer is: sum1=")<<(m1)<<(", sum2=")<<(m2)<<(endl); 
  } 
  else 
    { cout<<("Not implemented in expression template version: A=sum(X,axis) , test1 ok")
    <<(endl); } 
#endif

    // ... test A = transpose (X) (only works for 2D arrays) ...

 floatArray V1; floatArray V2; 
  V1=transpose(B(all,all,Internal_Index(-6)));   // V1 is an 11 x 11 array with
  // elements varying with j ranging
  // from 2 to 12
  V2=V1(all,Internal_Index(0));   // V2 is an 11 x1 array that has 2 for its
  // elements
  ...
  if (sum1 != m1)
  { 
    cout<<("ERROR in A=transpose(X),test1")<<(endl)<<("  sum1=")<<(sum1)<<(endl); 
 ...
 floatArray CCC(0); 
  CC.resize(11,11); 
  CCC.reference(CC(Range(5,7),Internal_Index(7))); 
  CCC.redim(0); 
  CC(5,5) = 7; 
 ...
 floatArray a(Range(-2,2),Range(-2,2)); 
  for (j = -2; j <= 2; j++)
     { for (i = -2; i <= 2; i++)
        { a(i,j) = i + 5 * j; } 
  } 
 ...
 float * aptr; 
  ...
  { 
    ...
    cout<<("   global setBase probably didn\047t work correctly")<<(endl); 
  } 

 floatArray ZSerial(ZZ.getLocalArray()); 
  ZSerial=22; 
  ZZ.display("ZZ"); 

 floatArray ZZSerial(ZZ.getLocalArrayWithGhostBoundaries()); 
  ZSerial=15; 
