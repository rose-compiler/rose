...
bool pointerAliasLattice::meetUpdate(Lattice* that_arg)
{
    ...
    //Union Compact Representation Graphs
    ...
    {
       //Dbg::dbg<<"Search value :"<<*al<<endl;
       //set::find() doesnt work well on pairs for some reason. Adding a search function for now
       if(thisAlias.find(*al) == thisAlias.end()){
         Dbg::dbg<<"Not found "<<*al<<" Adding it to this";  
         this->setAliases(*al);
         modified = true;
        }
    }
...
void pointerAliasLattice::printLattice(set<varID> vars)
{
    for(set<varID>::iterator item = vars.begin(); item!=vars.end(); item++)
    {
        Dbg::dbg<< *item <<",";
    }
}

bool pointerAliasLattice::widenUpdate(InfiniteLattice* that)
{}
...
void pointerAliasAnalysisTransfer::visit(SgAssignStatement *sgn)
{}


void pointerAliasAnalysisTransfer::visit(SgIntVal *sgn)
   {
   }
...
void pointerAliasAnalysisTransfer::visit(SgConditionalExp *sgn)
{}

void pointerAliasAnalysisTransfer::visit(SgConstructorInitializer *sgn)
{}

void pointerAliasAnalysisTransfer::visit(SgReturnStmt *sgn)
{
}
...
//process left hand side of expressions and calculate the dereference count
...
//Process aliases for Right hand side of expression and calculate derefernce count as well
