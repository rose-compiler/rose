...
int main() {
       ...
       for (unsigned int j = 14; j < 19; j++) {
           vidComplex[j] = boost::add_vertex(*graph4);
       }
       for (unsigned int j = 19; j < 21; j++) {
           vidComplex[j] = boost::add_vertex(*graph5);
       }
       ...
       EdgeID g27;
       boost::tie(g27, ok) = boost::add_edge(vidComplex[19], vidComplex[20], *graph5);
       EdgeID g28;
       boost::tie(g28, ok) = boost::add_edge(vidComplex[20], vidComplex[19], *graph5);
...
visitorTraversal* xis4 = new visitorTraversal;
       ...
       xis4->constructPathAnalyzer(graph4, false, vidComplex[14], vidComplex[18]);
